...
{
public:
	static OctTree<POINT>* createOctTree(POINT& ll, POINT& ur,
	...
	bool addPoint(POINT* pnt)
	{
		...
		if (_pnts.size() > OctTree<POINT>::_max_points_per_node) splitNode();
		return true;
	}
	 ...
	 * @param pnts
	 */
	void getPointsInRange(POINT const& min, POINT const& max,
