...
**************************************************************************/
void Surface::output(FILE* geo_file, int& p_index, int& l_index, int& pl_index,
                     int& s_index)
{
	int orient = 1;
	...
	// polyline_of_surface_list.begin();//CC
			...
			if (CGLn->mesh_index < 0)
			{
				...
				for (j = 0; j < Size0; j++)
				{
					...
					if (CGLn0->mesh_index >= 0)
						if (((CGLn->m_point1->PointDis(CGLn0->m_point1) <
						      DistTol) &&
						     (CGLn->m_point2->PointDis(CGLn0->m_point2) <
						      DistTol)) ||
						    ((CGLn->m_point1->PointDis(CGLn0->m_point2) <
						      DistTol) &&
						     (CGLn->m_point2->PointDis(CGLn0->m_point1) <
						      DistTol)))
						{
	...
	while (p != polyline_of_surface_vector.end())  // CC
	{
		...
		std::vector<CGLLine*>::iterator pl =
		    p_pline->line_vector.begin();  // WW NULL;
		if (orient > 0)
		{
			...
			while (pl != p_pline->line_vector.end())
			{
				...
				if (count == 0)
				{
					...
					if (CGLn->m_point2->PointDis(CGPnt) < DistTol) ortL = -1;
				}
				else
				{
					if (ortL0 > 0)
		...
		else
		{
			...
			while (pl != p_pline->line_vector.begin())  // CC
			{
				...
				CGLn = CGLn->CheckLineOutPut();
				int p_pline_line_list_size =
				    (int)p_pline->line_vector.size();  // CC
				...
				else
				{
					...
					else if (CGLn0->m_point1->PointDis(CGLn->m_point1) <
					         DistTol)
						ortL = 1;
					// else ortL = -1;//CC
				}
...
void Surface::Write(const std::string& path_name)
{
	...
	else
	{
		...
		fprintf(gli_file, "  %i\n", mat_group);
		//-----------------------------------------------------------------
		int polyline_of_surface_list_length =
		    (int)polyline_of_surface_vector.size();  // CC
		if (polyline_of_surface_list_length > 0)
		{
			fprintf(gli_file, " %s\n", "$POLYLINES");
			CGLPolyline* m_polyline = NULL;
			//   list<CGLPolyline*>::iterator p =
			//   m_surface->polyline_of_surface_list.begin();//CC
			std::vector<CGLPolyline*>::iterator p =
			    polyline_of_surface_vector.begin();
			while (p != polyline_of_surface_vector.end())  // CC
			{
				m_polyline = *p;
		...
		while (p != polyline_of_surface_vector.end())
		{
			...
			if (Firstply == true)
			{
...

				{
					...
					polyline_of_surface_vector.erase(
					    polyline_of_surface_vector.begin() + ite2);
...
void Surface::PolylineOrientation()
{
	...
	while (p != polyline_of_surface_vector.end())
	{
		...
		{
			m_line = *pl;
			if (m_line->point1 == line_end->point2 ||
			    m_line->point1 == line_end->point1
...
void GEOSurfaceTopology(void)
{
	for (size_t i = 0; i < surface_vector.size(); i++)
	{
		...
		if (sfc->polyline_of_surface_vector.size() == 0) continue;
		if (sfc->order == false)
		{
			for (size_t j = 0; j < sfc->polyline_of_surface_vector.size(); j++)
			{
				...
				if (ply->getDataType() == 1)  // OK not a toplogical polyline
					continue;
				// compute lines and put into polyline line, here all
				// orientation is 1
				ply->ComputeLines();
			}
			//..................................................................
			...
			//..................................................................
			// Check start and end point of polyline, change the orientation
			// value
...
void Surface::CreateTIN(void)
{
	...
	{               //  list<CGLPolyline*>::const_iterator  p =
		            //  polyline_of_surface_list.begin();
		std::vector<CGLPolyline*>::iterator p =
		    polyline_of_surface_vector.begin();
	...
	//----------------------------------------------------------------------
	// Memory
	if (TIN->Triangles.size() == 0) delete TIN;
}
/**************************************************************************
   GeoLib-Method:
   Task:
	...
	while (!new_keyword)
	{
		...
		if (line_string.find("$POLYLINES") != string::npos)  // subkeyword found
			...
			while (ok_true)
			{
				...
				// type = m_polyline->data_type; //MB Please set surface type
				// with surface type, not polyline type
				else
...
void Surface::ReadTIN(const std::string& tin_file_name)
{
	...
	/*  //11.08.2011 WW
	   //  tin_file_name_path = FileName;  //WW/JOD // LB: commented out to
	   compile GEO without dependency on FEM
	   basic_string <char>::size_type indexCh1a;
	   indexCh1a = tin_file_name_path.find_last_of("\\");

	   if( indexCh1a < tin_file_name_path.size()) { // JOD 4.7.10,  \ exists in
	   path, DEBUG case
...
void Surface::WriteTIN(const std::string& file_path)
{
	...
	long i;
	//----------------------------------------------------------------------
	if (TIN && ((long)TIN->Triangles.size() == 0)) return;
	//----------------------------------------------------------------------
	// File handling
	string tin_path;
	tin_path = file_path;
	...
	for (i = 0; i < (long)TIN->Triangles.size(); i++)
		tin_file << TIN->Triangles[i]->number << delimiter
...
/**************************************************************************
   ...
   last modification:08/2005 CC
**************************************************************************/
void GEOCreateLayerSurfaceTINs(int nb_prism_layers,
                               double thickness_prism_layers)
{
	long i, j;
		...
		if (m_surface->type == 0)
			...
			for (i = 0; i < nb_prism_layers; i++)
			{
				...
				for (j = 0; j < no_TIN_Triangles; j++)
				{
					...
					m_triangle->z[0] = m_surface->TIN->Triangles[j]->z[0] -
					                   (i + 1) * thickness_prism_layers;
					m_triangle->z[1] = m_surface->TIN->Triangles[j]->z[1] -
					                   (i + 1) * thickness_prism_layers;
					m_triangle->z[2] = m_surface->TIN->Triangles[j]->z[2] -
					                   (i + 1) * thickness_prism_layers;
...
void Surface::WriteTINTecplot(const std::string& file_path)
{
	...
	if (TIN && (TIN->Triangles.size() > 0))
	{
		...
		tin_file << "ZONE T = " << TIN->name << delimiter << "N = " << no_nodes
		         << delimiter << "E = " << no_triangles << delimiter
		         << "F = FEPOINT" << delimiter << "ET = TRIANGLE" << endl;
		for (i = 0; i < no_triangles; i++)
		{
			tin_file << TIN->Triangles[i]->x[0] << " "
			         << TIN->Triangles[i]->y[0] << " "
			         << TIN->Triangles[i]->z[0] << endl;
			tin_file << TIN->Triangles[i]->x[1] << " "
			         << TIN->Triangles[i]->y[1] << " "
			         << TIN->Triangles[i]->z[1] << endl;
			tin_file << TIN->Triangles[i]->x[2] << " "
			         << TIN->Triangles[i]->y[2] << " "
			         << TIN->Triangles[i]->z[2] << endl;
		}
		for (i = 0; i < no_triangles; i++)
	...
	if (m_polyline1)
		for (i = 0; i < no_points; i++)
			*tec_file << m_polyline1->point_vector[i]->x << " "
			          ...
			          << m_polyline1->point_vector[i]->z << " " << endl;

	if (m_polyline2)
		for (i = 0; i < no_points; i++)
			*tec_file << m_polyline2->point_vector[i]->x << " "
			          << m_polyline2->point_vector[i]->y << " "
			          << m_polyline2->point_vector[i]->z << " " << endl;
...
Surface* GEOGetSFCByName(const std::string& name)
{
	...
	{
		m_sfc = *p;
		if (m_sfc->name.compare(name) == 0) return m_sfc;
		++p;
	}
	return NULL;
}
...
void Surface::PolygonPointVector()
{
	...
	else
	{
		...
		while (p1 != polyline_of_surface_vector.end())
		{
			...
			{
				vector<CGLLine*>::iterator pb =
				    gl_polyline->line_vector.begin();
				vector<CGLLine*>::iterator pe = gl_polyline->line_vector.end();
				if (gl_polyline->line_vector.size() == 0) return;
				if (polyline_of_surface_orient[LocalPlyIndex] == 1)
					while (pb != gl_polyline->line_vector.end())
					{
						gl_line = *pb;
						polygon_point_vector.push_back(
						    GEOGetPointById(gl_line->point2));
					...
					while (pe != gl_polyline->line_vector.begin())
					{
						gl_line = *(--pe);
						polygon_point_vector.push_back(
						    GEOGetPointById(gl_line->point1));
						++i;
					}
			}
			//....................................................................
			...
			{
				vector<CGLPoint*>::iterator pv =
				    gl_polyline->point_vector.begin();
