...
namespace GEOLIB
{
Polygon::Polygon(const Polyline& ply, bool init) : Polyline(ply)
{
	if (init) initialise();
}

Polygon::Polygon(const std::vector<Point*>& pnt_vec) : Polyline(pnt_vec)
{
}
...
bool Polygon::initialise()
{
	...
	{
		std::cerr
		    << "ERROR in Polygon::initialise() - base polyline is not closed"
		    << "\n";
		return false;
	}
}
...
bool Polygon::isPartOfPolylineInPolygon(const Polyline& ply) const
{
	...
	for (size_t k(0); k < ply_size - 1; k++)
	{
		...
		{
			if (MathLib::lineSegmentIntersect(
			        *(getPoint(j)), *(getPoint(j + 1)), *(ply.getPoint(k)),
			        *(ply.getPoint(k + 1)), *s))
			{
				delete s;
				return true;
...
bool Polygon::getNextIntersectionPointPolygonLine(
    GEOLIB::Point const& a, GEOLIB::Point const& b,
    GEOLIB::Point* intersection_pnt, size_t& seg_num) const
{
	if (_simple_polygon_list.empty())
	{
		const size_t n_nodes(getNumberOfPoints() - 1);
		...
		{
			if (MathLib::lineSegmentIntersect(*(getPoint(k)),
			                                  *(getPoint(k + 1)), a, b,
			                                  *intersection_pnt))
			{
				seg_num = k;
	...
	else
	{
		     ...
		     ++it)
		{
			...
			const size_t n_nodes_simple_polygon(polygon->getNumberOfPoints() -
			                                    1);
			for (size_t k(0); k < n_nodes_simple_polygon; k++)
			{
				if (MathLib::lineSegmentIntersect(*(polygon->getPoint(k)),
				                                  *(polygon->getPoint(k + 1)),
...
{
	if (_simple_polygon_list.empty()) _simple_polygon_list.push_back(this);
	return _simple_polygon_list;
}

void Polygon::computeListOfSimplePolygons()
{
	if (!_simple_polygon_list.empty()) return;
...
void Polygon::ensureCWOrientation()
{
	...
	if (0 < min_x_max_y_idx && min_x_max_y_idx < n_pnts - 2)
		orient = MathLib::getOrientation(tmp_polygon_pnts[min_x_max_y_idx - 1],
		                                 tmp_polygon_pnts[min_x_max_y_idx],
		                                 tmp_polygon_pnts[min_x_max_y_idx + 1]);
	else
	{
		if (0 == min_x_max_y_idx)
			orient = MathLib::getOrientation(tmp_polygon_pnts[n_pnts - 1],
			                                 tmp_polygon_pnts[0],
			                                 tmp_polygon_pnts[1]);
		else
			orient = MathLib::getOrientation(tmp_polygon_pnts[n_pnts - 2],
    ...
    std::list<Polygon*>::iterator polygon_it)
{
	...
	while (polygon_it != _simple_polygon_list.end())
	{
		...
		bool is_simple(!MathLib::lineSegmentsIntersect(*polygon_it, idx0, idx1,
		                                               *intersection_pnt));
		if (!is_simple)
		{
			...
			const_cast<std::vector<Point*>&>(_ply_pnts)
			    .push_back(intersection_pnt);

			// split Polygon
			if (idx0 > idx1) BASELIB::swap(idx0, idx1);

			GEOLIB::Polygon* polygon0(
			    new GEOLIB::Polygon((*polygon_it)->getPointsVec(), false));
...
void Polygon::splitPolygonAtPoint(
    std::list<GEOLIB::Polygon*>::iterator polygon_it)
{
	...
	for (size_t k(0); k < n - 1; k++)
		if (id_vec[k] == id_vec[k + 1])
		{
			...
			std::list<GEOLIB::Polygon*>::iterator polygon0_it, polygon1_it;
			polygon1_it = _simple_polygon_list.insert(
			    _simple_polygon_list.erase(polygon_it), polygon1);
			polygon0_it = _simple_polygon_list.insert(polygon1_it, polygon0);
...
{
	if (lhs.getNumberOfPoints() != rhs.getNumberOfPoints()) return false;
	...
	else
	{
		...
		// position
		// *** ATTENTION
		std::cerr << "operator==(Polygon const& lhs, Polygon const& rhs) - not "
		             "tested case (implementation is probably buggy) - please "
		             "contact thomas.fischer@ufz.de mentioning the problem"
		          << "\n";
		// in second polygon
		if (lhs.getPointID(1) == rhs.getPointID(k - 1))
		{
