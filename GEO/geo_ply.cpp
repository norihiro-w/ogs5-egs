...
**************************************************************************/
CGLPolyline::CGLPolyline(const std::string& ply_name) : name(ply_name)
{
	//	closed = false;
	epsilon = 0.01;
	type = 2;
...
void GEOPolylineGLI2GEO(FILE* geo_file)
{
	...
	for (i = 0; i < (int)surface_vector.size(); i++)
	{
		if (!surface_vector[i]->meshing_allowed)  // OK/TK
			continue;
		s_counter++;
		surface_vector[i]->output(geo_file, p_counter, l_counter, pl_counter,
		                          s_counter);
	}
	//----------------------------------------------------------------------
	// Null the index
...
CGLPoint* CGLPolyline::CenterPoint(void)
{
	...
	if (polyline_point_vector_length == 0) return NULL;
...
void CGLPolyline::SetPointOrderByDistance(CGLPoint* m_pnt)
{
	...
	if ((int)point_vector_aux.size() == 0) return;
	for (i = 0; i < (int)point_vector.size(); i++)
	{
		m_pnt_i = point_vector[i];
		if (m_pnt_i == point_vector_aux[0]) continue;
		point_vector_aux.push_back(m_pnt_i);
	}
	point_vector.clear();
...
CColumn* COLGet(int col_id)
{
	...
	{
		m_col = column_vector[i];
		if (m_col->getMatGroup() == col_id) return m_col;
	}
	return NULL;
}
...
from CGLPoint-to-CGLPoint
   along the entire polyline.
   Argument:
   Ergebnis:
   double poly_length : Length of the polyline, along the points, not from end
to end
   Programing:
