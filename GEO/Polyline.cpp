	...
	if (_ply_pnt_ids.size() > 1)
	{
		...
		else
		{
			if (pos == _ply_pnt_ids.size() - 1)
			{
				...
				_length.insert(_length.begin() + pos,
    ...
    const std::vector<Polyline*>& ply_vec, double prox)
{
	...
	while (!local_ply_vec.empty())
	{
		     ...
		     ++it)
		{
			if (pnt_vec == (*it)->getPointsVec())
			{
				...
				// == (*it)->getPointID(0))
				             ...
				             pnt_vec, new_ply->getPointID(
				                          new_ply->getNumberOfPoints() - 1),
				             (*it)->getPointID(0), prox))
				{
				...
				std::cout << "Error in "
				             "Polyline::contructPolylineFromSegments() - Line "
		...
		if (!ply_found)
		{
			std::cout << "Error in Polyline::contructPolylineFromSegments() - "
			             "Not all segments are connected..."
			          << "\n";
			new_ply = NULL;
			break;
		}
...
Polyline* Polyline::closePolyline(const Polyline& ply)
{
	...
	std::cout << "Error in Polyline::closePolyline() - Input polyline needs to "
	             "be composed of at least three points..."
	          << "\n";
	return NULL;
}

Location::type Polyline::getLocationOfPoint(size_t k,
...
bool containsEdge(const Polyline& ply, size_t id0, size_t id1)
{
	...
	for (size_t k(0); k < n; k++)
	{
		...
		if (ply_pnt0 > ply_pnt1) BASELIB::swap(ply_pnt0, ply_pnt1);
		if (ply_pnt0 == id0 && ply_pnt1 == id1) return true;
	}
	return false;
}

bool isLineSegmentIntersecting(const Polyline& ply, GEOLIB::Point const& s0,
                               GEOLIB::Point const& s1)
{
	...
	{
		intersect = MathLib::lineSegmentIntersect(*(ply.getPoint(k)),
		                                          *(ply.getPoint(k + 1)), s0,
		                                          s1, intersection_pnt);
	}
	return intersect;
...
bool operator==(Polyline const& lhs, Polyline const& rhs)
{
	...
	for (size_t k(0); k < n; k++)
	{
		if (lhs.getPointID(k) != rhs.getPointID(k)) return false;
	}
