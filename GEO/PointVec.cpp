...
void PointVec::push_back(Point* pnt, std::string const* const name)
{
	...
	{
		std::cerr
		    << "ERROR: PointVec::push_back (): two points with the same name"
		    << "\n";
		return;
	}
                              ...
                              std::vector<size_t>& pnt_id_map, double eps)
{
	...
	// determine intervals with identical points to resort for stability of
	// sorting
	...
	//			cnt++;
	//		} else reg_ids.insert(std::pair<size_t, size_t>(k,
	// reg_ids[pnt_id_map[k]]));
	//	}
	//	for (size_t k(0); k < n_pnts_in_file; k++)
...
std::vector<GEOLIB::Point*>* PointVec::getSubset(
    const std::vector<size_t>& subset)
{
	std::vector<GEOLIB::Point*>* new_points(
	    new std::vector<GEOLIB::Point*>(subset.size()));

	const size_t nPoints(subset.size());
	for (size_t i = 0; i < nPoints; i++)
		(*new_points)[i] = new GEOLIB::PointWithID(
		    (*this->_data_vec)[subset[i]]->getData(), subset[i]);
