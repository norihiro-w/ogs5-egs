...
CGLPoint::CGLPoint(double x1, double y1, double z1)
...
CGLPoint::CGLPoint(const double* coordinates)
    : x(data[0]), y(data[1]), z(data[2])
{
...
int CGLPoint::IsPointExist()
{
	...
	double tolerance = 1e-6;  // This is very subjective in digital numbers that
	                          // represent real numbers.
	// So, what ever points in what scale, always use OpenGL coordinates here.
	int YesThereIs =
	    -100;  // Set this to mean there is no duplicate in the file

	int sizeOfPointList = (int)gli_points_vector.size();
	for (int i = 0; i < sizeOfPointList; ++i)
	{
		double Same =
		    sqrt((gli_points_vector[i]->x - x) * (gli_points_vector[i]->x - x) +
		...
		if (Same < tolerance) YesThereIs = gli_points_vector[i]->id;
	}
...
CGLPoint* GEOGetPointById(long number)
{
	...
	else
	{
		...
		{
			m_point = *p;
			if (m_point->id == number) return m_point;
			++p;
		}
	}
	return NULL;
...
**************************************************************************/
std::vector<CGLPoint*> GEOLIB_SetGLIPoints_Vector(
    std::vector<CGLPoint*> gl_point)
{
...
void GEO_Search_DoublePoints(double tolerance)
{
	...
	for (j = 0; j < pointsvectorsize; j++)
	{
		...
		for (k = 0; k < j; k++)
		{
			...
			point_distance = EuklVek3dDistCoor(x2check, y2check, z2check,
			                                   x2check0, y2check0, z2check0);
			...
			if (hits > 0 && fabs(x2check0 - x2check) < MKleinsteZahlen &&
			    fabs(y2check0 - y2check) < MKleinsteZahlen &&
...
bool CGLPoint::IsInsidePolygonPlain(double* xp, double* yp, double* zp, long np)
{
	...
	for (k = 1; k < np; k++)
	{
		...
		if (norm_dn <= d_eps || norm_dp <= d_eps) return true; /* auf Rand */
...
{
	return sqrt((x - m_pnt->x) * (x - m_pnt->x) +
	            (y - m_pnt->y) * (y - m_pnt->y));
}
...
int GEOMaxPointID()
{
	...
	for (int i = 0; i < (long)gli_points_vector.size(); i++)
	{
		...
		if (m_pnt->id > max_pnt_id) max_pnt_id = m_pnt->id;
	}
	return max_pnt_id;
}
		...
		for (i = 0; i < gli_points_vector_size; i++)
			if (gli_points_vector[i]->id > -1)  // OK
				fprintf(f, " %ld %#16.11g %#16.11g %#16.11g $MD %g $ID %s\n",
				        gli_points_vector[i]->id, gli_points_vector[i]->x,
				        gli_points_vector[i]->y, gli_points_vector[i]->z,
...
**************************************************************************/
double AngleSumPointInsideTriangle(double* point,
                                   double* tri_p1,
                                   double* tri_p2,
                                   double* tri_p3,
                                   double tolerance)
{
	...
	/*Angle: at P0 -P1P2*/
	cos_alpha[0] = ((a[0] * b[0]) + (a[1] * b[1]) + (a[2] * b[2])) /
	               (a_quantum * b_quantum);
	...
	/*Angle: at P0 -P2P3*/
	cos_alpha[1] = ((b[0] * c[0]) + (b[1] * c[1]) + (b[2] * c[2])) /
	               (b_quantum * c_quantum);
	...
	/*Angle: at P0 -P3P1*/
	cos_alpha[2] = ((c[0] * a[0]) + (c[1] * a[1]) + (c[2] * a[2])) /
	               (c_quantum * a_quantum);
	...
	if ((point[0] == tri_p1[0] && point[1] == tri_p1[1] &&
	     point[2] == tri_p1[2]) ||
	    (point[0] == tri_p2[0] && point[1] == tri_p2[1] &&
	     point[2] == tri_p2[2]) ||
	    (point[0] == tri_p3[0] && point[1] == tri_p3[1] &&
	     point[2] == tri_p3[2]))
		sum = 360;
...
{
	double v[3] = {p1->getX() - p0->getX(), p1->getY() - p0->getY(),
	               p1->getZ() - p0->getZ()};
	return MathLib::scpr(v, v, 3);
}

CGLPoint* isPointInPointVector(const std::vector<CGLPoint*>& vec,
                               const CGLPoint* const p,
                               double tol)
{
	size_t size(vec.size());
	for (size_t i = 0; i < size; ++i)
		if (sqrDist(p, vec[i]) < tol) return vec[i];
	return NULL;
}
