...
//		edges[k] = new CNode ((elem.nodes[k])->GetIndex(), (elem.nodes[k])->X(),
//(elem.nodes[k])->Y(), (elem.nodes[k])->Z());
//	}
...
CElem::~CElem()
{
	...
	owner = NULL;
	if (transform_tensor) delete transform_tensor;
	transform_tensor = NULL;
#ifndef OGS_ONLY_TH
...
#endif
	transform_tensor = NULL;
	if (normal_vector) delete[] normal_vector;
	normal_vector = NULL;
#ifndef OGS_ONLY_TH
	delete[] angle;
#endif
...
{
	if (!normal_vector) normal_vector = new double[3];  // WW
	...
	if (this->GetElementType() == MshElemType::QUAD)  // BG 04/2011: calculation
	                                                  // of the normal vector of
	                                                  // a quad element
	{
		double const* const p0(nodes[0]->getData());
		double const* const p1(nodes[1]->getData());
...
void CElem::setElementProperties(MshElemType::type t, bool isFace)
{
	switch (t)
	{
		...
		case MshElemType::QUAD:
			nnodes = 4;
			nnodesHQ = (isFace) ? 8 : 9;  // if a QUAD is the face of a hex it
			                              // has 8 nodes, otherwise it has 9
			ele_dim = 2;
			geo_type = MshElemType::QUAD;
			nfaces = 4;
			nedges = 4;
