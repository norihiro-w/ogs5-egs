...
GridAdapter::GridAdapter(const MeshLib::CFEMesh* mesh)
    : _name(""),
      _nodes(new std::vector<GEOLIB::Point*>),
      _elems(new std::vector<Element*>),
      _mesh(mesh)
{
      ...
      _mesh(NULL)
{
	if (grid)
	{
		...
		const std::vector<GridAdapter::Element*>* elements =
		    grid->getElements();
...
int GridAdapter::convertCFEMesh(const MeshLib::CFEMesh* mesh)
{
	...
	for (size_t i = 0; i < nElems; i++)
	{
		...
		if (newElem->type != MshElemType::INVALID)
		{
			...
			std::cout << "GridAdapter::convertCFEMesh() - Error recognising "
			             "element type..."
			          << "\n";
	}
...
const MeshLib::CFEMesh* GridAdapter::toCFEMesh() const
{
	...
	size_t nNodes = _nodes->size();
	for (size_t i = 0; i < nNodes; i++)
		mesh->nod_vector.push_back(new MeshLib::CNode(
		    i, (*(*_nodes)[i])[0], (*(*_nodes)[i])[1], (*(*_nodes)[i])[2]));
	...
	while (getline(in, line))
	{
		...
		if (fields.size() >= 4)
		{
			...
			if (atoi(it->c_str()) == (int)_nodes->size())
			{
				...
				std::cout << "GridAdapter::readMeshFromFile() - Index error "
				             "while reading nodes..."
				          << "\n";
		}
		else
			std::cout << "GridAdapter::readMeshFromFile() - Error reading node "
			             "format..."
			          << "\n";
	}
				...
				if (atoi(it->c_str()) == (int)_elems->size())
				{
					...
					if (newElem->type != MshElemType::INVALID)
					{
						while ((++it) != fields.end())
						{
							...
							newElem->nodes.push_back(
							    atoi(it->c_str()));  // next node id
						}

						_elems->push_back(newElem);
					}
					else
						std::cout << "GridAdapter::readMeshFromFile() - Error "
						             "recognising element type..."
						          << "\n";
				}
				else
					std::cout << "GridAdapter::readMeshFromFile() - Index "
					             "error while reading element " << *it << "... "
					          << "\n";
			}
			else
				std::cout << "GridAdapter::readMeshFromFile() - Error reading "
				             "element format..."
				          << "\n";
		}
	}
	else
		std::cout << "GridAdapter::readMeshFromFile() - Index error after "
		             "reading nodes..."
		          << "\n";
...
const std::vector<GridAdapter::Element*>* GridAdapter::getElements(
    size_t matID) const
{
	std::vector<GridAdapter::Element*>* matElems =
	    new std::vector<GridAdapter::Element*>;
	size_t nElements = _elems->size();
	for (size_t i = 0; i < nElements; i++)
		if ((*_elems)[i]->material == matID) matElems->push_back((*_elems)[i]);
