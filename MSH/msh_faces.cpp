                                                       ...
                                                       const double Point3[3])
{
	...
	this->normal_vector[0] = this->vector1[1] * this->vector2[2] -
	                         this->vector1[2] * this->vector2[1];
	this->normal_vector[1] = this->vector1[2] * this->vector2[0] -
	                         this->vector1[0] * this->vector2[2];
	this->normal_vector[2] = this->vector1[0] * this->vector2[1] -
	                         this->vector1[1] * this->vector2[0];

	// norm the normal vector to 1
	double norm =
	    sqrt(MathLib::scpr(this->normal_vector, this->normal_vector, 3));
	...
	// Point2[0] << ", " << Point2[1] << ", " << Point2[2] << " " << Point3[0]
	// << ", " << Point3[1] << ", " << Point3[2];
	// Calculate Lambda for Normal equation of the plane (= scalar product of
	// normal vector and one point of the plane)
	this->Lambda_NormalEquation = this->Point[0] * this->normal_vector[0] +
	                              this->Point[1] * this->normal_vector[1] +
...
{
	(void)Point;
	// the point is included in the normal equation of the plane, it is part of
	// the plane if the equation is fullfilled
	if (this->Lambda_NormalEquation ==
	    this->Point[0] * this->normal_vector[0] +
	        this->Point[1] * this->normal_vector[1] +
	        this->Point[2] * this->normal_vector[2])
		return 1;
	else
...
/*-------------------------------------------------------------------------
   ...
   Task: Calculates the coordinates of the centre of each face of a
   EclipseBlock; Assumes at the moment regular hexahedron
   Return: nothing
   Programming: 09/2009 BG
   Modification:
   -------------------------------------------------------------------------*/
...
{
	// Order of the faces: 0..left(x); 1..right(x); 2..front(y); 3..back(y);
	// 4..bottom(z); 5..top(z)
	...
	// FaceGravityCentre is calculated for quadrilateral element by calculating
	// the gravitycentre for each triangle
	// the gravity-centre of the quadrilateral is the area weighted average of
	// both centres of the triangles
	...
	double a1 = 0.5 * sqrt((vec_a[1] * vec_b[2] - vec_a[2] * vec_b[1]) *
	...
	double a2 = 0.5 * sqrt((vec_c[1] * vec_b[2] - vec_c[2] * vec_b[1]) *
	                           (vec_c[1] * vec_b[2] - vec_c[2] * vec_b[1]) +
	                       (vec_c[2] * vec_b[0] - vec_c[0] * vec_b[2]) *
	                           (vec_c[2] * vec_b[0] - vec_c[0] * vec_b[2]) +
	                       (vec_c[0] * vec_b[1] - vec_c[1] * vec_b[0]) *
	                           (vec_c[0] * vec_b[1] - vec_c[1] * vec_b[0]));
...
/*-------------------------------------------------------------------------
   ...
   Task: Calculates the x-, y and z-components of a norm of a given vector with
   the normal vector
   Return: nothing
   Programming: 09/2009 BG
   Modification:
   -------------------------------------------------------------------------*/
void CFaces::Calculate_components_of_a_vector(int flag, int phase_index,
                                              bool Radialmodell)
{
	double* normal_vector;
	normal_vector = this->PlaneEquation->GetNormalVector();
	// contoll that normalvector is positiv in all directions, is only valid if
	// it is no radial model
	if (Radialmodell == false)
	{
		if ((this->model_axis == "I+") || (this->model_axis == "I-"))
			if (normal_vector[0] < 0.0)
			{
				std::cout << "Error at the normal vector. The i-component is "
				             "not positiv!"
				          << "\n";
				// system("Pause");
				exit(1);
		...
		if ((this->model_axis == "J+") || (this->model_axis == "J-"))
			...
			{
				std::cout << "Error at the normal vector. The j-component is "
				             "not positiv!"
				          << "\n";
				// system("Pause");
				exit(1);
	...
	for (int i = 0; i < 3; i++)
	{
		...
		if (flag == 0)
			this->phases[phase_index]->q[i] =
			    this->phases[phase_index]->q_norm * normal_vector[i];
...
/*-------------------------------------------------------------------------
   ...
   Modification:
   -------------------------------------------------------------------------*/
bool CFaces::CreateFace(MeshLib::CNode* Point1,
                        MeshLib::CNode* Point2,
                        MeshLib::CNode* Point3,
                        MeshLib::CNode* Point4)
{
	...
	this->PlaneEquation->CalculatePlaneEquationFrom3Points(
	    coord_Point1, coord_Point2, coord_Point3);
	...
	// the order of the point was changed to calculate the correct gravity
	// centre
	if (this->Calculate_FaceGravityCentre(coord_Point1, coord_Point2,
	                                      coord_Point4, coord_Point3) == false)
		return false;
