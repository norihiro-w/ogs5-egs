...
void MSHWriteVOL2TEC(std::string m_msh_name)
{
	...
	while (p_vol != volume_vector.end())
	{
		...
		std::string vol_file_name =
		    tec_path + "VOL_" + m_vol->name + TEC_FILE_EXTENSION;
			...
			if (m_ele->GetElementType() == MshElemType::PRISM)
			{
				...
				for (j = 0; j < 6; j++)
				{
					double const* const pnt(
					    m_msh->nod_vector[node_indeces[j]]->getData());
					x += pnt[0];
					y += pnt[1];
					z += pnt[2];
				}
		...
		for (long i = jb; i < je; i++)
		{
			...
			if (m_ele->GetElementType() == MshElemType::PRISM)
			{
				...
				//        if(m_sfc->PointInSurface(&m_point)){
				//          vol_file
				//            << node_indeces[0]+1 << " " << node_indeces[0]+1
				//            << " " << node_indeces[1]+1 << " " <<
				//            node_indeces[2]+1 << " "
				//            << node_indeces[3]+1 << " " << node_indeces[3]+1
				//            << " " << node_indeces[4]+1 << " " <<
				//            node_indeces[5]+1 << "\n";
				//        }
			}
...
void MSHWriteTecplot()
{
	...
	if (m_gsp) file_path = m_gsp->path + "MSH";
	//----------------------------------------------------------------------
...
void MSHLayerWriteTecplot()
{
	...
	for (int j = 0; j < (int)fem_msh_vector.size(); j++)
	{
		...
		for (size_t k = 0; k < m_msh->getNumberOfMeshLayers(); k++)
		{
			...
			// WW no_nodes = (long) m_msh->nod_vector.size() /
			...
			// File handling
			std::string msh_file_name = file_path + "MSH_LAYER" + no_layer_str +
			                            "_" + m_msh->pcs_name +
			                            TEC_FILE_EXTENSION;
			std::fstream msh_file(msh_file_name.data(),
			                      std::ios::trunc | std::ios::out);
			...
			switch (ele_type)
			{
				...
				case MshElemType::LINE:
					...
					for (size_t i = k * no_elements; i < (k + 1) * no_elements;
					     i++)
					{
						m_ele = m_msh->ele_vector[i];
						m_ele->GetNodeIndeces(node_indeces);
...
void SetRFIPointsClose(CGLLine* m_lin)
{
	...
	/*OK411
	   ...
	   anz_relevant++;
	   // Feld anpassen
	   m_lin->msh_nodes = (long *)
	   Realloc(m_lin->msh_nodes,sizeof(long)*anz_relevant);
	   relevant = (INFO *) Realloc(relevant, sizeof(INFO)*anz_relevant);
