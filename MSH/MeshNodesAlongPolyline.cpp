    ...
    : _ply(ply), _mesh(mesh), _linear_nodes(0)
{
	...
	// We need exactly defined polyline for DDC. If there is not any node
	// located within the
	...
	// WW while (_msh_node_ids.empty())
	{
		...
		for (size_t k = 0; k < ply->getNumberOfPoints() - 1; k++)
		{
			...
			for (size_t j = 0; j < n_nodes; j++)
			{
				...
				// line g(lambda) = _ply->getPoint(k) + lambda *
				// (_ply->getPoint(k+1) - _ply->getPoint(k))
				// at the k-th line segment of the polyline, i.e. 0 <= lambda <=
				// 1?
				if (MathLib::calcProjPntToLineAndDists(
				        mesh_nodes[j]->getData(),
				        (_ply->getPoint(k))->getData(),
				        (_ply->getPoint(k + 1))->getData(), lambda,
				        dist) <= epsilon_radius)
				{
						...
						else
						{
							    ...
							    msh_node_higher_order_ids.end())
							{
								...
								dist_of_proj_higher_order_node_from_ply_start
								    .push_back(act_length_of_ply + dist);
							}
						}
					}  // end if lambda
				}
...
//	"\n";
	...
	// append distances for higher order nodes at the end of vector
	// _dist_of_proj_node_from_ply_start
	for (size_t k(0); k < dist_of_proj_higher_order_node_from_ply_start.size();
	     k++)
		_dist_of_proj_node_from_ply_start.push_back(
...
MeshNodesAlongPolyline::getDistOfProjNodeFromPlyStart() const
{
