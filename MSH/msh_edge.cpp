...
/**************************************************************************
   ...
   03/2010 TF moved initialization of attributes to initialization list, added
docu
**************************************************************************/
CEdge::CEdge(size_t Index, bool quadr) : CCore(Index), nodes_of_edges(3)
{
	quadratic = quadr;
...
double CEdge::getLength() const
{
	...
	double const dx(pnt1[0] - pnt0[0]), dy(pnt1[1] - pnt0[1]),
	    dz(pnt1[2] - pnt0[2]);
...
bool CEdge::operator==(CEdge& ed)
{
	...
	identical = 0;
	for (int i = 0; i < 2; i++)
		if (nodes_of_edges[i] == ed.nodes_of_edges[i]) identical++;
	if (identical == 2) return true;

	identical = 0;
	for (int i = 0; i < 2; i++)
		if (nodes_of_edges[1 - i] == ed.nodes_of_edges[i]) identical++;
	if (identical == 2) return true;
...
void CEdge::GetEdgeVector(double* edge_vector)
{
	...
	double const* const pnt1(nodes_of_edges[1]->getData());
	edge_vector[0] =
	    pnt1[0] - pnt0[0];  // nodes_of_edges[1]->X() - nodes_of_edges[0]->X();
	edge_vector[1] =
	    pnt1[1] - pnt0[1];  // nodes_of_edges[1]->Y() - nodes_of_edges[0]->Y();
	edge_vector[2] =
	    pnt1[2] - pnt0[2];  // nodes_of_edges[1]->Z() - nodes_of_edges[0]->Z();
}
