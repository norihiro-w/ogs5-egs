...
/**************************************************************************
 ...
 Programing:
 01/2007 JOD Implementation
 10/2008 JOD overland node shifting for soil columns, averaging automatically
 4.7.10
 **************************************************************************/
#if !defined(USE_PETSC) && \
    !defined(NEW_EQS)  // && defined(other parallel libs)//03~04.3012. WW
                               ...
                               CNodeValue* cnodev)
{
	...
	GetCouplingFieldVariables(&h_this, &h_cond, &h_cond_shifted, &help, &z_this,
	                          &z_cond, m_st,
	                          cnodev);  // z_cond shifted for soil columns
	////////   coupling factor
	if (h_this > h_cond_shifted)
		relPerm = GetRelativeCouplingPermeability(
		    m_pcs_this, h_this, m_st,
		    cnodev->msh_node_number);  // groundwater or richards
	else
		relPerm = GetRelativeCouplingPermeability(
		    m_pcs_cond, h_cond, m_st,
	...
	if (m_st->getProcessType() == FiniteElement::GROUNDWATER_FLOW &&
	    h_this < z_cond && m_st->pcs_type_name_cond == "OVERLAND_FLOW")
		condArea = 0;
	...
	m_pcs_this->SetNodeValue(cnodev->msh_node_number,
	                         ...
	                         m_pcs_this->GetNodeValueIndex("COUPLING") + 1,
	                         h_this);
                               ...
                               CNodeValue* cnodev)
{
	...
	double epsilon = 1.e-7, value_jacobi, h_this_epsilon = 0.0, relPerm_epsilon,
	       condArea_epsilon;  // OK411 epsilon as in
	                          // pcs->assembleParabolicEquationNewton
	...
	{
		for (long i = 0; i < (long)cnodev->msh_node_numbers_averaging.size();
		     i++)
			if (cnodev->msh_node_numbers_averaging[i] ==
			    cnodev->msh_node_number)
				h_this_epsilon =
				    h_this_averaged +
				    epsilon * cnodev->msh_node_weights_averaging[i];
	}
	else
		h_this_epsilon = h_this + epsilon;
	/////  factor
	if (h_this_shifted + epsilon > h_cond)
		relPerm_epsilon = GetRelativeCouplingPermeability(
		    m_pcs_this, h_this + epsilon, m_st, cnodev->msh_node_number);
	else
		relPerm_epsilon = GetRelativeCouplingPermeability(
		    m_pcs_cond, h_cond, m_st,
		    cnodev->msh_node_number_conditional);  // richards or groundwater
                              ...
                              CNodeValue* cnodev)
{
	...
	gamma = mfp_vector[0]->Density() * GRAVITY_CONSTANT;
	long msh_node_2nd;
	double const* const xyz_this(
	    m_pcs_this->m_msh->nod_vector[cnodev->msh_node_number]->getData());
	//   double y_this =
	//   m_pcs_this->m_msh->nod_vector[cnodev->msh_node_number]->Y();
	//   double z_this =
	//   m_pcs_this->m_msh->nod_vector[cnodev->msh_node_number]->Z();
	...
	if (m_st->getProcessType() == FiniteElement::OVERLAND_FLOW)
	{
		...
		double const* const xyz_cond(
		    m_pcs_cond->m_msh->nod_vector[cnodev->msh_node_number_conditional]
		        ->getData());
		...
		pressure0 =
		    m_pcs_cond->GetNodeValue(cnodev->msh_node_number_conditional, nidx);
		...
		sat =
		    mmp_vector[group]->SaturationCapillaryPressureFunction(-pressure1);
		cond1 *= mmp_vector[group]->PermeabilitySaturationFunction(sat, phase);
		// use of relative permeability for second node (absolute perm. for top
		// node !!!!)
	...
	{  // Richards
		...
		long bc_eqs_index =
		    m_pcs_this->m_msh->nod_vector[cnodev->msh_node_number]
		        ->GetEquationIndex();
		double z_cond =
		    m_pcs_cond->m_msh->nod_vector[cnodev->msh_node_number_conditional]
		        ->getData()[2];
		depth = std::max(0., m_pcs_cond->GetNodeValue(
		                         ...
		                         m_pcs_cond->GetNodeValueIndex("HEAD") + 1) -
		                         z_cond);
		...
		msh_ele = m_pcs_this->m_msh->nod_vector[cnodev->msh_node_number]
		              ->getConnectedElementIDs()[0];
		...
		// -pressure0);
		// cond0 *=
		// mmp_vector[group]->PermeabilitySaturationFunction(sat,phase);
		...
		if (aktuelle_zeit < m_st->rainfall_duration)
		   ...
		   rainfall = 0;
		*/
		inf_cap = (depth + deltaZ - pressure1 / gamma) * (cond0 + cond1) /
		          (2 * deltaZ);
		supplyRate = m_st->rainfall;  //+ (depth ) / dt; // dt = timeStep

		m_pcs_this->SetNodeValue(
		    cnodev->msh_node_number,
		    // update coupling variable for error estimation
		    m_pcs_this->GetNodeValueIndex("COUPLING") + 1, inf_cap);

		if (inf_cap > supplyRate)
			bc_value = pressure1 - deltaZ * gamma +
			           gamma * supplyRate * deltaZ * 2 / (cond0 + cond1);
		else
			bc_value = pressure1 - deltaZ * gamma +
			           gamma * inf_cap * deltaZ * 2 / (cond0 + cond1);
		// bc_value = supplyRate * gamma * dt;

		MXRandbed(bc_eqs_index, bc_value,
		          m_pcs_this->getEQSPointer()->b);  // getEQSPointer. WW
		value = 0;
 ...
 **************************************************************************/
// double CSourceTermGroup::GetRiverNODValue(int i,CSourceTerm* m_st, long
// msh_node) //WW
// void GetRiverNODValue(double &value, CNodeValue* cnodev, const CSourceTerm*
// m_st) //WW
//{
//	double h;
...
/**************************************************************************
 ...
 02/2006 WW Change argument
 **************************************************************************/
// double CSourceTermGroup::GetCriticalDepthNODValue(CNodeValue*
// cnodev,CSourceTerm* m_st, long msh_node)
void GetCriticalDepthNODValue(double& value, CSourceTerm* m_st, long msh_node)
{
	...
	flowdepth = m_pcs_this->GetNodeValue(msh_node, nidx1) -
	            m_pcs_this->m_msh->nod_vector[msh_node]->getData()[2] -
	            m_st->rill_height;
	...
	if (flowdepth < 0.0)
	{
		value = 0;
		m_pcs_this->SetNodeValue(
		    msh_node, m_pcs_this->GetNodeValueIndex("FLUX") + 0, -value);
	}
	else
	{
		...
		{
			msh_ele = m_pcs_this->m_msh->nod_vector[msh_node]
			              ->getConnectedElementIDs()[0];
		...
		value_jacobi =
		    sqrt(GRAVITY_CONSTANT * flowdepth3_epsilon) * width + value;
...
void GetNormalDepthNODValue(double& value, CSourceTerm* st, long msh_node)
{
	...
	if (st->getNormalDepthSlope() ==
	    -1)  // TF: WARNING comparison of double via ==
	{
		if (mesh->GetMaxElementDim() > 1)
			std::cout << "!!!!! give slope for NORMAL DEPTH in st-file !!!!!"
			          << "\n";

		double elementlength = sqrt(MathLib::sqrDist(
		    m_ele->GetNode(1)->getData(), m_ele->GetNode(0)->getData()));
		//    		  (MathLib::fastpow(m_ele->GetNode(1)->X()-
		//    m_ele->GetNode(0)->X(), 2)
		//    		  +
		//    MathLib::fastpow(m_ele->GetNode(1)->Y()-m_ele->GetNode(0)->Y(), 2)
		//    		  + MathLib::fastpow(m_ele->GetNode(1)->Z() -
		//    m_ele->GetNode(0)->Z(), 2));
		S_0 = (m_ele->GetNode(1)->getData()[2] -
		       m_ele->GetNode(0)->getData()[2]) /
		      elementlength;
		if (S_0 < 0) S_0 = -S_0;
	}
	else
	...
	double flowdepth = pcs_this->GetNodeValue(msh_node, 1) -
	                   mesh->nod_vector[msh_node]->getData()[2] -
	                   st->rill_height;
	...
	if (mmp_vector[group]->channel == 1)
	{
		value = -pow(flowdepth * width / (2 * flowdepth + width), depth_exp) *
		        flowdepth * temp;
		value_for_jacobi =
		    pow(flowdepth_epsilon * width / (2 * flowdepth_epsilon + width),
		        depth_exp) *
	...
	pcs_this->SetNodeValue(msh_node, pcs_this->GetNodeValueIndex("FLUX") + 0,
	                       -value);
}
#endif
...
// 05/2010 TF restructured a little bit
//**************************************************************************/
// CSourceTerm* STGet(const std::string& pcs_name, const std::string&
// geo_type_name, const std::string& geo_name)
//{
...
void GetCouplingFieldVariables(double* h_this, double* h_cond,
                               double* h_shifted, double* h_averaged,
                               double* z_this, double* z_cond,
                               CSourceTerm* m_st, CNodeValue* cnodev)
{
	...
	gamma = mfp_vector[0]->Density() * GRAVITY_CONSTANT;
	*z_this =
	    m_pcs_this->m_msh->nod_vector[cnodev->msh_node_number]->getData()[2];
	*z_cond = m_pcs_cond->m_msh->nod_vector[cnodev->msh_node_number_conditional]
	              ->getData()[2];
	nidx = m_pcs_this->GetNodeValueIndex(convertPrimaryVariableToString(
	           m_st->getProcessPrimaryVariable())) +
	       1;
