...
public:
	...
	int* rateflag;  /* flag used for determining if reaction are calculated with
	                   PHREEQC */
	int countsteps; /* number of timesteps, after which reactions are
	                   recalculated in the whole model domain */
	int number_of_comp; /* Number of components in above data structures: size =
	                       number_of_comp * NodeListLength */
	...
	bool check_no_reaction_nodes; /* flag if CheckNoReactionNodes has been
	                                 performed */
	double temperature; /* temperature of water at node as specified in input
	                       file */
	long elenumber;     // number of elements
	/* hier später arrays of reactions reinhängen ?*/

	// rcml moved here
	int rcml_number_of_master_species; /* number of master species (inorgan.
	                                      equilibrium) */
	...
	int rcml_number_of_pqcsteps; /* Anzahl der Reaktionsschritte in PHREEQC aus
	                                dem Befehl: -steps "time" in "pqcsteps"
	                                steps */
	int rcml_pH_charge; /* =0, no charge balance for pH; =1, used for charge
	                       balance (keyword charge in line with pH*/
