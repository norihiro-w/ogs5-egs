...
// CMCD 9/2004 GeoSys 4
	...
	double PorosityDrainedStrain(long index,
	                             double val0,
	                             CFiniteElementStd* assem);  // AJ 12.2014
	double PorosityDrainedStrainTemp(long index,
	                                 double val0,
	                                 int thermo_mode,
	                                 double T0,
	                                 double alpha_b,
	...
	double HeatCapacity(long number,
	                    ...
	                    CFiniteElementStd* assem = NULL,
	                    double* var = NULL);
	double dHeatCapacitydP(long number,
	                       double theta,
	                       CFiniteElementStd* assem,
	                       double* var);
	double dHeatCapacitydT(long number,
	                       double theta,
	                       CFiniteElementStd* assem,
	                       double* var);
	double* HeatConductivityTensor(int number, double* variables = NULL);  // MX
	double* HeatDispersionTensorNew(int ip, double* var = NULL);  // CMCD
...
public:
	...
	int evaporation;  // if it is 647 then evaporation ON, else OFF: and
	                  // corresponding heat loss will compensated by heat ST
	double heatflux;
