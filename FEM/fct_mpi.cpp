...
std::ios::pos_type ReadRank(std::ifstream* is, int myrank)
{
	...
	for (size_t i = 0; i < ct.vec_neighbors.size(); i++)
	{
		...
		nei.overlapping_inner_nodes.assign(overlapping_innder_nodes.begin(),
		                                   overlapping_innder_nodes.end());
		nei.overlapping_ghost_nodes.assign(overlapping_ghost_nodes.begin(),
		                                   overlapping_ghost_nodes.end());
	}
			 ...
			 Math_Group::Vector& globalR_min)
{
	...
	// send R^+,R^- and get R^+,R^- to/from neighbors (i: inner nodes, j: ghost
	// nodes)
...
void gatherK(const CommunicationTable& ct, Math_Group::SparseMatrixDOK& globalK)
{
	...
	for (int i = 0; i < n_neighbors; i++)
	{
		...
		for (size_t j = 0; j < neighbor.overlapping_edges.size(); j++)
		{
			...
			//			std::cout << "rank= " << myrank << ": neighbor rank " <<
			// neighbor.rank << "- edge (" << edge.first << "," << edge.second
			// <<
			//")\n";
		}
	}
	...
	for (int i = 0; i < n_neighbors; i++)
	{
		...
		// vec_sendbuf[i].size() << " data to rank " << neighbor_rank << "\n";
	}
	...
	for (int i = 0; i < n_neighbors; i++)
	{
		...
		// vec_recvbuf[i].size() << " data from rank " << neighbor_rank << "\n";
	}
	...
	//		int done = 0;
	//		if (myrank>0) {
	//			MPI_Recv(&done, 1, MPI_INT, myrank-1, 0, MPI_COMM_WORLD,
	//&status);
	//		}
	//
              ...
              Math_Group::SparseMatrixDOK& d)
{
	...
	for (size_t i = 0; i < node_size; i++)
	{
		...
		for (jj = col->begin(); jj != col->end(); ++jj)
		{
			...
			// double d0 = d1; //TODO should use AuxMatrix at the previous time
			// step
                 ...
                 Math_Group::SparseMatrixDOK::mat_t& fct_f)
{
	...
	//            std::cout << "rank= " << myrank << ": node i=" << i_global <<
	//            ", ML=" << (*ML)(i_global) << "\n";
	//        }
	for (size_t i = 0; i < node_size; i++)
	{
