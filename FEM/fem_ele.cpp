      ...
      extrapo_method(ExtrapolationMethod::EXTRAPO_LINEAR)
{
	...
	if (M_Process) D_Flag = 4;
	if (MH_Process) D_Flag = 41;
	F_Flag = H_Process;
	T_Flag = T_Process;
	...
	switch (ele_dim)
	{
		...
		case 3:
		{
			...
			invJacobian[0] =
			    Jacobian[4] * Jacobian[8] - Jacobian[7] * Jacobian[5];
			invJacobian[1] =
			    Jacobian[2] * Jacobian[7] - Jacobian[1] * Jacobian[8];
			invJacobian[2] =
			    Jacobian[1] * Jacobian[5] - Jacobian[2] * Jacobian[4];
			//
			invJacobian[3] =
			    Jacobian[5] * Jacobian[6] - Jacobian[8] * Jacobian[3];
			invJacobian[4] =
			    Jacobian[0] * Jacobian[8] - Jacobian[6] * Jacobian[2];
			invJacobian[5] =
			    Jacobian[2] * Jacobian[3] - Jacobian[5] * Jacobian[0];
			//
			invJacobian[6] =
			    Jacobian[3] * Jacobian[7] - Jacobian[6] * Jacobian[4];
			invJacobian[7] =
			    Jacobian[1] * Jacobian[6] - Jacobian[7] * Jacobian[0];
			invJacobian[8] =
			    Jacobian[0] * Jacobian[4] - Jacobian[3] * Jacobian[1];
			for (size_t i = 0; i < ele_dim * ele_dim; i++)
				invJacobian[i] /= DetJac;
			break;
		}  // end case 3
...
void CElement::FaceIntegration(double* NodeVal)
{
	...
	for (gp = 0; gp < nGaussPoints; gp++)
	{
		...
		switch (MeshElement->GetElementType())
		{
			...
			default:
				std::cerr
				    << "CElement::FaceIntegration element type not handled"
				    << "\n";
				break;
		}
