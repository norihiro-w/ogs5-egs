...
double CFluidProperties::phi_r_d(double rho, double T) const
{
	...
	for (i = 0; i < limit[3]; i++)
	{
		...
		{
			if (i < limit[1])
				phi_b = phi_b + (K[0][i] * exp(-pow(delta, K[3][i])) *
				                 (pow(delta, K[1][i] - 1) * pow(tau, K[2][i]) *
				                  (K[1][i] - K[3][i] * pow(delta, K[3][i]))));
			else if (i < limit[2])
				phi_c =
				    phi_c +
				    (K[0][i] * pow(delta, K[1][i]) * pow(tau, K[2][i]) *
			...
			else if (i < limit[3])
			{
				...
				dDELTA_deriv = dDELTA_ddelta(delta, K[6][i], THETA, K[11][i],
				                             K[7][i], K[4][i]);
				dDELTApowbddelta =
				    dDELTApowb_ddelta(K[5][i], DELTA, dDELTA_deriv);
				DPHI = dphi_ddelta(K[8][i], delta, PHI);

				phi_d = phi_d +
				        K[0][i] * (pow(DELTA, K[5][i]) * (PHI + delta * DPHI) +
...
double CFluidProperties::phi_r_tt(double rho, double T) const
{
	...
	{
		if (i < limit[0])
			phi_a = phi_a + (K[0][i] * K[2][i] * (K[2][i] - 1) *
			                 pow(delta, K[1][i]) * pow(tau, (K[2][i] - 2)));
		else if (i < limit[1])
			phi_b = phi_b +
			        (K[0][i] * K[2][i] * (K[2][i] - 1) * pow(delta, K[1][i]) *
			...
			phi_c =
			    ...
			    (K[0][i] * pow(delta, K[1][i]) * pow(tau, K[2][i]) *
			     exp(-K[10][i] * ((delta - K[13][i]) * (delta - K[13][i])) -
			         K[11][i] * ((tau - K[12][i]) * (tau - K[12][i]))) *
			     (pow((K[2][i] / tau - 2 * K[11][i] * (tau - K[12][i])), 2) -
			      K[2][i] / (tau * tau) - 2 * K[11][i]));
		else if (i < limit[3])
		{
			THETA = theta_fn(tau, K[6][i], delta, K[11][i]);
			DELTA = delta_fn(THETA, K[7][i], delta, K[4][i]);
			...
			phi_d =
			    phi_d + (K[0][i] * delta * (D2DELTA * PHI + 2 * DDELTA * DPHI +
			                                pow(DELTA, K[5][i]) * D2PHI));
		}
	}
	return phi_a + phi_b + phi_c + phi_d;
}
...
double CFluidProperties::phi_0_t(double T) const
{
	...
	for (i = 3; i < 8; i++)
		phi_e =
		    phi_e + (k[0][i] * k[1][i] * (1 / (1 - exp(-k[1][i] * tau)) - 1));
...
double CFluidProperties::phi_0_tt(double T) const
{
	...
	else
	{
		...
		for (i = 3; i < 8; i++)
			phi_e =
			    phi_e + (k[0][i] * (k[1][i] * k[1][i]) * exp(-k[1][i] * tau) *
			             pow(1 - exp(-k[1][i] * tau), -2));
...
double CFluidProperties::phi_r_t(double rho, double T) const
{
	...
	for (i = 0; i < limit[0]; i++)
		phi_a = phi_a + (K[0][i] * K[2][i] * pow(delta, K[1][i]) *
		                 pow(tau, (K[2][i] - 1)));

	for (i = limit[0]; i < limit[1]; i++)
		phi_b = phi_b + (K[0][i] * K[2][i] * pow(delta, K[1][i]) *
		                 pow(tau, (K[2][i] - 1)) * exp(-pow(delta, K[3][i])));

	for (i = limit[1]; i < limit[2]; i++)
		phi_c =
		    phi_c + (K[0][i] * pow(delta, K[1][i]) * pow(tau, (K[2][i])) *
		             exp(-K[10][i] * ((delta - K[13][i]) * (delta - K[13][i])) -
		                 K[11][i] * (tau - K[12][i]) * (tau - K[12][i])) *
	...
	for (i = limit[2]; i < limit[3]; i++)
	{
		...
		phi_d = phi_d + (K[0][i] * delta *
		                 (ddeltatau * phifn + pow(deltafn, K[5][i]) * dphitau));
	}
...
double CFluidProperties::phi_r_dt(double rho, double T) const
{
	...
	for (i = 0; i < limit[0]; i++)
		phi_a = phi_a + (K[0][i] * K[1][i] * K[2][i] *
		                 pow(delta, (K[1][i] - 1)) * pow(tau, (K[2][i] - 1)));

	for (i = limit[0]; i < limit[1]; i++)
		phi_b = phi_b + (K[0][i] * K[2][i] * pow(delta, (K[1][i] - 1)) *
		                 pow(tau, (K[2][i] - 1)) *
		                 (K[1][i] - K[3][i] * pow(delta, K[3][i])) *
		                 exp(-pow(delta, K[3][i])));
	...
	for (i = limit[2]; i < limit[3]; i++)
	{
		...
		dDELTAderiv =
		    dDELTA_ddelta(delta, K[6][i], thetafn, K[11][i], K[7][i], K[4][i]);
		...
		d2DELTApowbddeltadtau = d2DELTApowb_ddeltadtau(
		    K[6][i], K[5][i], K[11][i], deltafn, delta, thetafn, dDELTAderiv);

		phi_d =
		    phi_d + (K[0][i] *
		             (pow(deltafn, K[5][i]) * (dphidtau + delta * d2phideriv) +
...
double CFluidProperties::phi_r_dd(double rho, double T) const
{
	...
	for (i = 0; i < limit[0]; i++)
		phi_a = phi_a + (K[0][i] * K[1][i] * (K[1][i] - 1) *
		                 pow(delta, (K[1][i] - 2)) * pow(tau, K[2][i]));
	for (i = limit[0]; i < limit[1]; i++)
		phi_b = phi_b + ((K[0][i] * exp(-(pow(delta, K[3][i])))) *
		                 (((pow(delta, (K[1][i] - 2)) * pow(tau, K[2][i]))) *
		                  (((K[1][i] - K[3][i] * pow(delta, K[3][i]))) *
		                       (K[1][i] - 1 - K[3][i] * pow(delta, K[3][i])) -
		                   (pow(K[3][i], 2) * pow(delta, K[3][i])))));

	for (i = limit[1]; i < limit[2]; i++)
		phi_c =
		    phi_c + ((K[0][i] * pow(tau, K[2][i])) *
		             exp(-K[10][i] * pow((delta - K[13][i]), 2) -
		                 K[11][i] * ((tau - K[12][i]) * (tau - K[12][i]))) *
		             ((-2 * K[10][i] * pow(delta, K[1][i]) +
		               4 * (K[10][i] * K[10][i]) * pow(delta, K[1][i]) *
		                   ((delta - K[13][i]) * (delta - K[13][i]))) +
		              (-4 * K[1][i] * K[10][i] * pow(delta, (K[1][i] - 1)) *
		                   (delta - K[13][i]) +
	...
	for (i = limit[2]; i < limit[3]; i++)
	{
		...
		dphiddelta = dphi_ddelta(K[8][i], delta, phifn);
		d2DELTA_deriv = d2DELTA_ddelta2(delta, dDELTA_deriv, K[6][i], thetafn,
		                                K[11][i], K[7][i], K[4][i]);
		d2DELTApowbddelta2 =
		    d2DELTApowb_ddelta2(K[5][i], deltafn, d2DELTA_deriv, dDELTA_deriv);

		phi_d =
		    phi_d +
		    (K[0][i] *
		     (pow(deltafn, K[5][i]) * (2 * dphiddelta + delta * d2phiddelta2) +
