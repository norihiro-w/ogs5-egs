...
double dDELTA_ddelta(double delta, double A, double theta_fn, double beta,
                     double B, double a)
{
	return (delta - 1) *
	       (A * theta_fn * 2 / beta *
	...
	//   return
	//   ((delta-1)*(A*theta_fn*2/beta*pow(pow((delta-1),2),(1/(2*beta)-1))+2*B*a*pow(pow((delta-1),2),(a-1))));
}
                       ...
                       double a)
{
	...
	return 1 / (delta - 1) * dDELTA_deriv +
	       delta_m1_quad * (4 * B * a * (a - 1) * pow(delta_m1_quad, (a - 2)) +
	                        2 * A * A * pow((1 / beta), 2) * t0 * t0 +
	                        A * theta_fn * 4 / beta * (1 / (2 * beta) - 1) *
...
double d2DELTApowb_ddelta2(double b, double delta_fn, double d2DELTA_deriv,
                           double dDELTA_deriv)
{
	return b * (pow(delta_fn, (b - 1)) * d2DELTA_deriv +
	            (b - 1) * pow(delta_fn, (b - 2)) * dDELTA_deriv * dDELTA_deriv);
	//   return
	//   b*(pow(delta_fn,(b-1))*d2DELTA_deriv+(b-1)*pow(delta_fn,(b-2))*pow(dDELTA_deriv,2));
}
...
{
	return 2 * b * pow(delta_fn, (b - 1)) +
	       4 * theta_fn * theta_fn * b * (b - 1) * pow(delta_fn, (b - 2));
	//   return
	//   2*b*pow(delta_fn,(b-1))+4*pow(theta_fn,2)*b*(b-1)*pow(delta_fn,(b-2));
}
                              ...
                              double dDELTA_deriv)
{
	...
	//   return
	//   -A*b*2/beta*pow(delta_fn,(b-1))*(delta-1)*pow(pow((delta-1),2),(1/(2*beta)-1))
	//      -2*theta_fn*b*(b-1)*pow(delta_fn,(b-2))*dDELTA_deriv;
}

double d2phi_ddeltadtau(double C, double D, double delta, double tau,
                        double phi_fn)
{
...
double CFluidProperties::phi_r_d(double rho, double T) const
{
	...
	{
		if (i < limit[0])
			phi_a = phi_a + (K_[0][i] * K_[1][i] * pow(delta, (K_[1][i] - 1)) *
			                 pow(tau, K_[2][i]));
		else
		{
			if (i < limit[1])
				phi_b = phi_b + (K_[0][i] * exp(-pow(delta, K_[3][i])) *
				                 (pow(delta, K_[1][i] - 1) * pow(tau, K_[2][i]) *
				                  (K_[1][i] - K_[3][i] * pow(delta, K_[3][i]))));
			else if (i < limit[2])
				phi_c =
				    phi_c +
				    (K_[0][i] * pow(delta, K_[1][i]) * pow(tau, K_[2][i]) *
				     exp(-K_[10][i] * (delta - K_[13][i]) * (delta - K_[13][i]) -
				         K_[11][i] * (tau - K_[12][i]) * (tau - K_[12][i])) *
				     (K_[1][i] / delta - 2 * K_[10][i] * (delta - K_[13][i])));
			else if (i < limit[3])
			{
				THETA = theta_fn(tau, K_[6][i], delta, K_[11][i]);
				DELTA = delta_fn(THETA, K_[7][i], delta, K_[4][i]);
				PHI = phi_fn(K_[8][i], delta, K_[9][i], tau);
				dDELTA_deriv = dDELTA_ddelta(delta, K_[6][i], THETA, K_[11][i],
				                             K_[7][i], K_[4][i]);
				dDELTApowbddelta =
				    dDELTApowb_ddelta(K_[5][i], DELTA, dDELTA_deriv);
				DPHI = dphi_ddelta(K_[8][i], delta, PHI);

				phi_d = phi_d +
				        K_[0][i] * (pow(DELTA, K_[5][i]) * (PHI + delta * DPHI) +
...
double CFluidProperties::phi_r_tt(double rho, double T) const
{
	...
	{
		if (i < limit[0])
			phi_a = phi_a + (K_[0][i] * K_[2][i] * (K_[2][i] - 1) *
			                 pow(delta, K_[1][i]) * pow(tau, (K_[2][i] - 2)));
		else if (i < limit[1])
			phi_b = phi_b +
			        (K_[0][i] * K_[2][i] * (K_[2][i] - 1) * pow(delta, K_[1][i]) *
			         pow(tau, (K_[2][i] - 2)) * exp(-pow(delta, K_[3][i])));
		else if (i < limit[2])
			phi_c =
			    phi_c +
			    (K_[0][i] * pow(delta, K_[1][i]) * pow(tau, K_[2][i]) *
			     exp(-K_[10][i] * ((delta - K_[13][i]) * (delta - K_[13][i])) -
			         K_[11][i] * ((tau - K_[12][i]) * (tau - K_[12][i]))) *
			     (pow((K_[2][i] / tau - 2 * K_[11][i] * (tau - K_[12][i])), 2) -
			      K_[2][i] / (tau * tau) - 2 * K_[11][i]));
		else if (i < limit[3])
		{
			THETA = theta_fn(tau, K_[6][i], delta, K_[11][i]);
			DELTA = delta_fn(THETA, K_[7][i], delta, K_[4][i]);
			...
			D2DELTA = d2DELTA_dtau2(K_[5][i], DELTA, THETA);
			DDELTA = dDELTA_dtau(THETA, K_[5][i], DELTA);
			DPHI = dphi_dtau(K_[9][i], tau, PHI);
			D2PHI = d2phi_dtau2(K_[9][i], tau, PHI);

			phi_d =
			    phi_d + (K_[0][i] * delta * (D2DELTA * PHI + 2 * DDELTA * DPHI +
			                                pow(DELTA, K_[5][i]) * D2PHI));
		}
	}
	return phi_a + phi_b + phi_c + phi_d;
}
...
double CFluidProperties::phi_0_t(double T) const
{
	...
	for (i = 3; i < 8; i++)
		phi_e =
		    phi_e + (k_[0][i] * k_[1][i] * (1 / (1 - exp(-k_[1][i] * tau)) - 1));
...
double CFluidProperties::phi_0_tt(double T) const
{
	if (fluid_id == 3)  // N2
	{
		...
		const double phi_zero_tt =
		    (-a[0] + 2.0 * a[1] / tau + 6.0 * a[2] / tau / tau +
		     12 * a[3] / tau / tau / tau -
		     a[4] * a[5] * a[5] * tau * tau * exp_a8 /
		         ((exp_a8 - 1.0) * (exp_a8 - 1.0))) /
		    (tau * tau);
		return phi_zero_tt;
	}
	else
	{
		...
		tau = Tc / T;
		phi_d = k_[0][2] / (tau * tau);
		for (i = 3; i < 8; i++)
			phi_e =
			    phi_e + (k_[0][i] * (k_[1][i] * k_[1][i]) * exp(-k_[1][i] * tau) *
			             pow(1 - exp(-k_[1][i] * tau), -2));
...
double CFluidProperties::phi_r_t(double rho, double T) const
{
	...
	for (i = 0; i < limit[0]; i++)
		phi_a = phi_a + (K_[0][i] * K_[2][i] * pow(delta, K_[1][i]) *
		                 pow(tau, (K_[2][i] - 1)));

	for (i = limit[0]; i < limit[1]; i++)
		phi_b = phi_b + (K_[0][i] * K_[2][i] * pow(delta, K_[1][i]) *
		                 pow(tau, (K_[2][i] - 1)) * exp(-pow(delta, K_[3][i])));

	for (i = limit[1]; i < limit[2]; i++)
		phi_c =
		    phi_c + (K_[0][i] * pow(delta, K_[1][i]) * pow(tau, (K_[2][i])) *
		             exp(-K_[10][i] * ((delta - K_[13][i]) * (delta - K_[13][i])) -
		                 K_[11][i] * (tau - K_[12][i]) * (tau - K_[12][i])) *
		             (K_[2][i] / tau - 2 * K_[11][i] * (tau - K_[12][i])));

	for (i = limit[2]; i < limit[3]; i++)
	{
		thetafn = theta_fn(tau, K_[6][i], delta, K_[11][i]);
		deltafn = delta_fn(thetafn, K_[7][i], delta, K_[4][i]);
		ddeltatau = dDELTA_dtau(thetafn, K_[5][i], deltafn);
		phifn = phi_fn(K_[8][i], delta, K_[9][i], tau);
		dphitau = dphi_dtau(K_[9][i], tau, phifn);

		phi_d = phi_d + (K_[0][i] * delta *
		                 (ddeltatau * phifn + pow(deltafn, K_[5][i]) * dphitau));
	}
...
double CFluidProperties::phi_r_dt(double rho, double T) const
{
	...
	int i;
	double delta, tau;
	double phifn, thetafn, deltafn, d2phideriv, dDELTAderiv, dDELTApowbddelta,
	    dphidtau, dDELTApowbdtau, dphiddelta, d2DELTApowbddeltadtau;

	tau = Tc / T;
	delta = rho / rhoc;

	for (i = 0; i < limit[0]; i++)
		phi_a = phi_a + (K_[0][i] * K_[1][i] * K_[2][i] *
		                 pow(delta, (K_[1][i] - 1)) * pow(tau, (K_[2][i] - 1)));

	for (i = limit[0]; i < limit[1]; i++)
		phi_b = phi_b + (K_[0][i] * K_[2][i] * pow(delta, (K_[1][i] - 1)) *
		                 pow(tau, (K_[2][i] - 1)) *
		                 (K_[1][i] - K_[3][i] * pow(delta, K_[3][i])) *
		                 exp(-pow(delta, K_[3][i])));
	...
	for (i = limit[2]; i < limit[3]; i++)
	{
		...
		thetafn = theta_fn(tau, K_[6][i], delta, K_[11][i]);
		deltafn = delta_fn(thetafn, K_[7][i], delta, K_[4][i]);
		d2phideriv = d2phi_ddeltadtau(K_[8][i], K_[9][i], delta, tau, phifn);

		dDELTAderiv =
		    dDELTA_ddelta(delta, K_[6][i], thetafn, K_[11][i], K_[7][i], K_[4][i]);

		dDELTApowbddelta = dDELTApowb_ddelta(K_[5][i], deltafn, dDELTAderiv);
		dphidtau = dphi_dtau(K_[9][i], tau, phifn);
		dDELTApowbdtau = dDELTA_dtau(thetafn, K_[5][i], deltafn);
		dphiddelta = dphi_ddelta(K_[8][i], delta, phifn);
		d2DELTApowbddeltadtau = d2DELTApowb_ddeltadtau(
		    K_[6][i], K_[5][i], K_[11][i], deltafn, delta, thetafn, dDELTAderiv);

		phi_d =
		    phi_d + (K_[0][i] *
		             (pow(deltafn, K_[5][i]) * (dphidtau + delta * d2phideriv) +
...
double CFluidProperties::phi_r_dd(double rho, double T) const
{
	...
	int i;
	double delta, tau;
	double thetafn, deltafn, phifn, dphiddelta, d2phiddelta2, dDELTA_deriv,
	    dDELTApowbddelta, d2DELTA_deriv, d2DELTApowbddelta2;

	tau = Tc / T;
	delta = rho / rhoc;

	for (i = 0; i < limit[0]; i++)
		phi_a = phi_a + (K_[0][i] * K_[1][i] * (K_[1][i] - 1) *
		                 pow(delta, (K_[1][i] - 2)) * pow(tau, K_[2][i]));
	for (i = limit[0]; i < limit[1]; i++)
		phi_b = phi_b + ((K_[0][i] * exp(-(pow(delta, K_[3][i])))) *
		                 (((pow(delta, (K_[1][i] - 2)) * pow(tau, K_[2][i]))) *
		                  (((K_[1][i] - K_[3][i] * pow(delta, K_[3][i]))) *
		                       (K_[1][i] - 1 - K_[3][i] * pow(delta, K_[3][i])) -
		                   (pow(K_[3][i], 2) * pow(delta, K_[3][i])))));

	for (i = limit[1]; i < limit[2]; i++)
		phi_c =
		    phi_c + ((K_[0][i] * pow(tau, K_[2][i])) *
		             exp(-K_[10][i] * pow((delta - K_[13][i]), 2) -
		                 K_[11][i] * ((tau - K_[12][i]) * (tau - K_[12][i]))) *
		             ((-2 * K_[10][i] * pow(delta, K_[1][i]) +
		               4 * (K_[10][i] * K_[10][i]) * pow(delta, K_[1][i]) *
		                   ((delta - K_[13][i]) * (delta - K_[13][i]))) +
		              (-4 * K_[1][i] * K_[10][i] * pow(delta, (K_[1][i] - 1)) *
		                   (delta - K_[13][i]) +
		               K_[1][i] * (K_[1][i] - 1) * pow(delta, (K_[1][i] - 2)))));
	...
	for (i = limit[2]; i < limit[3]; i++)
	{
		...
		phifn = phi_fn(K_[8][i], delta, K_[9][i], tau);
		dphiddelta = dphi_ddelta(K_[8][i], delta, phifn);
		d2phiddelta2 = d2phi_ddelta2(K_[8][i], delta, phifn);
		dDELTA_deriv =
		    dDELTA_ddelta(delta, K_[6][i], thetafn, K_[11][i], K_[7][i], K_[4][i]);
		dDELTApowbddelta = dDELTApowb_ddelta(K_[5][i], deltafn, dDELTA_deriv);
		dphiddelta = dphi_ddelta(K_[8][i], delta, phifn);
		d2DELTA_deriv = d2DELTA_ddelta2(delta, dDELTA_deriv, K_[6][i], thetafn,
		                                K_[11][i], K_[7][i], K_[4][i]);
		d2DELTApowbddelta2 =
		    d2DELTApowb_ddelta2(K_[5][i], deltafn, d2DELTA_deriv, dDELTA_deriv);

		phi_d =
		    phi_d +
		    (K_[0][i] *
		     (pow(deltafn, K_[5][i]) * (2 * dphiddelta + delta * d2phiddelta2) +
...
double enthalpy(double rho, double T, int c)
{
	...
	double h =
	    (1 + tau * (fluid_prop->phi_0_t(T) + fluid_prop->phi_r_t(rho, T)) +
	     delta * fluid_prop->phi_r_d(rho, T)) *
	    R * T;
	return h;
}
...
/**********************************************************************
   ...
   linearised in a specified temperature interval for typical technological
 applications
 ***********************************************************************/
...
double linear_heat_capacity(double T, int c)
{
	...
	switch (c)
	{
		case 1:  // H2O
			...
			break;
		case 3:  // N2
			     // 1056.8+(1146.4-1056.8)/(900.0-500.0)*(variables[1]-500.0);
			Tl[0] = 275.0;
			Tl[1] = 1200.0;
			cpl[0] = 1039.0;
			cpl[1] = 1204.0;
		...
		case 5:  // O2
			...
			break;
		default:
			std::cout << "WARNING: Fluid not specified in "
			             "linear_heat_capacity. Setting cp to 1000.\n";
			return 1000.0;
	}
...
double isochoric_heat_capacity(double rho, double T, int c)
{
	...
	double cv =
	    -(h * h * (fluid_prop->phi_0_tt(T) + fluid_prop->phi_r_tt(rho, T))) *
...
double isobaric_heat_capacity(double rho, double T, int c)
{
	...
	double cp =
	    (-tau * tau * (fluid_prop->phi_0_tt(T) + fluid_prop->phi_r_tt(rho, T)) +
	     (pow((1 + delta * fluid_prop->phi_r_d(rho, T) -
	           delta * tau * fluid_prop->phi_r_dt(rho, T)),
	          2)) /
	         ((1 + 2 * delta * fluid_prop->phi_r_d(rho, T) +
	           delta * delta * fluid_prop->phi_r_dd(rho, T)))) *
	    fluid_prop->getSpecificGasConstant();
...
/**********************************************************************
   Function for calculating viscosity of CO2 depending on density and
 Temperature.
   Programming: NB
          Aug 2008
 ***********************************************************************/
...
/**********************************************************************
   Function for calculating heat conductivity of CO2 depending on density and
 Temperature.
   (Vesovic&Wakeham)
...
double co2_heat_conductivity(double rho, double T)
{
	...
	double a[13] = {0,       3,       6.70697, 0.94604, 0.3,  0.3, 0.39751,
	                0.33791, 0.77963, 0.79857, 0.9,     0.02, 0.2};
	double g[11] = {0, 0, 0, 1.5, 0, 1, 1.5, 1.5, 1.5, 3.5, 5.5};
	double h[11] = {0, 1, 5, 1, 1, 2, 0, 5, 9, 0, 0};
	double n[11] = {0, 7.69857587E+00, 1.59885811E-01, 1.56918621E+00,
	                -6.73400790E+00, 1.63890156E+01, 3.69415242E+00,
	                2.23205514E+01, 6.61420950E+01, -1.71779133E-01,
	                4.33043347E-03};
	...
	temp1 = rho_r * exp(-pow(rho_r, a[1]) / a[1] - pow(a[2] * (Tr - 1), 2) -
	                    pow(a[3] * (rho_r - 1), 2));
	temp2 = pow(
	    pow((1 - 1 / Tr) + a[4] * pow(pow(rho_r - 1, 2), 1. / (2. * a[5])), 2),
	    a[6]);
	...
	//    double
	//    Lamda_C=pow(R,5./6.)*pow(pc,2./3.)/pow(Tc,1./6.)/pow(M,0.5)/pow(NA,1./3.);
...
double ch4_viscosity_295K(double p)
{
	...
	p = p / 100000;
	h = (-3.7091411E-14 * MathLib::fastpow(p, 4) +
	     9.1937114E-10 * MathLib::fastpow(p, 3) - 6.6099446E-06 * p * p +
	     4.8400147E-02 * p + 1.0934694E+01) /
	    1.0e6;
...
double o2_viscosity(double rho, double T)
{
	...
	double rho_min(0.2);  // densities lower than that do not influence eta_r
	                      // (less than 1%)
	if (rho < rho_min)
		return eta_0 *
		       1e-6;  // for better efficiency near dilute gas (eta in Pa*s)
	...
	for (unsigned i = 0; i < 5; i++)
	{
		eta_r += N[i] * pow(tau, t[i]) * MathLib::fastpow(delta, d[i]) *
...
{
	// TODO: make a global function for all properties: Fluid_Property(int c,
	// int property, double T, double P) (NB)
...
double sublime_pressure_co2(double T, double Tt, double pt)
{
	...
	p = exp(Tt / T * (a[0] * (1 - T / Tt) + a[1] * pow((1 - T / Tt), 1.9) +
	                  a[2] * pow((1 - T / Tt), 2.9))) *
	    pt;
...
double preos(const CFluidProperties* mfp, double T, double P)
{
	...
	P = P / 1000;  // P in kPa
	alpha = ((1 +
	          (0.37464 + 1.5422 * omega - 0.26992 * omega * omega) *
	              (1 - sqrt(T / Tc)))) *
	        ((1 +
	          (0.37464 + 1.5422 * omega - 0.26992 * omega * omega) *
	              (1 - sqrt(T / Tc))));
	...
	z2 = (pow(MM, 2) * (b * P - Ru * T)) /
	     (b * (P * pow(b, 2) + b * Ru * T - a * alpha));
	z3 = (MathLib::fastpow(MM, 3) * P) /
	     (b * (P * b * b + b * Ru * T - a * alpha));
...
double h2o_heat_conductivity_IAPWS_ind(double rho, double T)
{
	...
	lamda_0 = sqrt(T) * sum1;
	lamda_1 =
	    b[0] + b[1] * rho + b[2] * exp(B[0] * (rho + B[1]) * (rho + B[1]));
	...
	lamda_2 =
	    (d[0] / MathLib::fastpow(T, 10) + d[1]) * pow(rho, 9. / 5.) *
	        exp(C[0] * (1 - pow(rho, 14. / 5.))) +
	    d[2] * S * pow(rho, Q) *
	        exp((Q / (1. + Q)) * (1 - pow(rho, (1. + Q)))) +
...
double ch4_heat_conductivity(double rho, double T)
{
	...
	phi_id_tt = -Q[0] + 4 * Q[1] / 9 * pow(tau, -1. / 3.) +
	            10 * Q[2] / 9 * pow(tau, -2. / 3.) + 2 * Q[3] * pow(tau, -1) -
	            Q[4] * Q[5] * Q[5] * tau * tau * exp(Q[5] * tau) *
	                pow((exp(Q[5] * tau) - 1), -2);
	...
	//(Eq. 3)
	P_sigma = P_c * exp(H[0] * T_star / (1 - T_star) + H[1] * T_star +
	                    H[2] * pow(T_star, epsilon) + H[3] * pow(T_star, 2) +
	                    H[4] * pow(T_star, 3));
	Z_c = P_c / (R * T_c * rho_c);
	...
	for (i = 0; i < 7; i++)  // (Eq. 17)
		...
		sum =
		    sum +
		    j[i] * MathLib::fastpow(delta, r[i]) * MathLib::fastpow(tau, s[i]);
...
double o2_heat_conductivity(double rho, double T)
{
	...
	const double lam_0 = N[0] * eta_0 + N[1] * pow(tau, t[0]) +
	                     N[2] * pow(tau, t[1]);  // note counting is different
	                                             // due to different array sizes
...
double n2_heat_conductivity(double rho, double T)
{
	...
	c_v0 = R * (sum + ((f[7] * (f[8] / T) * (f[8] / T) * (exp((f[8] / T)))) /
	                       (temp * temp) -
	                   1));
	sum = 0;
...
double vapour_pressure_IF97(double T)
{
	...
	double h;
	h = MathLib::fastpow((2.0 * C / (-B + sqrt(B * B - 4.0 * A * C))), 4) *
	    10.0;
	return h;
}
...
double vapour_pressure_ch4(double T)
{
	...
	theta = (1 - T / Tc);
	h = Tc / T * (n1 * theta + n2 * pow(theta, 1.5) + n3 * theta * theta +
	              n4 * pow(theta, 4.5));
...
double vapour_pressure_h2o(double T)
{
	...
	h = Tc / T *
	    (a1 * theta + a2 * pow(theta, 1.5) + a3 * MathLib::fastpow(theta, 3) +
	     a4 * pow(theta, 3.5) + a5 * MathLib::fastpow(theta, 4) +
...
/*************************************************************
   ...
   Last modification: NB Jun 2009
*************************************************************/
void auswahl(double T, double P, int fluid, double* x1, double* x2,
             double /*eps*/)
{
	switch (fluid)
	{
                      ...
                      double* rMM)
{
	...
	for (i = 0; i < 3; i++)
		for (j = 0; j < 3; j++)
			for (k = 0; k < 3; k++)
				a_mix +=
				    x[i] * x[j] * x[k] * pow(a[i] * a[j] * a[k], (1. / 3.));
...
double liquid_saturation_density_ch4(double T)
{
	...
	theta = (1 - T / Tc);
	h = (n1 * pow(theta, 0.354) + n2 * sqrt(theta) +
	     n3 * pow(theta, (5. / 2.)));
...
double vapour_saturation_density_ch4(double T)
{
	...
	theta = (1 - T / Tc);
	h = (n1 * pow(theta, 0.354) + n2 * pow(theta, (5. / 6.)) +
	     n3 * pow(theta, (3. / 2.)) + n4 * pow(theta, (5. / 2.)) +
	     n5 * pow(theta, (25. / 6.)) + n6 * pow(theta, (47. / 6.)));
...
double vapour_pressure_n2(double T)
{
	...
	theta = (1 - T / Tc);
	h = (Tc / T) * (n1 * theta + n2 * pow(theta, (1.5)) +
	                n3 * pow(theta, (2.5)) + n4 * MathLib::fastpow(theta, (5)));
...
double liquid_saturation_density_n2(double T)
{
	...
	theta = (1 - T / Tc);
	h = (n1 * pow(theta, 0.3294) + n2 * pow(theta, (2. / 3.)) +
	     n3 * pow(theta, (8. / 3.)) + n4 * pow(theta, (35. / 6.)));
...
double vapour_saturation_density_n2(double T)
{
	...
	theta = (1 - T / Tc);
	h = (Tc / T) * (n1 * pow(theta, 0.34) + n2 * pow(theta, (5. / 6.)) +
	                n3 * pow(theta, (7. / 6.)) + n4 * pow(theta, (13. / 6.)) +
	                n5 * pow(theta, (14. / 3.)));
...
/**********************************************************************
   Function thermal_properties (fluid, critical_density, critical_temperature,
...
void CFluidProperties::therm_prop(string caption)
{
	...
	char letter;
	int i, j;
	// TODO: Change first letter approach to an integer (NB) - done
	// (fluid_number) NB, 2009-06-02
	letter = caption[0];
	Ru = 8.314472;
	switch (letter)
	{
		case 'C':  // CARBON DIOXIDE
		{
			...
			// n
			K_[0][0] = 3.8856823203161E-01;
			K_[0][1] = 2.9385475942740E+00;
			K_[0][2] = -5.5867188534934E+00;
			K_[0][3] = -7.6753199592477E-01;
			K_[0][4] = 3.1729005580416E-01;
			K_[0][5] = 5.4803315897767E-01;
			K_[0][6] = 1.2279411220335E-01;
			K_[0][7] = 2.1658961543220E+00;
			K_[0][8] = 1.5841735109724E+00;
			K_[0][9] = -2.3132705405503E-01;
			K_[0][10] = 5.8116916431436E-02;
			K_[0][11] = -5.5369137205382E-01;
			K_[0][12] = 4.8946615909422E-01;
			K_[0][13] = -2.4275739843501E-02;
			K_[0][14] = 6.2494790501678E-02;
			K_[0][15] = -1.2175860225246E-01;
			K_[0][16] = -3.7055685270086E-01;
			K_[0][17] = -1.6775879700426E-02;
			K_[0][18] = -1.1960736637987E-01;
			K_[0][19] = -4.5619362508778E-02;
			K_[0][20] = 3.5612789270346E-02;
			K_[0][21] = -7.4427727132052E-03;
			K_[0][22] = -1.7395704902432E-03;
			K_[0][23] = -2.1810121289527E-02;
			K_[0][24] = 2.4332166559236E-02;
			K_[0][25] = -3.7440133423463E-02;
			K_[0][26] = 1.4338715756878E-01;
			K_[0][27] = -1.3491969083286E-01;
			K_[0][28] = -2.3151225053480E-02;
			K_[0][29] = 1.2363125492901E-02;
			K_[0][30] = 2.1058321972940E-03;
			K_[0][31] = -3.3958519026368E-04;
			K_[0][32] = 5.5993651771592E-03;
			K_[0][33] = -3.0335118055646E-04;
			K_[0][34] = -2.1365488688320E+02;
			K_[0][35] = 2.6641569149272E+04;
			K_[0][36] = -2.4027212204557E+04;
			K_[0][37] = -2.8341603423999E+02;
			K_[0][38] = 2.1247284400179E+02;
			K_[0][39] = -6.6642276540751E-01;
			K_[0][40] = 7.2608632349897E-01;
			K_[0][41] = 5.5068668612842E-02;
		...
		case 'W':  // WATER
		{
			...
			K_[0][0] = 1.2533547935523E-02;
			K_[0][1] = 7.8957634722828E+00;
			K_[0][2] = -8.7803203303561E+00;
			K_[0][3] = 3.1802509345418E-01;
			K_[0][4] = -2.6145533859358E-01;
			K_[0][5] = -7.8199751687981E-03;
			K_[0][6] = 8.8089493102134E-03;
			K_[0][7] = -6.6856572307965E-01;
			K_[0][8] = 2.0433810950965E-01;
			K_[0][9] = -6.6212605039687E-05;
			K_[0][10] = -1.9232721156002E-01;
			K_[0][11] = -2.5709043003438E-01;
			K_[0][12] = 1.6074868486251E-01;
			K_[0][13] = -4.0092828925807E-02;
			K_[0][14] = 3.9343422603254E-07;

			K_[0][15] = -7.5941377088144E-06;
			K_[0][16] = 5.6250979351888E-04;
			K_[0][17] = -1.5608652257135E-05;
			K_[0][18] = 1.1537996422951E-09;
			K_[0][19] = 3.6582165144204E-07;
			K_[0][20] = -1.3251180074668E-12;
			K_[0][21] = -6.2639586912454E-10;
			K_[0][22] = -1.0793600908932E-01;
			K_[0][23] = 1.7611491008752E-02;
			K_[0][24] = 2.2132295167546E-01;
			K_[0][25] = -4.0247669763528E-01;
			K_[0][26] = 5.8083399985759E-01;
			K_[0][27] = 4.9969146990806E-03;
			K_[0][28] = -3.1358700712549E-02;
			K_[0][29] = -7.4315929710341E-01;

			K_[0][30] = 4.7807329915480E-01;
			K_[0][31] = 2.0527940895948E-02;
			K_[0][32] = -1.3636435110343E-01;
			K_[0][33] = 1.4180634400617E-02;
			K_[0][34] = 8.3326504880713E-03;
			K_[0][35] = -2.9052336009585E-02;
			K_[0][36] = 3.8615085574206E-02;
			K_[0][37] = -2.0393486513704E-02;
			K_[0][38] = -1.6554050063734E-03;
			K_[0][39] = 1.9955571979541E-03;
			K_[0][40] = 1.5870308324157E-04;
			K_[0][41] = -1.6388568342530E-05;
			K_[0][42] = 4.3613615723811E-02;
			K_[0][43] = 3.4994005463765E-02;
			K_[0][44] = -7.6788197844621E-02;

			K_[0][45] = 2.2446277332006E-02;
			K_[0][46] = -6.2689710414685E-05;
			K_[0][47] = -5.5711118565645E-10;
			K_[0][48] = -1.9905718354408E-01;
			K_[0][49] = 3.1777497330738E-01;
			K_[0][50] = -1.1841182425981E-01;
			K_[0][51] = -3.1306260323435E+01;
			K_[0][52] = 3.1546140237781E+01;
			K_[0][53] = -2.5213154341695E+03;
			K_[0][54] = -1.4874640856724E-01;
			K_[0][55] = 3.1806110878444E-01;
		...
		case 'N':  // Nitrogen
		{
			...
			// real gas part
			K_[0][0] = 0.924803575275;
			K_[0][1] = -0.492448489428;
			K_[0][2] = 0.661883336938;
			K_[0][3] = -0.192902649201e1;
			K_[0][4] = -0.622469309629e-1;
			K_[0][5] = 0.349943957581;
			K_[0][6] = 0.564857472498;
			K_[0][7] = -0.161720005987e1;
			K_[0][8] = -0.481395031883;
			K_[0][9] = 0.421150636384;
			K_[0][10] = -0.161962230825e-1;
			K_[0][11] = 0.172100994165;
			K_[0][12] = 0.735448924933e-2;
			K_[0][13] = 0.168077305479e-1;
			K_[0][14] = -0.107626664179e-2;
			K_[0][15] = -0.137318088513e-1;
			K_[0][16] = 0.635466899859e-3;
			K_[0][17] = 0.304432279419e-2;
			K_[0][18] = -0.435762336045e-1;
			K_[0][19] = -0.723174889316e-1;
			K_[0][20] = 0.389644315272e-1;
			K_[0][21] = -0.212201363910e-1;
			K_[0][22] = 0.408822981509e-2;
			K_[0][23] = -0.551990017984e-4;
			K_[0][24] = -0.462016716479e-1;
			K_[0][25] = -0.300311716011e-2;
			K_[0][26] = 0.368825891208e-1;
			K_[0][27] = -0.255856846220e-2;
			K_[0][28] = 0.896915264558e-2;
			K_[0][29] = -0.441513370350e-2;
			K_[0][30] = 0.133722924858e-2;
			K_[0][31] = 0.264832491957e-3;
			K_[0][32] = 0.196688194015e2;
			K_[0][33] = -0.209115600730e2;
			K_[0][34] = 0.167788306989e-1;
			K_[0][35] = 0.262767566274e4;
...
//*****************************************************************************/
// double DuanMixCompressibility(double T,double P,double V,CVirialCoefficients
// w)
double DuanMixCompressibility(double T, double P, double V,
                              VirialCoefficients w)
{
	double R = 83.14472;
	return P * V / R / T - 1 - w.B / V - w.C / (V * V) - w.D / (V * V * V * V) -
	       w.E / (V * V * V * V * V) -
...
double k_co2_h20(int number, double T)
{
	...
	else if (T > 673.15)
	{
		switch (number)
		{
			...
			case 2:
				return -46.646 + 4.2877e-02 * T - 1.0892e-05 * T * T +
				       1.5782e+04 / T;
			case 3:
				return 0.9;
			default:
	...
	else if (T < 495.15)
	{
		...
		{
			case 1:
				return -10084.5042 - 4.27134485 * T + 256477.783 / T +
				       0.00166997474 * T * T + 1816.78 * log(T);
			case 2:
				return 9.000263 - 0.00623494 * T - 2307.7125 / T;
			case 3:
				return -74.1163 + 0.1800496 * T - 1.40904946e-4 * T * T +
				       101305246 / T;
			default:
				return 1;
		}
...
double k_ch4_h2o(int number, double T)
{
	(void)number;
	(void)T;
	// Not implemented yet! If you feel constrained to change this, you'll find
	// the correlation in Duan, Moller and Weare ,1992.
	return 1;
}
...
{
	// number : k1, k2 or k3
	// fluid_a, fluid_b: a-b, b-c, a-c have different binary interaction
	// parameters
	// i,j,k  : needed for mixing rule
	// T : bip depends on Temperature

	if (((number == 1) && (i == j)) ||
	    (((number == 2) || (number == 3)) && ((i == j) && (i == k))))
		return 1;
	else
		switch (fluid_a + fluid_b)
		{
			case (1):  // 2+3=5 --> CO2+H2O (duan 1992) or 0+1=1 --> CO2+H20 fo
				       // high pressure range, duan 2006
				return k_co2_h20(number, T);
			case (5):  // 2+3=5 --> CO2+H2O (duan 1992) or 0+1=1 --> CO2+H20 fo
				       // high pressure range, duan 2006
				return k_co2_h20(number, T);
...
//* Programming: NB, Sep10
//*****************************************************************************/
// void MixDuansVirialCoefficients(CVirialCoefficients fluid_a,
// CVirialCoefficients fluid_b, double T, double x_0, CVirialCoefficients*mix)
...
{
	// this code may look weird, I tried to avoid functions like pow() to
	// increase speed
		...
		for (int j = 0; j < 2; j++)
		{
			...
			for (int k = 0; k < 2; k++)
			{
				...
				for (int l = 0; l < 2; l++)
					for (int m = 0; m < 2; m++)
					{
						double D1 = ((W3(D[i]) + W3(D[j]) + W3(D[k]) +
						              W3(D[l]) + W3(D[m])) /
						             5);
						V5 = ((W3(Vc[i]) + W3(Vc[j]) + W3(Vc[k]) + W3(Vc[l]) +
						       W3(Vc[m])) /
						      5);
						Dij = D1 * D1 * D1;
						Vcij = V5 * V5 * V5;
						DVc += x[i] * x[j] * x[k] * x[l] * x[m] * Dij * Vcij *
						       Vcij * Vcij * Vcij;
						for (int n = 0; n < 2; n++)
						{
							double E1 = ((W3(E[i]) + W3(E[j]) + W3(E[k]) +
							              W3(E[l]) + W3(E[m]) + W3(E[n])) /
							             6);
							V6 = ((W3(Vc[i]) + W3(Vc[j]) + W3(Vc[k]) +
							       W3(Vc[l]) + W3(Vc[m]) + W3(Vc[n])) /
							      6);
							Eij = E1 * E1 * E1;
							Vcij = V6 * V6 * V6;
							EVc += x[i] * x[j] * x[k] * x[l] * x[m] * x[n] *
							       Eij * Vcij * Vcij * Vcij * Vcij * Vcij;
						}  // ijklmn
					}      // ijklm
			}              // ijk
		}                  // ij
...
//*  Programming: NB, Sep10
//*****************************************************************************/
double DuansMixingRule(double T, double P, double x, int fluid1, int fluid2,
                       bool neu)
{
	(void)neu;  // unused
	P /= 1e5;
