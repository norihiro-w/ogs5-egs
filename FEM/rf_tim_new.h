...
public:
	...
	// WW vector<double> time_step_target_vector; // kg44 for adaptive
	// steps..intermediate time target that need to be reached
	double time_start;
	double time_end;
	double time_current;
...
public:
	...
	double CalcTimeStep(
	    double crt_time = 0.0);  // Add argument double crt_time. 25.08.2008. WW
	...
	//
	// JT: done differently now. // double CheckTime(double const c_time, const
	// double dt0);
...
//
	...
	bool PIDControl();
	bool isPIDControl() const
	{
		return usePIDControl || usePIDControlInSelfAdaptive;
	}
};
...
extern double dt;
extern int rwpt_numsplits;  // JT 2010, for specifying sub time step for random
                            // walker in .tim input file
#endif
