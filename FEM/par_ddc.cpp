...
vector<int> node_connected_doms;  // This will be removed after sparse class is
                                  // finished WW
...
void DOMCreate()
{
	...
	{
		m_pcs = pcs_vector[i];
		//    if(m_pcs->pcs_type_name.find("DEFORMATION")!=string::npos) { // TF
		//    10/2010
...
long CPARDomain::GetDOMNode(long global_node)
{
	...
	for (i = 0; i < no_nodes; i++)
		if (nodes[i] == global_node) return i;
	return -1;
}
...
void CPARDomain::CreateEQS()
{
	...
	if (sparse_graph) eqs = new Linear_EQS(*sparse_graph, dof_nonDM);
	if (sparse_graph_H) eqsH = new Linear_EQS(*sparse_graph_H, dof_DM);
}
	...
	void CPARDomain::CreateEQS(CRFProcess * m_pcs)
	{
		...
		if (m_pcs->type == 4)
			eqs = CreateLinearSolverDim(
			    m_pcs->m_num->ls_storage_method, dof, dof * nnodesHQ_dom);
		//    InitializeLinearSolver(eqs,m_num);
		else if (m_pcs->type == 41)
			eqs = CreateLinearSolverDim(m_pcs->m_num->ls_storage_method,
			                            dof,
			                            dof * nnodesHQ_dom + nnodes_dom);
...
bool NodeExists(long node, vector<long> node_vector)
{
	...
	for (i = 0; i < no_nodes; i++)
		if (node == node_vector[i]) return true;
	return false;
}
...
void CPARDomain::WriteTecplot(string msh_name)
{
	...
	dom_name += name;
	string dom_file_name = dom_name + TEC_FILE_EXTENSION;
	...
	for (i = 0; i < no_nodes; i++)
	{
		...
		dom_file << coords[0] << " " << coords[1] << " " << coords[2] << " "
		         << ID << endl;
		//      m_nod = m_msh->nod_vector[i];
		//      dom_file << m_nod->X() << " " << m_nod->Y() << " " << m_nod->Z()
		//      << " " << ID << endl;
	}
	//......................................................................
	for (i = 0; i < (long)elements.size(); i++)
	{
		...
		{
			case MshElemType::LINE:
				dom_file << m_ele->getNodeIndices()[0] + 1 << " "
...
// Sort
	...
	for (int k = 0; k < (int)dom_vector.size(); k++)
	{
		...
		for (i = 0; i < (long)m_dom->nodes.size(); i++)
		{
			...
			if (node_connected_doms[g_index] > 1.0)
			{
				if (g_index >= nnodes_l)
				{
					...
					long_buffer[i] =
					    -(long)boundary_nodes_HQ.size() - nnodes_gl;
				}
				else
				{
		...
		for (i = 0; i < (long)inner_nodes_HQ.size(); i++)
			m_dom->nodes[i + j] =
			    m_dom->nodes_inner[i + m_dom->num_inner_nodes];
		//     m_dom->nodes_inner[i+m_dom->num_inner_nodes] = i+j;
		j += (long)inner_nodes_HQ.size();
		for (i = 0; i < (long)boundary_nodes_HQ.size(); i++)
			m_dom->nodes[i + j] =
			    m_dom->nodes_halo[i + m_dom->num_boundary_nodes];
...
void DDCCreate()
{
	...
	if (dom_vector.size() > 0)
	{
		...
		if (!DOF_gt_one) m_pcs = pcs_vector[0];
		// -----------------------
		DOMCreate();
		//
		for (i = 0; i < no_processes; i++)
...
void CPARDomain::ConfigEQS(CNumerics* m_num, const long n, bool quad)
{
	...
	//
	// receive_cnt_i[]: number of subdomain inner nodes in the concatenated
	// array
	MPI_Allgatherv(&inner_size, 1, MPI_INT, receive_cnt_i, receive_cnt,
	               receive_disp, MPI_INT, MPI_COMM_WORLD);
	inner_size = 0;
	for (i = 0; i < mysize; i++)
	{
...
#if defined(NEW_BREDUCE)
	// receive_cnt_b[]: number of subdomain border nodes in the concatenated
	// array
	MPI_Allgatherv(&border_size, 1, MPI_INT, receive_cnt_b, receive_cnt,
	               receive_disp, MPI_INT, MPI_COMM_WORLD);
	border_size = 0;
	for (i = 0; i < mysize; i++)
	{
	...
	if (quadratic)
	{
		eqsH->f_buffer[(int)eqsH->f_buffer.size() - 3] =
		    new double[border_size];
...
double CPARDomain::Dot_Interior(const double* localr0, const double* localr1)
{
	...
	/*
	   ...
	   Dum<<"[i+n_loc*ii] "<< i+n_loc*ii <<" localr0[i+n_loc*ii] "<<
	   localr0[i+n_loc*ii]<<endl;
...
/*************************************************************************
   ...
   12/2007 WW Revise
 **************************************************************************/
void CPARDomain::Global2Local(const double* global_x, double* local_x,
                              const long n)
{
	long i, ig;
	int ii;
...
/*************************************************************************
   ...
   02/2008 WW Revise
 **************************************************************************/
void CPARDomain::Local2Global(const double* local_x, double* global_x,
                              const long n)
{
	long i, ig, b_index;
	int ii, k;
	...
	for (k = 0; k < nq; k++)
		for (i = b_start[k]; i < b_end[k]; i++)
		{
			...
			ig = nodes[b_index];
			for (ii = 0; ii < dof; ii++)
				global_x[ig + n_global * ii] +=
				    fac * local_x[b_index + n_loc * ii];
		}
	//
}
 ...
 ********************************************************************/
//#define NEW_BREDUCE2
void CPARDomain::CatInnerX(double* global_x, const double* local_x,
                           const long n)
{
	long i, j, ig;
	int ii, k;
...
//
	...
	for (j = 0; j < mysize; j++)
	{
		...
		for (k = 0; k < a_dom->nq;
		     k++)  // This should come from different processors
...
void CPARDomain::ReduceBorderV(double* local_x)
{
	...
	// Concatentate
	MPI_Allgatherv(x_g, counter, MPI_DOUBLE, x_cat, receive_cnt_b,
	               receive_disp_b, MPI_DOUBLE, MPI_COMM_WORLD);
	for (i = 0; i < dof * n_bc; i++)
		x_b[i] = 0.0;
	//
	CPARDomain* a_dom;
