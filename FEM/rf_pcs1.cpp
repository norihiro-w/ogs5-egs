...
void CRFProcess::Recovery_du_JFNK()
{
	...
	{
		p_idx = p_var_index[k];
		if (k < problem_dimension_dm) p_idx--;
	...
	for (i = 0; i < (long)BC_JFNK.size(); i++)
	{
		...
		if (bc_entry.incremental) continue;
		SetNodeValue(bc_entry.bc_node, bc_entry.var_idx,
		             GetNodeValue(bc_entry.bc_node, bc_entry.var_idx) -
		                 bc_entry.bc_value);
	}
}
...
bool CRFProcess::ForceTermCriterion(double* Jdx, const int num_iteration)
{
	...
	if (norm_f_pls_Jdx < f_factor * norm_F0) done = true;
...
void CRFProcess::Jacobian_Multi_Vector_JFNK(double* v, double* Jv)
{
	...
	// float sign_rhs;
	bool HM = false;
	if (problem_dimension_dm != pcs_number_of_primary_nvals) HM = true;
	...
	if (v == NULL)
	{
		...
		norm_u_JFNK[0] = 0.;
		if (HM) norm_u_JFNK[1] = 0.;
		j = 0;
		for (k = 0; k < pcs_number_of_primary_nvals; k++)
		{
			p_idx = p_var_index[k];
			if (k < problem_dimension_dm && HM) p_idx--;
...
#define pert_2
	...
	for (k = 0; k < pcs_number_of_primary_nvals; k++)
	{
		...
		if ((k == problem_dimension_dm - 1) && HM) norm_v[1] = norm_v[0];
	}
	if (HM)
	{
		norm_v[0] -= norm_v[1];
...
#define apert_2
				...
				perturbation[k] =
				    b_fac * perturbation[k] / (norm_v[k] * size) + b_fac;
				if (perturbation[k] < DBL_MIN) perturbation[k] = pert_defalt;
#else
				perturbation[k] =
		...
		for (k = 0; k < num_pcs; k++)
		{
			if (norm_v[k] > DBL_MIN)
			{
				...
				perturbation[k] =
				    sign * b_fac * max(udv[k], typu[k] * uv[k]) / norm_v[k];
			}
			else
				perturbation[k] = pert_defalt;
			if (perturbation[k] < DBL_MIN) perturbation[k] = pert_defalt;
		...
		for (k = 0; k < pcs_number_of_primary_nvals; k++)
		{
			...
			for (i = 0; i < num_nodes_p_var[k]; i++)
			{
				SetNodeValue(i, p_idx,
				             array_u_JFNK[j] + perturbation[0] * v[j]);
				j++;
			}
		}
...
double CRFProcess::LineSearch()
{
	...
	for (;;)
	{
		...
		for (k = 0; k < pcs_number_of_primary_nvals; k++)
		{
			...
			{
				SetNodeValue(i, p_idx,
				             array_u_JFNK[j] + damping * eqs_new->x[j]);
				j++;
			}
		}
		GlobalAssembly();
