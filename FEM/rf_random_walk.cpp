...
#define CountParticleNumber  // YS: to count the number of particles leave the
                             // domain
...
RandomWalk::RandomWalk(int srand_seed)
{
	...
	numOfParticles = 0;
	leavingParticles = 0;  // YS: to count the number of particles in the
	                       // outflow
	...
	// run.
	if (srand_seed == 0) srand((int)time(0));
	// To produce same pseudo-random series each time your program is run.
	else if (srand_seed == 1)
		srand(1);

	// These are the allowable outputs (input as options to file
	// <file_base_name>.out
...
{
	//    if(X) delete [] X;
	if (ChanceOfIrreversed) delete[] ChanceOfIrreversed;
	...
	if (nnode == 4)
	{
		...
		for (int i = 0; i < nnode; ++i)
		{
			...
			/* //WW
			   ...
			   vx[i] =
			   m_pcs->GetNodeValue(theEle->GetNodeIndex(i),m_pcs->GetNodeValueIndex("VELOCITY1_X")+1)/porosity;
			   vy[i] =
			   m_pcs->GetNodeValue(theEle->GetNodeIndex(i),m_pcs->GetNodeValueIndex("VELOCITY1_Y")+1)/porosity;
			   vz[i] =
			   m_pcs->GetNodeValue(theEle->GetNodeIndex(i),m_pcs->GetNodeValueIndex("VELOCITY1_Z")+1)/porosity;
			 */
		}

		// solve for Jm at xm = (xhat,yhat)=(1/2,1/2) <- RT0
		double Jm;
		Jm = 0.5 * (x[0] * y[1] - y[0] * x[1] + x[1] * y[2] - y[1] * x[2] +
		            y[0] * x[3] - x[0] * y[3] + x[2] * y[3] - y[2] * x[3]);
		...
		for (int i = 0; i < 4; ++i)
		{
			...
			double node1[3] = {(theNodesOfThisEdge[0]->getData())[0],
			                   (theNodesOfThisEdge[0]->getData())[1], 0.0};
			double node2[3] = {(theNodesOfThisEdge[1]->getData())[0],
			                   (theNodesOfThisEdge[1]->getData())[1], 0.0};
			// TF         node1[0] = theNodesOfThisEdge[0]->X(); node1[1] =
			// theNodesOfThisEdge[0]->Y();
			// TF        node2[0] = theNodesOfThisEdge[1]->X(); node2[1] =
			// theNodesOfThisEdge[1]->Y();
			...
			double angle = acos(
			    (dx * edge2mid[0] + dy * edge2mid[1]) /
			    (sqrt(dx * dx + dy * dy) *
			     sqrt(edge2mid[0] * edge2mid[0] + edge2mid[1] * edge2mid[1])));
		...
		if (t[0] < 0.0) t[0] = bigTime;
		t[1] = TB(Gx, uT, u[0], u[1], R[1]);
		if (t[1] < 0.0) t[1] = bigTime;
		t[2] = TA(Gy, vT, u[2], R[1]);
		if (t[2] < 0.0) t[2] = bigTime;
		t[3] = TB(Gy, vT, u[2], u[3], R[1]);
		if (t[3] < 0.0) t[3] = bigTime;
...
void RandomWalk::InterpolateVelocityOfTheParticleByInverseDistance(Particle* A)
{
	...
	for (int i = 0; i < nnodes; ++i)
	{
		...
		if (m_msh->nod_vector[m_ele->GetNodeIndex(i)]->crossroad)
		{
			...
			// Find the velocity of the crossroad associated with the connected
			// planes.
			for (int k = 0; k < crossroad->numOfThePlanes; ++k)
			{
				// I am going to check the normal vector of the element and the
				// connected plane.
		...
		{
			vx = m_pcs->GetNodeValue(
			    m_ele->GetNodeIndex(i),
			    m_pcs->GetNodeValueIndex("VELOCITY1_X") + 1);
			vy = m_pcs->GetNodeValue(
			    m_ele->GetNodeIndex(i),
			    m_pcs->GetNodeValueIndex("VELOCITY1_Y") + 1);
			vz = m_pcs->GetNodeValue(
			    m_ele->GetNodeIndex(i),
			    m_pcs->GetNodeValueIndex("VELOCITY1_Z") + 1);

			// Let's solve pore velocity.
			// It is simple because Sw stuff automatically handles in Richards
			// Flow.
			// Thus, I only divide Darcy velocity by porosity only to get pore
			// velocity.
			CMediumProperties* MediaProp = mmp_vector[m_ele->GetPatchIndex()];
			double porosity = 0.0;
...
/**************************************************************************
   ...
   last modification:
**************************************************************************/
void RandomWalk::InterpolateVelocityOfTheParticleByBilinear(int option,
                                                            Particle* A)
{
	...
	CFEMesh* m_mini(
	    new CFEMesh(m_msh->getGEOObjects(), m_msh->getProjectName()));
	...
	else if (option == 1)  // miniFEM Way
	{
		...
		// If Element is outside of the domain or in the sink or on the edge or
		// at the corner vertix
		// Do not do anything. If not, then proceed.
		...
		if (eleIndex != -10 && IsOnTheEdge == 0)
		{
			...
			/*//WW
			   ...
			   mmp_vector[theEle->GetPatchIndex()];
			   double porosity = 0.0;
			   if(MediaProp->porosity > 10-6)
			   porosity = MediaProp->porosity;       // This is for simple one.
			   else
			...
			if (nnode == 4)  // Quad
			{
				...
				// Node 1 For edge 0
				GetNodeOfMiniFEMforTheEdge(theNode, theEdgesOfThisElement[0],
				                           A);
				m_mini->nod_vector.push_back(theNode);
				...
				m_mini->nod_vector.push_back(theNode);
				// Node 3 For edge 1
				GetNodeOfMiniFEMforTheEdge(theNode, theEdgesOfThisElement[1],
				                           A);
				m_mini->nod_vector.push_back(theNode);
				...
				m_mini->nod_vector.push_back(theNode);
				// Node 5 For edge 2
				GetNodeOfMiniFEMforTheEdge(theNode, theEdgesOfThisElement[2],
				                           A);
				m_mini->nod_vector.push_back(theNode);
				...
				m_mini->nod_vector.push_back(theNode);
				// Node 7 For edge 3
				GetNodeOfMiniFEMforTheEdge(theNode, theEdgesOfThisElement[3],
				                           A);
				m_mini->nod_vector.push_back(theNode);
...
// Todo
					...
					x = new double[size];
#if defined(LIS)
					m_pcs->EQSSolver(x);  // an option added to tell
					                      // FLUID_MOMENTUM for sparse matrix
					                      // system.
			...
			// else
			//	;	// This shouldn't happen here. There are only tri or quad
			// ele's in 2D
		}
		else if (IsOnTheEdge != 0)  // The particle is on the edge.
		{
			...
			// Thus, we only need to interpolate particle velocity along this
			// edge.
	...
	else  // Real Space and reference space way
	{
		...
		if (eleIndex != -10)
		{
			...
			for (int i = 0; i < 4; ++i)
			{
				...
				// TF           node2[0] = theNodesOfThisEdge[1]->X(); node2[1]
				// = theNodesOfThisEdge[1]->Y();
				// Get the referece position of these two ending points of the
				// edge
				IsoparametricMappingQuadfromPtoR(A->elementIndex, node1);
				IsoparametricMappingQuadfromPtoR(A->elementIndex, node2);
				...
				double angle = acos((dx * edge2mid[0] + dy * edge2mid[1]) /
				                    (sqrt(dx * dx + dy * dy) *
				                     sqrt(edge2mid[0] * edge2mid[0] +
				                          edge2mid[1] * edge2mid[1])));

				// EA and EB: x1hat = x2hat and x1hat*x2hat > 0
				if (fabs(node1[0] - node2[0]) < 0.5 &&
				    (node1[0] * node2[0]) > 0.0)
				{
					// EA: x1hat < 0
					if (node1[0] < 0.0)
                                            ...
                                            Particle* A)
{
	...
	double x[2] = {theEdge->GetNode(0)->getData()[0],
	               theEdge->GetNode(0)->getData()[1]};
	double y[2] = {theEdge->GetNode(1)->getData()[0],
	               theEdge->GetNode(1)->getData()[1]};
...
int RandomWalk::IsParticleOnTheEdge(Particle* A)
{
	...
	if (nnode == 3 || nnode == 4)
	{
		...
		for (int i = 0; i < nnode; ++i)
		{
			...
			x1x0[2] = x1[2] - A->z;
			double x2x1square;
			x2x1square = (x2[0] - x1[0]) * (x2[0] - x1[0]) +
			             (x2[1] - x1[1]) * (x2[1] - x1[1]) +
			             (x2[2] - x1[2]) * (x2[2] - x1[2]);
...
/**************************************************************************
   ...
   last modification:
**************************************************************************/
double* RandomWalk::InterpolateLocationOfTheParticleByBilinear(Particle* A,
                                                               double dt)
{
	...
	if (eleIndex != -10)
	{
		...
		b = (theEdgesOfThisElement[1]->GetVelocity(0) -
		     theEdgesOfThisElement[3]->GetVelocity(0)) /
		    dx;
		c = theEdgesOfThisElement[0]->GetVelocity(1);
		d = (theEdgesOfThisElement[2]->GetVelocity(1) -
		     theEdgesOfThisElement[0]->GetVelocity(1)) /
		    dy;
...
int RandomWalk::IndexOfTheElementThatThisParticleBelong(int option, Particle* A)
{
	...
	if (option == 0)
	{
		...
		if (xrw_range >
		    1.e-12)  // only if non-negligible range in this direction
		{
			if (x > pnt_x_max || x < pnt_x_min) return index;
			i = (long int)floor((x - pnt_x_min) / dx);
		}
		if (yrw_range > 1.e-12)
		{
			if (y > pnt_y_max || y < pnt_y_min) return index;
			j = (long int)floor((y - pnt_y_min) / dy);
		}
		if (zrw_range > 1.e-12)
		{
			if (z > pnt_z_max || x < pnt_z_min) return index;
		...
		/*
		   if(index == 322 || index == 323 || index == 342 || index == 343)	//
		   Sink condition
		   return -10;
		   else
		 */
...
/**************************************************************************
   ...
   last modification:
**************************************************************************/
int RandomWalk::SolveForTwoIntersectionsInTheElement(Particle* A, double* P1,
                                                     double* P2, int axis)
{
	// Get the element that the particle belongs
	MeshLib::CElem* m_ele = m_msh->ele_vector[A->elementIndex];
	...
	for (int i = 0; i < nnodes; ++i)
	{
		...
		double X[3] = {node->getData()[0], node->getData()[1],
		               node->getData()[2]};
	...
	for (int i = 0; i < nnodes; ++i)
	{
		...
		for (int j = 0; j < 2; ++j)
		{
			...
			{
				printf(
				    "Axis type in searching the intersection failed. Wrong "
				    "axis type.\n");
				abort();
			}
			...
			int status = G_intersect_line_segments(p1[0],
			                                       p1[1],
			                                       p2[0],
			                                       p2[1],
			                                       p3[0],
...
/**************************************************************************
   ...
   last modification:
**************************************************************************/
int RandomWalk::SolveForDisplacementByDerivativeOfDispersion(Particle* A,
                                                             double* dD)
{
	double TensorOfdD[9];
	...
	TensorOfdD[0] =
	    V[0] * A->dVxdx * (alphaL * (2.0 / U - V[0] * V[0] / (U * U * U)) -
	                       alphaT * (V[1] * V[1] + V[2] * V[2]) / (U * U * U));
	TensorOfdD[1] =
	    (alphaL - alphaT) *
	    (A->dVydy * V[0] / U - V[0] * V[1] * V[1] / (U * U * U) * A->dVydy);
	TensorOfdD[2] =
	    (alphaL - alphaT) *
	    (A->dVzdz * V[0] / U - V[0] * V[2] * V[2] / (U * U * U) * A->dVzdz);
	TensorOfdD[3] =
	    (alphaL - alphaT) *
	    (A->dVxdx * V[1] / U - V[1] * V[0] * V[0] / (U * U * U) * A->dVxdx);
	TensorOfdD[4] =
	    V[1] * A->dVydy * (alphaL * (2.0 / U - V[1] * V[1] / (U * U * U)) -
	                       alphaT * (V[0] * V[0] + V[2] * V[2]) / (U * U * U));
	TensorOfdD[5] =
	    (alphaL - alphaT) *
	    (A->dVzdz * V[1] / U - V[1] * V[2] * V[2] / (U * U * U) * A->dVzdz);
	TensorOfdD[6] =
	    (alphaL - alphaT) *
	    (A->dVxdx * V[2] / U - V[2] * V[0] * V[0] / (U * U * U) * A->dVxdx);
	TensorOfdD[7] =
	    (alphaL - alphaT) *
	    (A->dVydy * V[2] / U - V[2] * V[1] * V[1] / (U * U * U) * A->dVydy);
	TensorOfdD[8] =
	    V[2] * A->dVzdz * (alphaL * (2.0 / U - V[2] * V[2] / (U * U * U)) -
...
int RandomWalk::SolveForDerivativeOfVelocity(Particle* A)
{
	...
	if (m_ele->GetElementType() != MshElemType::LINE)
	{
		...
		double x1[3], x2[3], y1[3],
		    y2[3];  // I don't put the intersections for z direction for now.
		...
		if (status == -1)
			//		printf("Solving two intersections parallel to x axis
			// failed\n");
			return -1;  // Failed
		...
		double y = XR.y - XL.y;
		double z = XR.z - XL.z;
		double dx = sqrt(x * x + y * y +
		                 z * z);  // The distance does not make any difference.
		// RWPT-IM
		// Let me think if velocity should projected to the connected plane or
		// treated in true 3D.
		...
		if (status == -1)
			//		printf("Solving two intersections parallel to y axis
			// failed\n");
			return -1;  // Failed
	...
	else  // 1D line element
	{
		...
		double v1[3], v2[3];
		v1[0] =
		    m_pcs->GetNodeValue(m_ele->GetNodeIndex(0),
		                        m_pcs->GetNodeValueIndex("VELOCITY1_X") + 1);
		v1[1] =
		    m_pcs->GetNodeValue(m_ele->GetNodeIndex(0),
		                        m_pcs->GetNodeValueIndex("VELOCITY1_Y") + 1);
...
double RandomWalk::ComputeVolume(Particle* A, MeshLib::CElem* m_ele)
{
	...
	if (m_ele->GetElementType() == MshElemType::LINE)
	{
		...
		for (int i = 0; i < 2; ++i)
		{
			...
			double x2buff[3] = {pnt[0] - A2buff[0], pnt[1] - A2buff[1],
			                    pnt[2] - A2buff[2]};
			PieceOfVolume[i] =
			    sqrt(x2buff[0] * x2buff[0] + x2buff[1] * x2buff[1] +
			         x2buff[2] * x2buff[2]);
...
/**************************************************************************
   ...
   09/2005 PCH Implementation
**************************************************************************/
double RandomWalk::ComputeVolume(Particle* A, Particle* element,
                                 MeshLib::CElem* m_ele)
{
	double x1buff[3];
	double x2buff[3];
	...
	// TETRAHEDRAS = 5, HEXAHEDRA = 3, PRISM = 6
	         ...
	         eleType == MshElemType::HEXAHEDRON ||
	         eleType == MshElemType::PRISM)
	{
		PieceOfVolume = new double[4]();
...
void RandomWalk::AdvanceBySplitTime(double dt, int numOfSplit)
{
	...
	double ctime =
	    0.0;  // JT 05.2010: for continuous source compatible with SplitTimes.
	leavingParticles = 0;
...
void RandomWalk::TraceStreamline(void)
{
	...
	for (int i = 0; i < numOfParticles; ++i)
		if (X[i].Now.elementIndex != -10)
		{
			...
			// Record path
			if (i < 50) RecordPath(i, &(X[i].Now));
		}

#ifdef _FEMPCHDEBUG_
	// PCH Let's monitor what's going on in the FEM
	// This messagebox is for debugging the primary variables at every time
	// step.
...
void RandomWalk::AdvanceToNextTimeStep(double dt, double ctime)
{
	...
	for (int i = 0; i < numOfParticles; ++i)
	{
		TimeMobility = 0;  // JT 2010, using this for now. Setting identity = 1
		                   // causes simulation failure... not sure why??
		// X[i].Now.identity=1;
		...
		// components defined in .mcp should be syncronized with identity of
		// particles.
		...
		if (m_cp->mobil && TimeMobility > 0)
		{
			...
			if (X[i].Now.elementIndex != -10 && X[i].Now.identity != 2)
			{
				...
				if (PURERWPT != 2)
					...
					InterpolateVelocityOfTheParticleByBilinear(GridOption,
					                                           &(X[i].Now));
				...
				if (RWPTMode < 2 ||
				    RWPTMode > 3)  // 0 or 1 for advection and dispersion cases.
				...
				// Record path
				// if(i<50) // JT :: no longer needed... just use .particle
				// outpus and Paraview
				// RecordPath(i, &Y);
				...
				do
				{
					...
					if (RWPTMode < 4) SolveForDerivativeOfVelocity(&Y);
					if (RWPTMode < 2 ||
					    RWPTMode > 3)  // whenever dispersion is on
						SolveDispersionCoefficient(&Y);

					if (Astatus == -1) Y.t = dt;
					if (X[i].Now.identity ==
					    0)  // YS: attached and filtered particles don't move
						Astatus = SolveForNextPosition(&(X[i].Now), &Y);

#ifdef CountParticleNumber
					if (m_pcs->rwpt_app == 2)
					{
						if (Y.x < 1.e-20) Y.x = 1.e-20;
						if ((Y.identity != 2) &&
						    (Y.x > 0.1 || Y.y > 100 || Y.z > 100))
						{
							leavingParticles++;
					...
					if (RWPTMode % 2 == 1)
					{
						if (PURERWPT != 2)
							Y.elementIndex =
							    IndexOfTheElementThatThisParticleBelong(1, &Y);
						//			Y.elementIndex =
						// GetTheElementOfTheParticle(&(X[i].Now), &Y);
						else
							Y.elementIndex =
							    IndexOfTheElementThatThisParticleBelong(0, &Y);
					}
					...
					if (Y.elementIndex == -10)
					{
						...
						// record the particle postion that includes the element
						// index = -10.
						X[i].Now = Y;
						break;
					}
					...
					else
					{
						...
						if (Y.t < tolerance) Y.t = dt;

						// Update the current info
						// where the advected particle is in either the element
						// or
		...
		{
			/*
			         // Do mobile-Immobile by switching the identity of
			   particles
			         double ChanceOfMobile = randomZeroToOne();
			         double ChanceOfImmobile = randomZeroToOne();

			         // Rate coefficients definition here
			         // For the use of the existing parameters, I borrow
			   Freundlich non-isotherm parameter set,
			   ...
			   {
			   if(numOfComps > 2)	// If the number of components is bigger
			   than
			   2,
			   ...
			   else if(X[i].Now.identity == 1 && numOfComps > 1)	// Among
			   immobile particles,
			   {
			   ...
			   {
			   cout<< "Only Identity 0 and 1 are covered. There are more than 2
			   identities detected" << endl;
			   abort();
			   }
		...
		else if (m_pcs->rwpt_app ==
		         2)  // Is the application Cryptosporidium oocysts?
		{
			...
			// Irreversible Reactions - Oocysts filtered for good or some
			// chemicals decayed
			// C/C0 = exp(-kt)
...
/**************************************************************************
   MSHLib-Method:
   Task: Determines when to generate output files, and introduces call to data
output
   Programing:
   03/2010 JT
**************************************************************************/
void RandomWalk::RandomWalkOutput(double dbl_time, int current_time_step)
{
	...
	for (size_t i = 0; i < rwpt_out_strings.size(); i++)
	{
		...
		if (no_times == 0 && (out->getNSteps() > 0) &&
			...
			outputornot = true;
		if (current_time_step < 2) outputornot = true;
		...
		// DATWriteParticleConcFile(current_time_step); // routine not yet
		// configured
		{
			for (size_t j = 0; j < no_times; j++)
				if (CurrentTime >= out->getRWPTTimeVector()[j])
				{
					...
					// else
					// if(current_name.compare("PARTICLE_CONCENTRATION")==0)
					// DATWriteParticleConcFile(current_time_step); // routine
					// not yet configured (see commented text below)
	...
	/*
	   ...
	   UnitConcentration = (double)numOfParticles / (double)
	   ...
	   double NormConcentrationOfTheElement = (double)CountInThisElement /
	   UnitConcentration;
	   //		double NormConcentrationOfTheElement =
	   (double)CountInThisElement/numOfParticles;
	   SetElementValue(i, GetElementValueIndex("CONCENTRATION0")+1,
	   NormConcentrationOfTheElement);
	   }
	 */
...
void RandomWalk::ConcPTFile(const char* file_name)
{
	...
	for (size_t i = 0; i < m_msh->nod_vector.size(); ++i)
	{
		...
		if (MaxX < x_coord) MaxX = x_coord;
		if (MinX > x_coord) MinX = x_coord;
	}
	...
	for (int i = 0; i < gridDensity; ++i)
	{
		...
		for (int j = 0; j < numOfParticles; ++j)
		{
			...
			if ((X[j].Now.x >= seg_start) && (X[j].Now.x < seg_end)) ++count;
		}
...
/**************************************************************************
   ...
   10/2005 PCH Implementation
**************************************************************************/
void RandomWalk::RandomlyDriftAway(Particle* A, double dt, double* delta,
                                   int type)
{
	// OK411
	type = type;
	...
	// Let's generate three random components N(0,1) and use it to compute
	// deltaOfX
	double Z[3];
	...
	else if (ele_dim == 2)
	{
		if (UniformOrNormal == 1)
		{
			...
			delta[0] = sqrt(6.0 * A->D[0] * dt) * Z[0] +
			           sqrt(6.0 * A->D[1] * dt) * Z[1];
			delta[1] = sqrt(6.0 * A->D[3] * dt) * Z[0] +
			           sqrt(6.0 * A->D[4] * dt) * Z[1];
			delta[2] = 0.0;
		}
		else
		{
			...
			Z[1] = Marsaglia();
			delta[0] = sqrt(2.0 * A->D[0] * dt) * Z[0] +
			           sqrt(2.0 * A->D[1] * dt) * Z[1];
			delta[1] = sqrt(2.0 * A->D[3] * dt) * Z[0] +
			           sqrt(2.0 * A->D[4] * dt) * Z[1];
			delta[2] = 0.0;
		}
	}
	else if (ele_dim == 3)
	{
		if (UniformOrNormal == 1)
		{
			...
			delta[0] = sqrt(6.0 * A->D[0] * dt) * Z[0] +
			           sqrt(6.0 * A->D[1] * dt) * Z[1] +
			           sqrt(6.0 * A->D[2] * dt) * Z[2];
			delta[1] = sqrt(6.0 * A->D[3] * dt) * Z[0] +
			           sqrt(6.0 * A->D[4] * dt) * Z[1] +
			           sqrt(6.0 * A->D[5] * dt) * Z[2];
			delta[2] = sqrt(6.0 * A->D[6] * dt) * Z[0] +
			           sqrt(6.0 * A->D[7] * dt) * Z[1] +
			           sqrt(6.0 * A->D[8] * dt) * Z[2];
		}
		else
		{
			Z[0] = Marsaglia();
			...
			delta[0] = sqrt(2.0 * A->D[0] * dt) * Z[0] +
			           sqrt(2.0 * A->D[1] * dt) * Z[1] +
			           sqrt(2.0 * A->D[2] * dt) * Z[2];
			delta[1] = sqrt(2.0 * A->D[3] * dt) * Z[0] +
			           sqrt(2.0 * A->D[4] * dt) * Z[1] +
			           sqrt(2.0 * A->D[5] * dt) * Z[2];
			delta[2] = sqrt(2.0 * A->D[6] * dt) * Z[0] +
			           sqrt(2.0 * A->D[7] * dt) * Z[1] +
			           sqrt(2.0 * A->D[8] * dt) * Z[2];
		}
	}
}
...
void RandomWalk::SolveDispersionCoefficient(Particle* A)
{
	...
	molecular_diffusion_value =
	    m_cp->CalcDiffusionCoefficientCP(A->elementIndex, 1.0, m_pcs) *
	    MediaProp->TortuosityFunction(A->elementIndex, g, theta);
	molecular_diffusion_value /=
	    porosity;  // This should be divided by porosity in this RWPT method.

	// Just solve for the magnitude of the velocity to compute the dispersion
	// tensor
	V[0] = A->Vx;
	V[1] = A->Vy;
	V[2] = A->Vz;
	...
	// Some nice if condition to tell the need for transform will be nice.
	// Later....
	if (m_ele->GetDimension() < 3) ToTheXYPlane(m_ele, V);
	...
	if (Vmagnitude > tolerance)
	{
		// Dxx
		A->D[0] =
		    (alphaT * (V[1] * V[1] + V[2] * V[2]) + alphaL * V[0] * V[0]) /
		        Vmagnitude +
		    molecular_diffusion_value;
		// Dxy = Dyz
		...
		// Dyy
		A->D[4] =
		    (alphaT * (V[0] * V[0] + V[2] * V[2]) + alphaL * V[1] * V[1]) /
		        Vmagnitude +
		    molecular_diffusion_value;
		// Dyz = Dzy
		...
		// Dzz
		A->D[8] =
		    (alphaT * (V[0] * V[0] + V[1] * V[1]) + alphaL * V[2] * V[2]) /
		        Vmagnitude +
		    molecular_diffusion_value;
	}
	else
...
int RandomWalk::SolveForNextPosition(Particle* A, Particle* B)
{
	...
	if (ele_dim < 3)
	{
		...
		for (int i = 0; i < nEdges; ++i)
		{
			...
			double X1[3] = {theNodes[0]->getData()[0],
			                theNodes[0]->getData()[1],
			                theNodes[0]->getData()[2]};
			double X2[3] = {theNodes[1]->getData()[0],
			                theNodes[1]->getData()[1],
			                theNodes[1]->getData()[2]};
			//         X1[0] = theNodes[0]->X(); X1[1] = theNodes[0]->Y(); X1[2]
			//         = theNodes[0]->Z();
			//         X2[0] = theNodes[1]->X(); X2[1] = theNodes[1]->Y(); X2[2]
			//         = theNodes[1]->Z();
			...
			if (dDStatus == 1)
			{
				...
				{
					// RWPT - IM
					// This should be done carefully. Velocity should be
					// transformed to be on the XY plane.
					// dD[] should be fine because it is handled in the
					// SolveForDisplacementByDerivativeOfDispersion function.
					// Z[] should also be fine. Just randome nubmers.
					// D[] Yes, this should be fine too. It is handled in the
					// SolveDispersionCoefficient function.
			...
			if (status == 1)
			{
				...
				else
				{
					...
					if (d1 > tolerance)
					{
						...
						// But, keep in mind in this displacement there is no
						// advective displament.
						double Vzero[3];
						...
						// Let's convert these XY plance coordinates to the real
						// plane coordinates.
						...
						return 1;  // The element index switched to the neighbor
						           // element
					}
			...
			{
				printf(
				    "The program aborts because two segments are colinear.\n");
				++countNoIntersection;
				// keep going.
			}
			else
			{
				printf(
				    "The program aborts because status of intersection search "
				    "is not 1 or 0 or -1.\n");
				abort();
			}
		}
		...
		if (countNoIntersection == nEdges)
		{
			if (ele_dim == 2)
			{
				...
				// Assigning the next postion of the particle. The index of
				// element in this if condition
				// should be one of the connected planes randomly chosen.
				// Now just solve the real plane coordinates for the particle at
				// the next position.
				double P[3];
				P[0] = B->x;
				P[1] = B->y;
	...
	else  // For 3D elements
	{
		...
		if (RWPTMode % 2 == 1)
			// This currently only return TRUE (1)
			// WW dDStatus = SolveForDisplacementByDerivativeOfDispersion(A,
			// dD);
...
**************************************************************************/
void RandomWalk::GetDisplacement(Particle* B, double* Z, double* V, double* dD,
                                 double time, double* dsp)
{
	// Mount the proper mesh
	m_msh = selectMeshForFluidMomentumProcess();
	//	for(int i=0; i< (int)pcs_vector.size(); ++i)
	...
	MeshLib::CElem* theElement = m_msh->ele_vector[B->elementIndex];
	int ele_dim = theElement->GetDimension();
	double Dxx = 0.0, Dxy = 0.0, Dxz = 0.0, Dyx = 0.0, Dyy = 0.0, Dyz = 0.0,
	       Dzx = 0.0, Dzy = 0.0, Dzz = 0.0;
...
//	}
	...
	if (Pold->elementIndex != -10)
	{
		...
		for (int i = 0; i < nnode; ++i)
		{
			...
			// RWPT - IM
			// Two points in the edge
			double X1[3] = {theNodesOfThisEdge[0]->getData()[0],
			                theNodesOfThisEdge[0]->getData()[1],
			                theNodesOfThisEdge[0]->getData()[2]};
			double X2[3] = {theNodesOfThisEdge[1]->getData()[0],
			                theNodesOfThisEdge[1]->getData()[1],
			                theNodesOfThisEdge[1]->getData()[2]};
//         double X1[3], X2[3];
//         X1[0] = theNodesOfThisEdge[0]->X(); X1[1] =
//         theNodesOfThisEdge[0]->Y(); X1[2] = theNodesOfThisEdge[0]->Z();
//         X2[0] = theNodesOfThisEdge[1]->X(); X2[1] =
//         theNodesOfThisEdge[1]->Y(); X2[2] = theNodesOfThisEdge[1]->Z();
#ifdef TWODINTHREED
			ToTheXYPlane(theElement, X1);
			ToTheXYPlane(theElement, X2);
#endif
			...
			if (status == 0)  // Not intersect but extension intersects
				...
				;
			else if (status == 1)  // single intersection - Means the current
				                   // position is outside of this element

				// Do further implementation here.
				index = theEdgesOfThisElement[i]->connected_elements[1];
			else if (status == 2)            // Overlap just do nothing
				index = Pold->elementIndex;  // This should indicate the
			                                 // particle in this element, then.
			else
			{
	...
	if (A->elementIndex != -10)
	{
		...
		for (size_t i = 0; i < theElement->GetFacesNumber(); ++i)
		{
			...
			if (thisNeighbor->GetElementType() != MshElemType::LINE)
			{
				...
				{
					MeshLib::CElem* theNeighborsNeighbor =
					    thisNeighbor->GetNeighbor(j);

					if (theNeighborsNeighbor->GetElementType() !=
					    MshElemType::LINE)
					{
						// If the particle belongs to this element
						...
						for (size_t k = 0;
						     k < theNeighborsNeighbor->GetFacesNumber();
						     ++k)
						{
							MeshLib::CElem* theNeighborsNeighborsNeighbor =
							    theNeighborsNeighbor->GetNeighbor(k);

							if (theNeighborsNeighborsNeighbor
							        ->GetElementType() != MshElemType::LINE)
							{
								// If the particle belongs to this element
								A->elementIndex =
								    theNeighborsNeighborsNeighbor->GetIndex();
								index = IsTheParticleInThisElement(A);
								if (index != -1) return index;
							}
						}
		...
		// If the code pases the following loop, it means I am not lucky in this
		// neighbor search.
...
int RandomWalk::IsTheParticleInThisElement(Particle* A)
{
	...
	if (ele_dim == 2)
	{
		// Getting the number of the edges in the element that Particle P
		// belongs
		int nEdges = theElement->GetEdgesNumber();
		...
		if (countOfInterception % 2 == 1) return A->elementIndex;
	...
	else if (ele_dim == 1)
	{
		...
		for (int i = 0; i < (int)m_msh->ele_vector.size(); ++i)
		{
			double const* const pnt1(
			    m_msh->nod_vector[m_msh->ele_vector[i]->GetNodeIndex(0)]
			        ->getData());
			double const* const pnt2(
			    m_msh->nod_vector[m_msh->ele_vector[i]->GetNodeIndex(1)]
			        ->getData());
			if (coordinateflag == 10)  // x only
			{
...
int RandomWalk::G_intersect_line_segments(double ax1, double ay1, double ax2,
                                          double ay2, double bx1, double by1,
                                          double bx2, double by2, double* ra,
...
int RandomWalk::G_intersect_line_segments_3D(double* pl1, double* pl2,
                                             double* pp1, double* pp2,
                                             double* pp3, double* pi)
{
	// Solve for norm of the plane by performing cross product to solve for the
	// plane
	...
	c = normOfThePlane[2];
	d = -(a * pp1[0] + b * pp1[1] +
	      c * pp1[2]);  // Refer pp1 point for solving d of the plane

	// Now solution of intersection
	double u, denominator;  // The line eqn: P=P1+u(P2-P1) where P1 and P2 is on
	                        // the line
	denominator =
	    a * (pl1[0] - pl2[0]) + b * (pl1[1] - pl2[1]) + c * (pl1[2] - pl2[2]);

	// Check if denominator is zero or not
	if (fabs(denominator) < 10e-8)  // If this is zero
	{
		// The line is either parallel or on the plane.
		// Check if the line is on the plane first by sustituting P1 into the
		// plane eqn.
	...
	else  // The line segment either is in the element or has an intersectional
	      // point.
	{
...
/**************************************************************************
   Task: The function solves reference coordinates of quad elements for a given
physical
			...
			if (fabs(s) > tolerance)  // s is not zero
			{
				if (fabs(r) > tolerance)  // r is not zero
				{
					...
					if (d >= 0.0)  // Only for real roots
					{
						...
						{
							printf(
							    "Failed to solve reference position for the "
							    "particle\n");
				...
				if ((fabs(ay) > tolerance) && (fabs(bx) > tolerance))
				{
					...
					else  // If ay is not zero and bx is not zero,
					{
						...
						if (d >= 0.0)  // Only for real roots
						{
							...
							{
								printf(
								    "Failed to solve reference position for "
								    "the particle\n");
								abort();  // Failed find the solution.
							}
						}
...
/**************************************************************************
   Task: The function solves physical coordinates of quad elements for a given
reference
      coordinates. For bilinear elements such as quadrilateral elements,
...
void RandomWalk::IsoparametricMappingQuadfromRtoP(int index, double* P)
{
	...
	if (nnode == 4)
	{
		...
		P[0] = 0.25 * (x[0] * (1.0 - phat[0]) * (1.0 - phat[1]) +
		               x[1] * (1.0 + phat[0]) * (1.0 - phat[1]) +
		               x[2] * (1.0 + phat[0]) * (1.0 + phat[1]) +
		               x[3] * (1.0 - phat[0]) * (1.0 + phat[1]));
		P[1] = 0.25 * (y[0] * (1.0 - phat[0]) * (1.0 - phat[1]) +
		               y[1] * (1.0 + phat[0]) * (1.0 - phat[1]) +
...
void RandomWalk::DoJointEffectOfElementInitially(void)
{
	...
	for (int p = 0; p < this->numOfParticles; ++p)
	{
		...
		for (int i = 0; i < numOfEdgeIntheElement; ++i)
			// Is this a joint?
			if (theEdges[i]->GetJoint() == 1) theJoint = theEdges[i];
...
{
	CFEMesh* m_msh = NULL;
	if (fem_msh_vector.size() == 0) return;
	m_msh = selectMeshForFluidMomentumProcess();
	//	// Mount the proper mesh
	//	for(int i=0; i< (int)pcs_vector.size(); ++i)
	//	{
...
void RandomWalk::SolveAnglesOfTheElment(MeshLib::CElem* E)
{
	...
	if (Enorm[0] * Enorm[0] + Enorm[2] * Enorm[2] < tolerance)
		...
		alpha =
		    acos(Enorm[2] / sqrt(Enorm[0] * Enorm[0] + Enorm[2] * Enorm[2]));
...
int RandomWalk::Select(double* roulette, int numOfCases)
{
	...
	for (int i = 0; i < numOfCases; ++i)
		if (probability < roulette[i]) return i;
	return 0;
}
...
int RandomWalk::ReadInVelocityFieldOnNodes(string file_base_name)
{
	...
	while (End)
	{
		...
		for (int i = 0; i < (int)m_msh->nod_vector.size(); ++i)
		{
			...
			for (int j = 0; j < 3; ++j)
			{
				int nidx1 = m_pcs->GetNodeValueIndex(
				                m_pcs->pcs_primary_function_name[j]) +
				            1;
				m_pcs->SetNodeValue(m_msh->Eqs2Global_NodeIndex[i], nidx1,
				                    v[j]);
			}
		}
		End = 0;
...
void RandomWalk::buildFDMIndex(void)
{
	...
	for (index = 0; index < 4;
	     index++)  // 4 nodes because FDM method only for equa-sized quads
	{
		//      x =
		//      m_msh->nod_vector[m_msh->ele_vector[0]->GetNodeIndex(index)]->X();
		//      y =
		//      m_msh->nod_vector[m_msh->ele_vector[0]->GetNodeIndex(index)]->Y();
		//      z =
		//      m_msh->nod_vector[m_msh->ele_vector[0]->GetNodeIndex(index)]->Z();
		double const* const xyz(
		    m_msh->nod_vector[m_msh->ele_vector[0]->GetNodeIndex(index)]
		        ->getData());
	...
	dy = ymax - ymin;
	dz = zmax - zmin;
	if (dx < 1.e-12) dx = 1.e-12;
	if (dy < 1.e-12) dy = 1.e-12;
	if (dz < 1.e-12) dz = 1.e-12;
	...
	for (k = 0; k < nz; k++)  // loop over the dummy element set
	{
		for (j = 0; j < ny; j++)
			for (i = 0; i < nx; i++)
			{
				FDMIndex one;  // store dummy index by default, initialize class
				               // vector
				one.i = i;
				one.j = j;
				one.k = k;
				one.eleIndex =
				    -5;  // eleIndex -5 is dummy index different from -10
				for (iel = 0; iel < nels; iel++)  // loop over mesh elements,
				                                  // assign them to dummy
				                                  // elements
				{
					double const* center =
					    m_msh->ele_vector[iel]->GetGravityCenter();
...
void PCTRead(string file_base_name)
{
	...
	while (End)
	{
		...
		for (int i = 0; i < RW->numOfParticles; ++i)
		{
			...
			pct_file >> idx >> x >> y >> z >> identity >> Starting >> vx >>
			    vy >> vz >> K >> ws;
			...
			// Creat pathline
			if (i < 50) RW->pathline.push_back(path);
		}
...
void DATWriteParticleFile(int current_time_step)
{
	...
	vtk_file << "DATASET POLYDATA"
	         << endl;  // KR vtk_file << "DATASET PARTICLES"  << endl;
	vtk_file << "POINTS " << RW->numOfParticles << " double" << endl;

	// Write particle locations
	for (int i = 0; i < np; ++i)
		vtk_file << RW->X[i].Now.x << " " << RW->X[i].Now.y << " "
		         << RW->X[i].Now.z << endl;
	...
	/*
	   ...
	   for(i=0; i<np; ++i)
	   vtk_file << RW->X[i].Now.Vx << " " << RW->X[i].Now.Vy << " " <<
	   RW->X[i].Now.Vz << endl;
	 */
	// Let's close it, now
	vtk_file.close();
