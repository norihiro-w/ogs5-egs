...
// TODO
			...
			else if (dimension == 2)
			{
				if (axis == 1)  // x,y only
					nidx1 =
					    m_pcs->GetNodeValueIndex(
					        m_pcs->pcs_primary_function_name[(axis - d + 1) %
					                                         dimension]) +
					    1;
				...
				nidx1 = m_pcs->GetNodeValueIndex(
				            m_pcs->pcs_primary_function_name[d]) +
				        1;
			else
				abort();  // Just stop something's wrong.
 ...
 */
		...
		for (i = 0; i < (long)m_msh->ele_vector.size(); i++)
		{
			...
			m_pcs->SetElementValue(
			    i, m_pcs->GetElementValueIndex("VELOCITY1_X") + 1, vx);
			m_pcs->SetElementValue(
			    i, m_pcs->GetElementValueIndex("VELOCITY1_Y") + 1, vy);
			m_pcs->SetElementValue(
			    i, m_pcs->GetElementValueIndex("VELOCITY1_Z") + 1, vz);

			/*
			               break;
			            case 1:
			               m_pcs->SetElementValue(i,
			   m_pcs->GetElementValueIndex("VELOCITY2_X")+1, vx);
			                    m_pcs->SetElementValue(i,
			   m_pcs->GetElementValueIndex("VELOCITY2_Y")+1, vy);
			                    m_pcs->SetElementValue(i,
			   m_pcs->GetElementValueIndex("VELOCITY2_Z")+1, vz);
			               break;
			            default:
			               cout << "Error in VELCalcElementVelocity: invalid
			   phase number" << endl;
			         }
			 */
		}
...
void CFluidMomentum::ConstructFractureNetworkTopology()
{
	...
	for (int i = 0; i < (int)m_msh->nod_vector.size(); ++i)
	{
		...
		for (int j = 1; j < NumOfNeighborElements; ++j)
		{
			    ...
			    m_msh->GetCoordinateFlag() != 22)
			{
				...
				for (int k = 0; k < 3; ++k)
					normOther[k] =
					    m_msh->ele_vector[indexOther]->getTransformTensor(k +
					                                                      6);

			// Check two norms are same.
			if (fabs(norm[0] - normOther[0]) < tolerance &&
			    fabs(norm[1] - normOther[1]) < tolerance &&
			    fabs(norm[2] - normOther[2]) < tolerance)
				;  // Two elements stay on the same plane
			else
			{
				thisNode->crossroad = true;
				// I am going to store all the element indeces which are
				// different from
				// the reference element. Then, I will get rid of the duplicate
				// elements
				// of the same plane.
				int indexOther = -1;
		...
		// Get rid of duplicates of the elements that have the same norm for the
		...
		if (thisNode->crossroad)
		{
			...
			for (int j = 0; j < numOfPlanesAtCrossroad; ++j)
			{
				    ...
				    m_msh->GetCoordinateFlag() != 22)
				{
					...
					for (int k = 0; k < 3; ++k)
						normOther[k] =
						    m_msh->ele_vector[indexOther]->getTransformTensor(
						        k + 6);
				...
				for (int l = j + 1; l < numOfPlanesAtCrossroad; ++l)
				{
					    ...
					    m_msh->GetCoordinateFlag() != 22)
					{
						...
						for (int k = 0; k < 3; ++k)
							normAnother[k] = m_msh->ele_vector[indexAnother]
							                     ->getTransformTensor(k + 6);
					    ...
					    fabs(normOther[2] - normAnother[2]) < tolerance)
					{
						...
						for (int m = l; m < (numOfPlanesAtCrossroad - 1); ++m)
							thisNode->connected_planes[m] =
							    thisNode->connected_planes[m + 1];

						// Erase the element of the vector and adjust the number
						// of the planes at crossroad
						thisNode->connected_planes.erase(
						    thisNode->connected_planes.begin() +
						    numOfPlanesAtCrossroad - 1);
						numOfPlanesAtCrossroad =
						    (int)thisNode->connected_planes.size();
		...
		double V[3];
		V[0] =
		    m_pcs->GetNodeValue(i, m_pcs->GetNodeValueIndex("VELOCITY1_X") + 1);
		V[1] =
		    m_pcs->GetNodeValue(i, m_pcs->GetNodeValueIndex("VELOCITY1_Y") + 1);
		V[2] =
		    m_pcs->GetNodeValue(i, m_pcs->GetNodeValueIndex("VELOCITY1_Z") + 1);
		if (thisNode->crossroad == false)
		{
			...
			// Store the projected velocity back to the node velocity
			m_pcs->SetNodeValue(i, m_pcs->GetNodeValueIndex("VELOCITY1_X") + 1,
			                    Vp[0]);
			m_pcs->SetNodeValue(i, m_pcs->GetNodeValueIndex("VELOCITY1_Y") + 1,
			                    Vp[1]);
			m_pcs->SetNodeValue(i, m_pcs->GetNodeValueIndex("VELOCITY1_Z") + 1,
			                    Vp[2]);
		}
		else
		{
			// For the velocity of the node that has more than one connected
			// planes.
			double Vmag = 0.0;
			// Mount cross
			CrossRoad* thisCross;
			...
			for (int j = 0; j < thisCross->numOfThePlanes; ++j)
			{
				    ...
				    m_msh->GetCoordinateFlag() != 22)
				{
					...
					for (int k = 0; k < 3; ++k)
						thisCross->plane[j].norm[k] = norm[k] =
						    m_msh->ele_vector[index]->getTransformTensor(k + 6);

				// Store the position vector defined below
				double const* CenterOfEle(
				    m_msh->ele_vector[index]->GetGravityCenter());
				double const* const coords(thisNode->getData());
				thisCross->plane[j].Eele[0] =
				    CenterOfEle[0] - coords[0];  // thisNode->X();
				thisCross->plane[j].Eele[1] =
				    CenterOfEle[1] - coords[1];  // thisNode->Y();
				thisCross->plane[j].Eele[2] =
				    CenterOfEle[2] - coords[2];  // thisNode->Z();
				...
				// For ratio and Vmag
				thisCross->plane[j].ratio =
				    sqrt(Vp[0] * Vp[0] + Vp[1] * Vp[1] + Vp[2] * Vp[2]);
				Vmag += thisCross->plane[j].ratio;
				// Update the eleIndex for this plane
				thisCross->plane[j].eleIndex = thisNode->connected_planes[j];
			}
			// Let's sort the contribution of each plane and the index of this
			// node
			for (int j = 0; j < thisCross->numOfThePlanes; ++j)
			...
			// First, we will solve vectors from each potential crossroad to one
			// of the center
			...
			for (int j = 0; j < thisCross->numOfThePlanes; ++j)
			{
				...
				if (angle > PI / 2.0) thisNode->crossroad = false;
			}
			// Extraction ends here.

			// Now add this crossroad to the vector of all crossroads in the
			// domain
			if (thisNode->crossroad) crossroads.push_back(thisCross);
		}
	}
	...
	for (int i = 0; i < (int)m_msh->edge_vector.size(); ++i)
	{
		...
		V0[0] =
		    m_pcs->GetNodeValue(theNodesOfThisEdge[0]->GetIndex(),
		                        m_pcs->GetNodeValueIndex("VELOCITY1_X") + 1);
		V0[1] =
		    m_pcs->GetNodeValue(theNodesOfThisEdge[0]->GetIndex(),
		                        m_pcs->GetNodeValueIndex("VELOCITY1_Y") + 1);
		V0[2] =
		    m_pcs->GetNodeValue(theNodesOfThisEdge[0]->GetIndex(),
		                        m_pcs->GetNodeValueIndex("VELOCITY1_Z") + 1);
		V1[0] =
		    m_pcs->GetNodeValue(theNodesOfThisEdge[1]->GetIndex(),
		                        m_pcs->GetNodeValueIndex("VELOCITY1_X") + 1);
		V1[1] =
		    m_pcs->GetNodeValue(theNodesOfThisEdge[1]->GetIndex(),
		                        m_pcs->GetNodeValueIndex("VELOCITY1_Y") + 1);
		...
		if (theNodesOfThisEdge[0]->crossroad &&
		    theNodesOfThisEdge[1]->crossroad)
		{
			m_msh->edge_vector[i]->SetJoint(1);

			// Constructing the connected elements of an edge starts here
			int numOfCEfromNode0 =
			    (int)theNodesOfThisEdge[0]->getConnectedElementIDs().size();
			int numOfCEfromNode1 =
			    (int)theNodesOfThisEdge[1]->getConnectedElementIDs().size();

			for (int j = 0; j < numOfCEfromNode0; ++j)
				for (int k = 0; k < numOfCEfromNode1; ++k)
				{
					int indexOfCEfromNode0 =
					    theNodesOfThisEdge[0]->getConnectedElementIDs()[j];
					int indexOfCEfromNode1 =
					    theNodesOfThisEdge[1]->getConnectedElementIDs()[k];
					if (indexOfCEfromNode0 == indexOfCEfromNode1)
						m_msh->edge_vector[i]->connected_elements.push_back(
						    indexOfCEfromNode0);
				}
			// Constructing the connected elements of an edge ends here

			// Construct Joint vectors
			// For the velocity of the edge that has more than one connected
			// planes.
			double Vmag = 0.0;
			CrossRoad* thisJoint;
			thisJoint = new CrossRoad();
			thisJoint->numOfThePlanes =
			    (int)m_msh->edge_vector[i]->connected_elements.size();
			...
			for (int j = 0; j < thisJoint->numOfThePlanes; ++j)
			{
				    ...
				    m_msh->GetCoordinateFlag() != 22)
				{
					...
					for (int k = 0; k < 3; ++k)
						thisJoint->plane[j].norm[k] = norm[k] =
						    m_msh->ele_vector[index]->getTransformTensor(k + 6);
				...
				thisJoint->plane[j].Eele[0] =
				    CenterOfEle[0] - (pnt0[0] + pnt1[0]) / 2.0;
				thisJoint->plane[j].Eele[1] =
				    CenterOfEle[1] - (pnt0[1] + pnt1[1]) / 2.0;
				thisJoint->plane[j].Eele[2] =
				    CenterOfEle[2] - (pnt0[2] + pnt1[2]) / 2.0;
				...
				// For ratio and Vmag
				thisJoint->plane[j].ratio =
				    sqrt(Vp[0] * Vp[0] + Vp[1] * Vp[1] + Vp[2] * Vp[2]);
				Vmag += thisJoint->plane[j].ratio;
				// Update the eleIndex for this plane
				thisJoint->plane[j].eleIndex =
				    m_msh->edge_vector[i]->connected_elements[j];
			}
			// Let's sort the contribution of each plane and the index of this
			// node
			for (int j = 0; j < thisJoint->numOfThePlanes; ++j)
			...
			// First, we will solve vectors from each potential joint to one of
			// the center
			// of the connected elements.
			// Now some dot product.
			for (int j = 0; j < thisJoint->numOfThePlanes; ++j)
			{
				...
				// If this angle is bigger than Pi/2 (90 degree),
				// then this crossroad is not a realone.
				if (angle > PI / 2.0) m_msh->edge_vector[i]->SetJoint(0);
			}
			// Extraction ends here.
...
void CFluidMomentum::SolveForEdgeVelocity(void)
{
	...
	for (int i = 0; i < (int)m_msh->edge_vector.size(); ++i)
	{
		...
		double VectorOfEdge[3] = {coords1[0] - coords0[0],
		                          coords1[1] - coords0[1], 0.0};

		// Note MagOfVector is never zero
		double MagOfVector = sqrt(VectorOfEdge[0] * VectorOfEdge[0] +
		                          VectorOfEdge[1] * VectorOfEdge[1]);
		...
		MagOfVector = sqrt(VectorOfEdge[0] * VectorOfEdge[0] +
		                   VectorOfEdge[1] * VectorOfEdge[1]);
		...
		{
			// For Ve0 first
			cos = (VectorOfEdge[0] * V0[0] + VectorOfEdge[1] * V0[1]) /
			      (MagOfVector * MagOfV0);
		...
		{
			// For Ve1 second
			cos = (VectorOfEdge[0] * V1[0] + VectorOfEdge[1] * V1[1]) /
			      (MagOfVector * MagOfV1);
			sin = sqrt(1.0 - cos * cos);
		...
		// Checking the angle between node averaged velocity and flux averaged
		// velocity along the edge
		double angle =
		    acos((V[0] * Vc[0] + V[1] * Vc[1]) /
...
void DATWriteHETFile(const char* file_name)
{
	...
	for (int i = 0; i < (long)m_msh->ele_vector.size(); i++)
	{
		...
		fprintf(tet_file, "%17.12e %17.12e %17.12e %17.12e\n", center[0],
		        center[1], center[2], elem->mat_vector(0) * 1e7);
	}
