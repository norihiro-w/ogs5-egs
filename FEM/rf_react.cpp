...
void REACT::ExecuteReactionsPHREEQC(void)
{
	...
	/*
	   ...
	   this->rateflag[i],0,0); DisplayMsg(", ");}
	   DisplayMsgLn(" ");
	 */
...
void REACT::CreateREACT(void)
{
	...
	for (size_t i = 0; i < vector_size; i++)
	{
		...
		if (pcs_type == FiniteElement::MASS_TRANSPORT) np++;
		//		if (m_pcs->pcs_type_name.compare("HEAT_TRANSPORT") == 0)
		if (pcs_type == FiniteElement::HEAT_TRANSPORT) heatflag = 1;
...
void CRFProcess::InterpolateTempGP(CRFProcess* m_pcs, std::string name)
{
	...
	for (i = 0; i < (long)m_pcs->m_msh->ele_vector.size(); i++)
	{
		...
		for (j = 0; j < elem->GetVertexNumber(); j++)
		{
			...
			// m_pcs_mmp
			Node_T[j] =
			    m_pcs->GetNodeValue(enode, m_pcs->GetNodeValueIndex(name) + 1);
		}
...
REACT* REACT::GetREACT(void)
{
	REACT* rc = NULL;
	/* Tests */
	if (REACT_vec.capacity() > 0) rc = REACT_vec[0];
	return rc;
}
...
int REACT::ReadReactionModelNew(ifstream* pqc_infile)
{
	...
	CRFProcess* m_pcs = NULL;
	std::stringstream in;
	// WW int pH_found = 0;                              // Hplus_found = 0,
	// count = -1;
	...
	if ((this->gamma_Hplus > 0))
		cout << " pH found and H+ found, pH for PHREEQC input is calculated "
		        "using gamma_H+ and [H+] " << gamma_Hplus << endl;
	...
	cout << rcml_number_of_equi_phases << " equilibrium phases,  "
	     << rcml_number_of_ion_exchanges << " ion exchangers and ";
						...
						for (j = 0; j < np; j++)
						{
							if (strcmp(s, name[j]) == 0)
							{
								if (strcmp("pH", name[j]) != 0)
								{
									...
									FilePrintInt(f, j + 1);
									LineFeed(f);
									if (strcmp("pe", name[j]) == 0) pe_flag = 0;
									break;
								}
								/*		                        else{  // pH in
								   RF
							               ...
							               with the reaction! */
							{
								...
								if (pH_flag < 0)
								{
									...
									pH_flag = 1;
									p = 0;
									StrReadDouble(&dvalue, & str[p += pos], f,
									              &pos);
									StrReadStr(s,
									           & str[p += pos],
									           f,
			...
			else if (!strcmp(sub, "EQUILIBRIUM_PHASES"))
			{
				...
				if (nep != rcml_number_of_equi_phases)
					FilePrintString(f,
					                ...
					                "EQUILIBRIUM_PHASES not found in *.rfd "
					                "file!!!");
					...
					if (strstr(str, "-steps"))
					{
						...
						if (index > -1)
						{
							...
							if (time_vector.size() > 0)
								...
								cout << "Error in "
								        "MPCCalcCharacteristicNumbers: no time "
								        "discretization data !" << endl;
                             ...
                             /*ifstream *pqc_iinfile,*/ ofstream* out_file)
{
	...
	while (!pqc_infile.eof())
	{
			...
			while (line_string.find("#ende") == string::npos)
			{
				...
				else if (line_string.find("# temp") != string::npos)
				{
					...
					if (this->rcml_heat_flag > 0)
					{
						...
						dval = m_pcs->GetNodeValue(index, idx);
						if (dval < 273.0)
							dval += 273.15;  // change from °C to Kelvin if
						                     // necessary
		...
		{
			*out_file << endl
			          << "EQUILIBRIUM_PHASES   " << index + 1 << endl;
			while (line_string.find("#ende") == string::npos)
			{
				pqc_infile.getline(line, MAX_ZEILE);
				line_string = line;
				if (line_string.find("# comp") != string::npos)
				{
					...
					//                pcs_vector[pqc_process[count]]->pcs_number
					//                << ", " << pqc_index[count];
					//                if(index <2) cout << " EQ-Species " <<
					//                speciesname << " " << dval << endl;
			...
			*out_file << endl
			...
			while (line_string.find("#ende") == string::npos)
			{
				...
				if (line_string.find("# comp") != string::npos)
				{
					...
					if (dval < 1.0e-19) dval = 0.0;
					/*                cout << "Testing index vectors: " <<
					   speciesname << ", " << m_pcs->pcs_number << ", "<< idx
					   <<",  With vectors: " << pqc_names[count] << ", " <<
					   pcs_vector[pqc_process[count]]->pcs_number << ", " <<
					   pqc_index[count];
					                cout << " Values: " << dval << ", " << dval1
					   << endl;
					 */
					*out_file << speciesname << "       " << dval
					          << "       # comp " << endl;
			...
			*out_file << endl
			...
			while (line_string.find("#ende") == string::npos)
			{
				...
				else if (line_string.find("# comp") != string::npos)
				{
					...
					// pcs_vector[pqc_process[count]]->pcs_number << ", " <<
					// pqc_index[count];
					// cout << " Molar mass: " << dval  << ", component / total
					// mass: " << dval/mm << endl;
					// dval is molar mass of gas component, convert to partial
					// pressure [Pa]
					if (mm > 0.0)
					...
					// partial/total press: " << (partial_press/press) << endl;
		...
		if (line_string.find("-steps") != string::npos)  // keyword found
		{
			...
			*out_file << "-steps " << dval << " in "
			          << this->rcml_number_of_pqcsteps << " steps" << endl;
		}  // end -steps
		//-------------------------------------------------------------------------------------------------------------
	...
	for (i = 0; i < this->nodenumber; i++)
	{
		if (this->rateflag[i] > 0)
		{
			...
			for (j = 0; j < rcml_number_of_master_species + 2; j++)
				if (ein >> dval) this->val_out[j][i] = dval;
...
void REACT::TestPHREEQC(string file_base_name)
{
	...
	if (pqc_file.good()) flag_pqc = true;
	pqc_file.close();
}
...
void REACT::SetNeighborNodesActive(long startnode, long level, int* help)
{
	...
	/*OK411
	   ...
	   liefert zu viele Elemente zurück
...
void REACT::ExecuteReactionsPHREEQC0(void)
{
	...
	if (aktuelle_zeit > 0) GetTransportResults2Element();
...
int REACT::CheckNoReactionNodes(void)
{
	...
	if (aktueller_zeitschritt < 2)  // do not in the very first calculation
		                            // before first time step and in the first
		                            // time step
	...
	else
	{
		...
		if (KinReactData_vector.size() > 0) m_krd = KinReactData_vector[0];
		if (m_krd == NULL)
		         ...
		         0)  // reaction nodes specified in krc input file
		{
			...
			if (m_msh == NULL)
			{
				cout << "No mesh in CheckNoReactionNodes" << endl;
				exit(1);
			}
...
// TODO: don't try to run phreeqc if output is not ok
	...
	cout << "MDL_DEBUG: final input is " << nline << " lines, nodes " << ii
	     << "/" << this->nodenumber << endl;
	cout << endl
	     << "MDL_DEBUG: ****** Current input:" << endl
	     << out_buff.str() << endl
	     << "MDL_DEBUG: ****** end of input" << endl;
#endif
...
int REACT::ReadOutputPhreeqcNewLib(double* pqc_vec)
{
	...
	for (index = 0; index < this->nodenumber; index++)
	{
		if (this->rateflag[index] > 0)
		{
			...
			// pH
			j = n1;
			pcs_vector[pqc_process[j]]->SetNodeValue(index, pqc_index[j] + dix,
			                                         pqc_vec[index * ntot + j]);
...
int REACT::Call_PhreeqcLib(int nsim, int npunch, int nline,
                           stringstream* pqc_buffer, double* pqc_out)
{
	int i, out;
	string line;
	size_t length;
	...
	cout << "MDL_DEB:: Call_PhreeqcLib ->  nsim = " << nsim
	     << "; npunch = " << npunch << "; nline = " << nline << endl;
#endif
