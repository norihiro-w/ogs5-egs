	...
	while (!new_keyword)
	{
		...
		if (line_string.find("$SWELLING_PRESSURE_TYPE") != string::npos)
		{
			...
			else if (SwellingPressureType == 3 || SwellingPressureType == 4)
			{
				...
				{
					std::cout
					    << "No multi-phase flow coupled. The thermal elatic "
					       "model can only be used in H2 coupled proccess."
					    << "\n";
		...
		if (line_string.compare("CONDUCTIVITY") == 0)
		{
			...
			switch (Conductivity_mode)
			{
				case 0:  //  = f(T) //21.12.2009 WW
					...
					{
						in_sd.str(GetLineFromFile1(msp_file));
						in_sd >> (*data_Conductivity)(i, 0) >>
						    (*data_Conductivity)(i, 1);
						in_sd.clear();
					}
					// WW
		...
		//....................................................................
		if (line_string.find("CONDUCTIVITY_TENSOR") !=
		    string::npos)  // subkeyword found
		{
			...
			switch (thermal_conductivity_tensor_type_name[0])
			{
				case 'I':  // isotropic
					...
					thermal_conductivity_tensor[1] =
					    thermal_conductivity_tensor[2] =
					        thermal_conductivity_tensor[0];
					break;
				case 'O':  // orthotropic
					thermal_conductivity_tensor_type = 1;
					in_sd >> thermal_conductivity_tensor_dim;
					if (thermal_conductivity_tensor_dim == 0)
						std::cout << "Error in CSolidProperties::Read: no "
						             "tensor dimension"
						          << "\n";
				...
				case 'A':  // anisotropic
					...
					break;
				default:
					cout << "Error in CSolidProperties::Read: no valid thermal "
					        "conductivity tensor type" << endl;
					break;
			}
			in_sd.clear();
		}
		...
		{  // subkeyword found
			...
			switch (type)  // 15.03.2008 WW
			{
				...
				case 3:
					...
					in_sd.clear();
					break;
				case 1000:  // case 10-13: transverse isotropic linear
					...
					//  1: E_a     (Young's modulus w.r.t. the anisotropy
					//  direction)
					//  2: nu_{ia} (Poisson's ratio w.r.t. the anisotropy
					//  direction)
					//  3: G_a     (shear modulus w.r.t. the anisotropy
					//  direction)
					//  4: n_x     (x-coefficient of the local axis of
					//  anisotropy (2D case: -\sin\phi))
					//  5: n_y     (y-coefficient of the local axis of
					//  anisotropy (2D case: \cos\phi))
					//  6: n_z     (z-coefficient of the local axis of
					//  anisotropy (2D case: 0))
		...
		if (line_string.find("$CREEP") != string::npos)
		{
			if (line_string.find("NORTON") != string::npos)
			{
				...
				/*! \f$\dot\epsilon_s=A
				 * \left(\frac{\sigma_v}{\sigma^\ast}\right)^n\f$ */
				// data_Creep:
				//  0: A,   coefficient
				//  1: n,   exponential
		...
		//....................................................................
		if (line_string.find("$SOLID_BULK_MODULUS") !=
		    string::npos)  // WX: 04.2013
		{
			in_sd.str(GetLineFromFile1(msp_file));
			in_sd >> Ks;
			in_sd.clear();
		...
		if (line_string.find("$PLASTICITY") != string::npos)
		{
			...
			if (sub_line.find("DRUCKER-PRAGER") != string::npos)
			{
				...
				{
					Plasticity_type = 10;
					if (sub_line.find("TENSIONCUTOFF") !=
					    string::npos)  // WX: 08.2010
					{
						Plasticity_type = 11;
						dFtds = new double[6];  // WX: 08.2010 Tensile yield
						                        // function
						dGtds = new double[6];
			...
			data_Plasticity = new Matrix(
			    Size + 1);  // TODO NW added +1 because WX added one for DP
			for (i = 0; i < Size; i++)
			{
				in_sd.str(GetLineFromFile1(msp_file));
...
CSolidProperties::~CSolidProperties()
{
	...
	if (data_Youngs) delete data_Youngs;
...
/**************************************************************************
   FEMLib-Method: CSolidProperties::Heat_Capacity(const double refence = 0.0)
const
 ...
 **************************************************************************/
bool CSolidProperties::CheckTemperature_in_PhaseChange(const double T0,
                                                       const double T1)
{
	bool stat = false;
...
/*************************************************************************
   ...
   09/2005 WW Implementation
 **************************************************************************/
double CSolidProperties::Heat_Capacity(double temperature, double porosity,
                                       double Sat)
{
	double val = 0.0;
	double sign = 1;
	         ...
	         (temperature < ((*data_Capacity)(2) + (*data_Capacity)(3))))
	{
		if (sign > 0.0)  // Temperature increase
			val = dens * (*data_Capacity)(0) +
			      dens*((*data_Capacity)(1) - (*data_Capacity)(0)) *
			          (temperature - (*data_Capacity)(2)) /
			          (*data_Capacity)(3) +
			      porosity* Sat * m_mfp->Density() * (*data_Capacity)(4) /
			          (*data_Capacity)(3);
		else
			val = dens * (*data_Capacity)(0) +
			      dens*((*data_Capacity)(1) - (*data_Capacity)(0)) *
...
/**************************************************************************
   ...
   09/2005 WW Implementation
**************************************************************************/
double CSolidProperties::Enthalpy(double temperature,
                                  const double latent_factor)
{
	         ...
	         (temperature < ((*data_Capacity)(2) + (*data_Capacity)(3))))
	{
		...
		val += dT * latent_factor * (*data_Capacity)(4) / (*data_Capacity)(3) /
		       Density();
		val += (*data_Capacity)(0) * dT +
		       0.5 * dT* dT*((*data_Capacity)(1) - (*data_Capacity)(0)) /
		           (*data_Capacity)(3);
	}
	else
	{
		...
		val += (*data_Capacity)(0) * dT +
		       0.5 * dT* dT*((*data_Capacity)(1) - (*data_Capacity)(0)) /
		           (*data_Capacity)(3);
		val += (*data_Capacity)(1) *
		       (temperature - (*data_Capacity)(3) - (*data_Capacity)(2) - T0);
	}
		...
		case 2:
			...
			if (refence < (*data_Conductivity)(2))  // Wet
				val = (*data_Conductivity)(0);
			         ...
			         (refence <
			          ((*data_Conductivity)(2) + (*data_Conductivity)(3))))
				val = (*data_Conductivity)(0) +
				      ((*data_Conductivity)(0) - (*data_Conductivity)(1)) *
				          (refence - (*data_Conductivity)(2)) /
				          (*data_Conductivity)(3);
			else
                                              ...
                                              int /*group*/, double* variables)
{
	...
	// Test for DECOVALEX
	// thermal_conductivity_tensor[0]
	// =0.5+0.8*PCSGetELEValue(number,NULL,theta,"SATURATION1");

	//--------------------------------------------------------------------
	// There are a number of cases where the heat conductivity tensor is defined
	...
	switch (Conductivity_mode)
	{
		...
		default:  // Normal case
			cout << "***Error in CSolidProperties::HeatConductivityTensor(): "
			        "conductivity mode is not supported " << endl;
	...
	switch (thermal_conductivity_tensor_type)
	{
		...
		case 1:  // Ortho
			...
			break;
		case 2:  // Aniso
			for (i = 0; i < thermal_conductivity_tensor_dim *
			                    thermal_conductivity_tensor_dim;
			     i++)
...
/**************************************************************************
   FEMLib-Method: CSolidProperties::Youngs_Modulus(const double refence = 0.0)
const
   Task: Get density
   Programing:
   08/2004 WW Implementation
...
/*************************************************************************
   ROCKFLOW - Funktion:
CSolidProperties::CalculateTransformMatrixFromNormalVector

   Aufgabe:
   Generate rotation matrices for vectors and matrices based on a given normal
vector
    ...
    const int Dimension)
{
	...
	Crotm = new Matrix(
	    Dimension * 2,
	    Dimension * 2);  // rotation matrix for matrices: UJG 25.11.2009
	...
	if (Dimension == 3)
	{
		...
		(*Crotm)(3, 3) =
		    (*Crotv)(0, 0) * (*Crotv)(1, 1) + (*Crotv)(1, 0) * (*Crotv)(0, 1);
		(*Crotm)(3, 4) =
		    (*Crotv)(0, 0) * (*Crotv)(1, 2) + (*Crotv)(0, 2) * (*Crotv)(1, 0);
		(*Crotm)(3, 5) =
		    (*Crotv)(0, 1) * (*Crotv)(1, 2) + (*Crotv)(0, 2) * (*Crotv)(1, 1);

		(*Crotm)(4, 0) = 2.0 * (*Crotv)(0, 0) * (*Crotv)(2, 0);
		(*Crotm)(4, 1) = 2.0 * (*Crotv)(0, 1) * (*Crotv)(2, 1);
		(*Crotm)(4, 2) = 2.0 * (*Crotv)(0, 2) * (*Crotv)(2, 2);
		(*Crotm)(4, 3) =
		    (*Crotv)(0, 0) * (*Crotv)(2, 1) + (*Crotv)(0, 1) * (*Crotv)(2, 0);
		(*Crotm)(4, 4) =
		    (*Crotv)(0, 0) * (*Crotv)(2, 2) + (*Crotv)(2, 0) * (*Crotv)(0, 2);
		(*Crotm)(4, 5) =
		    (*Crotv)(0, 1) * (*Crotv)(2, 2) + (*Crotv)(0, 2) * (*Crotv)(2, 1);

		(*Crotm)(5, 0) = 2.0 * (*Crotv)(1, 0) * (*Crotv)(2, 0);
		(*Crotm)(5, 1) = 2.0 * (*Crotv)(1, 1) * (*Crotv)(2, 1);
		(*Crotm)(5, 2) = 2.0 * (*Crotv)(1, 2) * (*Crotv)(2, 2);
		(*Crotm)(5, 3) =
		    (*Crotv)(1, 0) * (*Crotv)(2, 1) + (*Crotv)(1, 1) * (*Crotv)(2, 0);
		(*Crotm)(5, 4) =
		    (*Crotv)(1, 0) * (*Crotv)(2, 2) + (*Crotv)(1, 2) * (*Crotv)(2, 0);
		(*Crotm)(5, 5) =
		    (*Crotv)(1, 1) * (*Crotv)(2, 2) + (*Crotv)(2, 1) * (*Crotv)(1, 2);
	}
	else
	{
		if (ny > 1.0)
...
void CSolidProperties::CalculateCoefficent_MOHR(double ep)  // WX:11.2010
{
	...
	if ((*data_Plasticity)(5) > 0 && (*data_Plasticity)(5) < 100)
		theta =
		    GetCurveValue((int)(*data_Plasticity)(5), 0, ep, &valid) * PI / 180;
	...
	if (tension < 0 || tension > Y0 / tan(theta)) tension = Y0 / tan(theta);
                                           ...
                                           const int Update)
{
	...
	if (F > 0.0 && (!PreLoad))  // in yield status
	{
		...
		err_corner = 4.5 * Xi * K * normXi / G +
		             BetaN *
		                 (0.5 * Hard * sqrt(1.0 + 3.0 * Xi * Xi) * normXi / G +
		                  Y0 + Hard * ep) /
		                 Al;
		// Multi-surface
		if (p3 > err_corner)
		{
			...
			while (isLoop)
			{
				...
				Jac = 9.0 * Xi * K +
				      3.0 * Xi * Xi * BetaN * Hard * (dPhi + dl2) / (fac * Al);
				F = 9.0 * Xi * K * (dPhi + dl2) +
	...
	if (Update > 0)
	{
		if (dPhi > 0.0) (*ele_val->pStrain)(GPiGPj) = ep;
                                                 ...
                                                 const int Update)
{
	...
	if (F > 0.0 && (!PreLoad))  // in yield status
	{
		...
		while (m > 0)
		{
			// Compute dlamda
			A_H = BetaN * Hard *
			      sqrt(1 + 3.0 * Xi * Xi);  // Hard: if it is not constant....
			for (i = 0; i < Size; i++)
				devS[i] = TryStress[i];
			I1 = DeviatoricStress(devS);
			sqrtJ2 = sqrt(TensorMutiplication2(devS, devS, Dim));
...
// WX
int CSolidProperties::DirectStressIntegrationDPwithTension(
    const int GPiGPj,
    Matrix* De,
    const ElementValue_DM* ele_val,
    double* TryStress,
    ...
    double& mm)
{
	...
	double I1 = 0.0;
	// WW double sy0,
	// WW double sy, ep, dlambda=0.0, tmpvalue=0., tmpvalue2=0., sqrtJ2I1;
	// //dTaun, dSign,
	...
	double dstrs[6];
	double dTempStr[6] = {0.}, dTempStr2[6] = {0.}, TmpStress0[6] = {0},
	       dStrainP[6] = {0.};
	...
	if (tension > (yy / Al / 3.0))
		tension =
		    yy / Al /
		    3.0;  // WX: tension strength must be positiv and has a max limit.
	...
	// WW  H = sqrtJ2/sqrt(2.0) - Tau_P - (Al_P * (I1/3. - tension));
	// //WX:16.09.2010.correct H.
	...
	if (F > 0.0 && (!PreLoad))
	{
		...
		else
		{
			...
			for (i = 0; i < Size; i++)
				TryStress[i] =
				    (*ele_val->Stress)(i, GPiGPj) + dstrs[i] - dTempStr[i];
			...
			if (sqrtJ2 <= sqrtJ2I1)
			{
				if ((dstrs[0] + dstrs[1] + dstrs[2]) != 0.0)
					mm = (3 * tension - ((*ele_val->Stress)(0, GPiGPj) +
			...
			else
			{
				...
				{
					if (i < 3)
						tmpStr[i] = (TryStress[i] + I1 / 3.0) -
						            tmpvalue * (dTempStr[i] + dI1 / 3.0);
					else
				...
				if (Update < 1)  // calculate mm for Dep
				{
					...
					for (i = 0; i < Size; i++)  // initialize stress devstress
					{
						...
						dTempStr[i] = dstrs[i];
						dTempStr2[i] =
						    TmpStress0[i] + dstrs[i];  // stress0+dstrs
						                               //  cout<<devS[i]<<endl;
					}
					...
					for (i = 0; i < Size; i++)
						for (j = 0; j < Size; j++)
							(*ConstitutiveMatrix)(i, j) =
							    (*De)(i, j) -
							    (1 - m2) * m1 * (*Dp_shear)(i, j) -
							    (1 - n2) * n1 * (*Dp_tension)(i, j);
	...
	if (failurestate != 0)
	{
		for (i = 0; i < Size; i++)
		{
			dTempStr[i] = (*ele_val->Stress)(i, GPiGPj) + dstrs[i] -
			              TryStress[i];  // d plas stress
			dStrainP[i] = 0.;
		}
		...
		ep += sqrt(TensorMutiplication2(dStrainP, dStrainP, Dim)) *
		      sqrt(2.0 / 3.0);
		delete invDe;
	}
	// Save the current stresses
...
void CSolidProperties::TangentialDPwithTension(Matrix* Dep, double mm)
{
	...
	for (i = 0; i < Size; i++)
		for (j = 0; j < Size; j++)
			(*Dep)(i, j) =
			    mm * (*Dep)(i, j) + (1 - mm) * ((*Dep)(i, j) - (*D_temp)(i, j));
	delete D_temp;
}
...
// WX: Mohr Coulomb
int CSolidProperties::DirectStressIntegrationMOHR(
    ...
    const int Update, Matrix* Dep)
{
	...
	*TmpDe = *Dep;
	...
	if (Size == 4) TmpStress0[4] = TmpStress[5] = 0.;

	// CalPrinStrs(TmpStress0, tmp_prin_str, Size);
	CalPrinStrDir(TmpStress0, tmp_prin_str, tmp_prin_dir,
	...
	{
		// WW double P_12, P_31, P_41, P_63, P_64, P_52, P_85, P_74, P_78, P_98,
		// P_45, P_X7;
		...
		if (P_12 >= 0 && P_31 <= 0 && P_41 <= 0)  // return to fmc
		{
			...
			for (i = 0; i < 3; i++)
				for (j = 0; j < 3; j++)
					(*dGds_dFds)(i, j) = rsp[i] * dFsdprin_s[j];
			//(D*dGds/(dFds*D*dGds))*dFds
	...
	if (Update > 0) (*ele_val->pStrain)(GPiGPj) = ep;
...
double CSolidProperties::CalVarP(double* vec1, double* vec2, double* sigma_B,
                                 double* sigma_l)
{
...
double CSolidProperties::CalVar_t(double* vecl,
                                  double* veclg,
                                  Matrix* D,
                                  double* sigma_B,
                                  double* sigma_l,
...
void CSolidProperties::VecCrossProduct(double* vec1, double* vec2,
                                       double* result_vec)
{
...
void CSolidProperties::CalDep_l(double* vecl, double* veclg, Matrix* D,
                                Matrix* Dep_l, double /*fkt*/)
{
	double TmpVec[6] = {0.};
	double TmpVal = 0.;
...
**************************************************************************/
void CSolidProperties::ConsistentTangentialDP(Matrix* Dep, const double dPhi,
                                              const int Dim)
{
	...
	else
	{
		...
		switch (Dim)
		{
			case 2:  // 2D
				...
				if (axisymmetry)
				{
					(*Dep)(0, 2) = -c1 / 3.0 + c2 + c3 * (Xi * s33 + Al * s11) +
					               c4 * s11 * s33;
					// Row 2
					(*Dep)(1, 2) = -c1 / 3.0 + c2 + c3 * (Xi * s33 + Al * s22) +
					               c4 * s33 * s22;
					// Row 3
					(*Dep)(2, 0) = -c1 / 3.0 + c2 + c3 * (Xi * s11 + Al * s33) +
					               c4 * s11 * s33;
					(*Dep)(2, 1) = -c1 / 3.0 + c2 + c3 * (Xi * s22 + Al * s33) +
					               c4 * s22 * s33;
					(*Dep)(2, 2) = 2.0 * c1 / 3.0 + c2 + c3 * (Al + Xi) * s33 +
					               c4 * s33 * s33;
...
int CSolidProperties::CalStress_and_TangentialMatrix_SYS(
    const int GPiGPj, const ElementValue_DM* ele_val, const Matrix* De,
    Matrix* D_ep, double* dStress, const int Update)
{
	...
	if (Stress_Inv[1] > 0.0)
	{
		...
		PHI = Stress_Inv[1] * pow(1 + Mat_n[5] * ang, Mat_n[6]) +
		      0.5 * Mat_n[0] * I_p2 + Mat_n[2] * Mat_n[2] * I_p4;
	...
	if (F > TolF && !PreLoad) /* In Yield Status */
	{
		...
		Co = sqrt(2.0 * Stress_Inv[1] *
		          TensorMutiplication2(Stress_n, Stress_n, dim));
		if (Co > 0.0)
			preSub =
			    1 +
			    (int)(acos(TensorMutiplication2(Stress_n, nStress, dim) / Co) /
			          den);
		else
			preSub = minSub;
		...
		for (iSub = 0; iSub < preSub; iSub++)
		{
			...
			Stress_Inv[1] = 0.5 * TensorMutiplication2(nStress, nStress, dim);
			Stress_Inv[2] =
			    TensorMutiplication3(nStress, nStress, nStress, dim);

			I_p2 = I_n1 * I_n1;
			I_p4 = I_p2 * I_p2;
			ang = Stress_Inv[2] / MathLib::fastpow(sqrt(Stress_Inv[1]), 3);
			PHI = sqrt(Stress_Inv[1] * pow(1 + Mat_n1[5] * ang, Mat_n1[6]) +
			           0.5 * Mat_n1[0] * I_p2 + Mat_n1[2] * Mat_n1[2] * I_p4);
			// The yield function
		...
		for (iSub = elasticSub; iSub < preSub; iSub++)
		{
			...
			while (ErrLoc > Tolerance_Local_Newton)
			{
				...
				Stress_Inv[0] = I_n1;
				Stress_Inv[1] =
				    0.5 * TensorMutiplication2(nStress, nStress, dim);
				Stress_Inv[2] =
				    TensorMutiplication3(nStress, nStress, nStress, dim);
				...
				ang = Stress_Inv[2] / MathLib::fastpow(sqrt(Stress_Inv[1]), 3);
				PHI =
				    sqrt(Stress_Inv[1] * pow(1 + Mat_n1[5] * ang, Mat_n1[6]) +
				         0.5 * Mat_n1[0] * I_p2 + Mat_n1[2] * Mat_n1[2] * I_p4);
				PSI = sqrt(Stress_Inv[1] / psi1 + 0.5 * Mat_n1[0] * I_p2 +
				...
				else
				{
					...
					dG__dNStress_dNStress(nStress, Stress_Inv, Mat_n1,
					                      LengthStrs);
						...
						for (j = 0; j < LengthStrs; j++)
						{
							...
							if (br > 0.0)
							{
								if (j == 2)  // xi_33=-(xi_11+xi_22+x_33)
								{
									...
									(*LocalJacobi)(l, LengthStrs + j - 1) +=
									    -dlmd * t1 * I_n1 / 3.0;
							}

							// dG_dStress_dMat
							if ((Ch > 0.0) && (Cd > 0.0))
								(*LocalJacobi)(l, 2 * LengthStrs - 1 + j) +=
								    dlmd * t2;
						}
					...
					for (i = 0; i < LengthMat; i++)
					{
						...
						for (j = 0; j < LengthStrs; j++)
						{
							...
							for (k = 0; k < LengthStrs; k++)
							{
								...
								if (k > 2) c2 = 2.0;
								t1 +=
								    c1 * c2 * Stress_n1[k] * (*d2G_dSdS)(k, j);
							}
					...
					dG__dStress_dStress(nStress, xi_n1, Stress_Inv, Mat_n1,
					                    LengthStrs);
					for (i = 0; i < LengthStrs; i++)
					{
						l = i;
					...
					for (i = 0; i < LengthStrs; i++)
					{
						...
						// d(r_xi)/dLambda
						(*LocalJacobi)(l, LocDim - 1) -=
						    br * t1 * (I_n1 * xi_n1[i] - mr * nStress[i]) /
						    (PSI * psi1 * I_n1);
					}
				...
				{
					printf(
					    "\n ~O~ ~O~  Local Newton-Raphson has problem in "
					    "convergence in MatCalcStressWM().. \n");
					printf("\n F = %g\n", F);
					break;  // abort();
				}
			...
			if (subPLASTIC > 0)
				ep += dlmd *
				      sqrt(2.0 * TensorMutiplication2(dG_dS, dG_dS, dim) / 3.0);
...
/**************************************************************************
   ...
   08/2004   WW  Set as a member of CSolidProperties
 ***************************************************************************/
void CSolidProperties::dF_dNStress(double* dFdS,
                                   const double* DevS,
                                   ...
                                   const int LengthStrs)
{
	...
	for (i = 0; i < LengthStrs; i++)
	{
		dFdS[i] *= MatN1[5] * MatN1[6] * S_Invariants[1] *
		           pow(1 + MatN1[5] * ang, MatN1[6] - 1.0);
		dFdS[i] += pow(1 + MatN1[5] * ang, MatN1[6]) * DevS[i];
		dFdS[i] /= (2 * PHI);
		if (i < 3)
		{
			dFdS[i] +=
			    (MatN1[0] * In1 + 4.0 * MatN1[2] * MatN1[2] * I_p3) / (2 * PHI);
...
/**************************************************************************
   ...
   08/2004   WW  Set as a member of CSolidProperties
 ***************************************************************************/
void CSolidProperties::dF_dStress(double* dFdS,
                                  const double* RotV,
                                  const double* S_Invariants,
                                  const double* MatN1,
                                  const int LengthStrs)
...
/**************************************************************************
   ...
   08/2004   WW  Set as a member of CSolidProperties
 ***************************************************************************/
void CSolidProperties::dF_dMat(double* dFdM, const double* S_Invariants,
                               const double* MatN1)
{
	double In1 = S_Invariants[0];
	double I_p2 = In1 * In1;
	...
	dFdM[4] = -1.0;
	/*dF_dGamma*/
	dFdM[5] = 0.5 * S_Invariants[1] * MatN1[6] * ang *
	          pow(1.0 + MatN1[5] * ang, MatN1[6] - 1.0) / PHI;
	/*dF_dM*/
...
/**************************************************************************
   ...
   08/2004   WW  Set as a member of CSolidProperties
 ***************************************************************************/
void CSolidProperties::dG_dNStress(double* dGdS,
                                   const double* DevS,
                                   const double* S_Invariants,
                                   const double* MatN1,
                                   const int LengthStrs)
...
/**************************************************************************
   ...
   Formalparameter: (E: Eingabe; R: Rueckgabe; X: Beides)
   E :
       double *dG_dSdS            : The second derivatives of the plastic
 potential
   with respect to stresses
   const long EleIndex        : Element index
   const double *DevS         : The deviatoric stresses
...
/**************************************************************************
 ...
 potential
   ...
   08/2004   WW  Set as a member of CSolidProperties
 ***************************************************************************/
void CSolidProperties::dG__dStress_dStress(const double* DevS,
                                           const double* RotV,
                                           const double* S_Invariants,
                                           const double* MatN1,
                                           const int LengthStrs)
...
/**************************************************************************
   ...
   Formalparameter: (E: Eingabe; R: Rueckgabe; X: Beides)
   E :
       double *dG_dSdM            : The second derivatives of the plastic
 potential
...
/**************************************************************************
   ...
   08/2004   WW  Set as a member of CSolidProperties
 ***************************************************************************/
void CSolidProperties::dfun2(const double* DevS, const double* RotV,
                             ...
                             const int LengthStrs)
{
	...
	for (i = 0; i < LengthStrs; i++)
	{
		...
		for (j = 0; j < LengthStrs; j++)
		{
			...
			(*d2G_dSdS)(l, j) /= In1;
			(*d2G_dSdS)(l, j) -= 0.5 * (In1 * RotV[i] - mr * DevS[i]) *
			                     Kronecker(kk, ll) / (PSI * In1 * In1);

			if (j < 4)
				(*d2G_dSdM)(l, j) = -In1 * (*d2G_dSdS)(i, j) / 3.0 -
				                    var* In1* Kronecker(ii, kk) *
				                        Kronecker(jj, ll) / (PSI * In1);
...
/**************************************************************************
   ...
   const int Update                    :   Indicator. If true, update the gauss
point
   values
    ...
    const int Update)
{
	...
	p = -((*ele_val->Stress)(0, GPiGPj) + (*ele_val->Stress)(1, GPiGPj) +
	      (*ele_val->Stress)(2, GPiGPj)) /
	    3.0;

	if (fabs(p) < pmin) p = pmin;
	...
	if (F > 0.0 && !PreLoad)  // in yield status
	{
		...
		while (isLoop)  // Newton step for the plastic multiplier
		{
			NPStep++;
			if (NPStep > MaxI)
				// printf("\n Too much iteration in Newton step in the
				// integration of Cam-Clay \n");
				break;
			// abort();
			dfdp = 2.0 * p - p_c;
			dfdq = 2.0 * q / M2;
			...
			J22 =
			    gamma2 * vartheta * vep * (2.0 * dev * K / gamma1 - 1.0) - 1.0;
		...
		while (isLoop)  // Newton step for the plastic multiplier
		{
			...
			while (isLoop)  // Damp
			{
				...
				while (isLoop)  // Newton step for p_c
				{
					NPStep++;
					if (NPStep > MaxI)
						//    printf("\n Too much iteration in Newton step the
						//    integration of Cam-Clay \n");
						// TEST	abort();
						// test
						break;
		...
		while (1)
		{
			...
			{
				printf(
				    "\n Too much iteration in Newton step in the integration "
				    "of Cam-Clay \n");
				abort();
			}
		...
		if (Update < 1)
		{
			...
			alpha1 =
			    -4.0 * G * q * a6 / M2 - K * ((2.0 * a2 - a4) * p - a2 * p_c);
			...
			alpha3 = -q_tr * (1.0 + 0.5 * vartheta * p_c * var1 / K) /
			         (p_tr - 0.5 * p_c);
...
/**************************************************************************
   ...
   E :
     ...
     double *Dep                         :   The consist tangential matrix
     const int GPiGPj                    :   Local indeces of Gauss Points
     const int Update                    :   Indicator. If true, update the
gauss point
                                             values
...
**************************************************************************/
void CSolidProperties::CalStress_and_TangentialMatrix_CC_SubStep(
    const int GPiGPj,
    const ElementValue_DM* ele_val,
    double* dStrain,
    Matrix* Dep,
    const int Update)
{
	...
	for (;;)  // WW while(OK)
	{
		...
		for (int kk = 0; kk < 2; kk++)
		{
			...
			// else
			//    K =
			//    (1.0+e_0)*fabs(p)*(exp(PoissonRatio*dev/SwellIndex)-1.0)/dev;
		...
		if (norms > DBL_EPSILON)
		{
			...
			if (R_n > DBL_EPSILON) q_f = 0.95 * sqrt(s_tol / R_n);
		}
		else
			R_n = 0.0;
		if (q_f < 0.2) q_f = 0.2;
		if (q_f > 1.2) q_f = 1.2;
		sub_step *= q_f;
		if ((sub_step_sum + sub_step) > 1.0) sub_step = 1.0 - sub_step_sum;
		// Ckeck convergence
		...
		if (F > f_tol && !PreLoad)  // in yield status
		{
			...
			while (isLoop)  // Newton step for the plastic multiplier
			{
				...
				alpha1 =
				    (2.0 * p - p_c) / (1.0 + (2.0 * K + vartheta * p_c) * vep);
				alpha2 = -q / (vep + M2 / (G * 6.0));
				alpha3 = vartheta * p_c * alpha1;
				alpha1 *= -K;
				...
				while (isLoop)  // Damp
				{
					...
					while (isLoop)  // Newton step for p_c
					{
						NPStep++;
						if (NPStep > MaxI)
							//    printf("\n Too much iteration in Newton step
							//    the integration of Cam-Clay \n");
							// TEST	abort();
							// test
							break;
		...
		//    ep += fabs(vep)*sqrt(2.0*(1.0/9.0+alpha1)/3.0);
		ep += 3.0 * fabs(vep) * q / M2;
		if (fabs(sub_step_sum - 1.0) < DBL_EPSILON) break;
	}
	//-------------------------------------------------------------
	// Consistent tangential matrix
                                         ...
                                         double* dstrain, double temperature)
{
	...
	switch (Creep_mode)
	{
		case 1:
			//  fac = pow(1.5,
			//  (*data_Creep)(1)+1.0)*(*data_Creep)(0)*pow(norn_S,
			//  (*data_Creep)(1)-1.0)*dt;
			// fac = pow(2.0/3.0, (*data_Creep)(1))*(*data_Creep)(0)*pow(norn_S,
			// (*data_Creep)(1))*dt;
		...
		case 2:
			...
			fac = 1.5 * dt * (*data_Creep)(0) *
			      exp(-(*data_Creep)(2) / (8.314472 * (temperature + 273.15))) *
...
/**************************************************************************
   ...
   last modified:
**************************************************************************/
void CSolidProperties::AccumulateEtr_HL_ODS(const ElementValue_DM* ele_val,
                                            const int nGS)
{
	int i, ns;
	ns = ele_val->xi->Size();
...
void CSolidProperties::CalcYoungs_SVV(const double strain_v)
{
	double nv = Poisson_Ratio();
	E = (*data_Youngs)(0) /
	    (1. + (*data_Youngs)(1) * pow(strain_v, (*data_Youngs)(2)));
	Lambda = E * nv / ((1. + nv) * (1. - 2. * nv));
	G = 0.5 * E / (1. + nv);
	K = (3.0 * Lambda + 2.0 * G) / 3.0;
}
/**************************************************************************
   FEMLib-Method:
   Task: Caculate Young modulus based on the expression of the drained bulk
compressibility
   Programing:
   06/2014 AJ
...
void CSolidProperties::CalcYoungs_Drained(const double stress_eff)
{
	...
	if (stress_eff <= 0)
		...
		E = (3 * (1 - 2 * nv)) / ((*data_Youngs)(1) +
		                          ((*data_Youngs)(0) - (*data_Youngs)(1)) *
		                              exp(-stress_eff / (*data_Youngs)(2)));
	Lambda = E * nv / ((1. + nv) * (1. - 2. * nv));
	G = 0.5 * E / (1. + nv);
	K = (3.0 * Lambda + 2.0 * G) / 3.0;
	biot_const = 1 - K / Ks;
...
/**************************************************************************
   ...
   last modified:
**************************************************************************/
void CSolidProperties::AddStain_by_HL_ODS(const ElementValue_DM* ele_val,
                                          double* stress_n, double* dstrain,
                                          double temperature)
{
	int i, ns, dim;
	...
	//
	norm_str =
	    sqrt(2.0 * TensorMutiplication2(epsilon_tr, epsilon_tr, dim) / 3.0);
	double max_etr =
	    norn_S / (*data_Creep)(6, 0) * exp((*data_Creep)(4, 0) * norn_S);
	...
	if (max_etr < DBL_EPSILON) return;
	for (i = 0; i < ns; i++)
	{
		(*data_Creep)(i, 1) +=
		    1.5 * dt * (1 - norm_str / max_etr) * stress_n[i] / eta_k;
		dstrain[i] -= 1.5 * dt *
		              ((1 - norm_str / max_etr) / eta_k + 1 / eta_m) *
		              stress_n[i];
	}
}
...
double StressNorm(const double* s, const int Dim)
{
	...
	if (Dim == 3) val += +2.0 * s[4] * s[4] + 2.0 * s[5] * s[5];
	return val;
}
...
**************************************************************************/
double TensorMutiplication3(const double* s1, const double* s2,
                            const double* s3, const int Dim)
{
	switch (Dim)
	{
		case 2:
			return (s1[0] * (s2[0] * s3[0] +
			                 s2[3] * s3[3])  // s1_11*(s2_11*s3_11+s2_12*s3_21)
			        +
			        s1[3] * (s2[0] * s3[3] +
			                 s2[3] * s3[1])  // s1_12*(s2_11*s3_12+s2_12*s3_22)
			        +
			        s1[3] * (s2[3] * s3[0] +
			                 s2[1] * s3[3])  // s1_21*(s2_21*s3_11+s2_22*s3_21)
			        +
			        s1[1] * (s2[3] * s3[3] +
...
SolidProp::CSolidProperties* MSPGet(std::string mat_name)
{
	...
	{
		m_msp = msp_vector[i];
		if (mat_name.compare(m_msp->name) == 0) return m_msp;
	}
	return NULL;
}
