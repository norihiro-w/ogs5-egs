...
CNumerics::CNumerics(string name)
{
	...
	nls_method = FiniteElement::INVALID_NL_TYPE;  // Default linear, 0: Picard.
	                                              // 1: Newton. 2:JFNK
...
LINEAR_SOLVER* SetZeroLinearSolver(LINEAR_SOLVER* ls)
{
	...
	if (!ls) return NULL;
	SetLinearSolver(ls);
	if (ls->matrix) MXInitMatrix();
	if (ls->b) MNulleVec(ls->b, ls->dim);
	if (ls->x) MNulleVec(ls->x, ls->dim);
...
{
	if (!ls) return NULL;
	if (ls->matrix) MXSetMatrixPointer(ls->matrix);
...
LINEAR_SOLVER* DestroyLinearSolver(LINEAR_SOLVER* ls)
{
	...
	if (ls->system_time_assemble_function_name)
		ls->system_time_assemble_function_name =
		    (char*)Free(ls->system_time_assemble_function_name);

	// OK    if (ls->name)
	// OK        ls->name = (char *) Free(ls->name);
	if (ls->b) ls->b = (double*)Free(ls->b);
	if (ls->x) ls->x = (double*)Free(ls->x);
	/* //WW
	   if (ls->bc_name)
	 ...
	 */
	if (ls->lsp_name) ls->lsp_name = (char*)Free(ls->lsp_name);
	if (ls->r) ls->r = (double*)Free(ls->r);
	if (ls->xx) ls->xx = (double*)Free(ls->xx);
	if (ls->memory) ls->memory = (double*)Free(ls->memory);
	if (ls->new_memory)
	...
	if (ls->name_ls) ls->name_ls = (char*)Free(ls->name_ls);

	if (ls->name_group_ls) ls->name_group_ls = (char*)Free(ls->name_group_ls);
	if (ls) ls = (LINEAR_SOLVER*)Free(ls);
	return NULL;
}
...
LINEAR_SOLVER* DestroyMemoryLinearSolver(LINEAR_SOLVER* ls)
{
	...
	if (ls->new_memory) ls->new_memory = (double**)Free(ls->new_memory);
...
void SetLinearSolverType(LINEAR_SOLVER* ls, CNumerics* m_num)
{
	...
	/*
	   ...
	   if (!ls->lsp) {
	   ls->lsp =
	   GetLinearSolverProperties(MASTER_DEFAULT_LINEAR_SOLVER_PROPERTIES);
	   if (ls->lsp) {
	   ls->store = get_lsp_store(ls->lsp);
	   }
	   }
	...
	switch (m_num->ls_method)  // OK (ls->lsp->type){
	{
		...
		case 12:
			...
			break;
		default:
			cout << "***ERROR in SetLinearSolverType(): Specified linear "
			        "solver type (" << m_num->ls_method
...
LINEAR_SOLVER* InitMemoryLinearSolver(LINEAR_SOLVER* ls, int memory_number)
{
	...
	if (!ls || memory_number <= 0 || ls->dim <= 0) return NULL;
	SetLinearSolver(ls);

	if (ls->new_memory) DestroyMemoryLinearSolver(ls);

	ls->new_memory = (double**)Malloc(memory_number * sizeof(double*));
	if (ls->new_memory == NULL) return NULL;
	...
	for (i = 0; i < memory_number; i++)
	{
		...
		if (ls->new_memory[i] == NULL) return DestroyMemoryLinearSolver(ls);
	}
...
*************************************************************************/
LINEAR_SOLVER* CreateLinearSolverDim(long store, int unknown_vector_dimension,
                                     long dim)
{
	LINEAR_SOLVER* ls;
                             ...
                             int method)
{
	...
	switch (method)
	{
		...
		case 2:
			for (i = 0l; i < length; i++)
				error =
				    max(error, 2. * fabs(new_iteration[i] - old_iteration[i]) /
				                   (fabs(new_iteration[i]) +
				                    fabs(old_iteration[i]) + MKleinsteZahl));
			return error;
		...
		case 3:
			...
			{
				error = max(error, fabs(new_iteration[i] - old_iteration[i]));
				max_c = max(max(max_c, fabs(new_iteration[i])),
				            fabs(old_iteration[i]));
			}
			...
			for (i = 0l; i < length; i++)
				error = max(error,
				            fabs(new_iteration[i] - old_iteration[i]) /
				                (fabs(new_iteration[i] - reference[i]) +
				                 MKleinsteZahl));
			return error;
