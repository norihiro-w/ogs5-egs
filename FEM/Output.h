...
public:
	...
	void CalculateThroughflow(MeshLib::CFEMesh*, std::vector<long>&,
	                          std::vector<double>&);  // 6/2012 JOD
	 ...
	 */
	void addInfoToFileName(std::string& fname, bool geo = true,
	                       bool process = true,
	                       bool mesh = true) const;  // 09/2010 TF
	...
	bool VARIABLESHARING;  // Coordinates of each node as well as connection
	                       // list is stored only for the first time step; BG:
	                       // 05/2011
