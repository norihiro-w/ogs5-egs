	...
	while (!new_keyword)
	{
		...
		if (line_string.find("$MATRIX") != string::npos)
		{
			for (i = 0; i < matrix_dimension[0] * matrix_dimension[1]; i++)
			{
				...
				line_stream >> *j >> *k >> *l;
				if ((int)dim_x.size() < matrix_dimension[0]) dim_x.push_back(j);
			...
			if ((int)variable_data_vector.size() !=
			    matrix_dimension[0] * matrix_dimension[1] +
			        matrix_dimension[0] + matrix_dimension[1])
			{
				std::cout << "FCT function: Error! The number of data does not "
				             "correspond to the specified matrix size"
				          << "\n";
				break;
			}
			line_stream.clear();
...
void FCTReadTIMData(std::string file_name_base)
{
	...
	if (header_cols.find("TIME") != std::string::npos) mode = 0;
	if (header_rows.find("TIME") != std::string::npos) mode = 1;
	//======================================================================
	...
	if (mode == 0)
	{
		...
		pos1 = pos2 + 1;
		sub_string =
		    line_string.substr(pos1, pos2 + fct_x_name.size() + 2 - pos1);
		...
		while (!csv_file.eof())
		{
			...
			pos1 = 0;
			sub_string =
			    get_sub_string(line_string, delimiter_type, pos1, &pos2);
			sub_string = line_string.substr(pos1, pos2 - pos1);
	...
	if (mode == 1)
	{
		...
		pos1 = pos2 + 1;
		sub_string =
		    line_string.substr(pos1, pos2 + fct_x_name.size() + 1 - pos1);
		fct_x_unit = sub_string;
		if (fct_x_unit.find("MONTH") != string::npos)
			scale_factor = 86400. * 30.;
	...
	*valid = true;
	...
	if (dis_type_name == "TRANSIENT")
	{
		...
		if (index % 2 == 1)
		{
			...
			np = index * 2;
			value =
			    variable_data_vector[pp][1] +
			    (variable_data_vector[np][1] - variable_data_vector[pp][1]) /
			        (variable_data_vector[np][0] -
			         variable_data_vector[pp][0]) *
