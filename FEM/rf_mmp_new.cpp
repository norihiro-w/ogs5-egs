...
std::ios::pos_type CMediumProperties::Read(std::ifstream* mmp_file)
{
	...
	while (!new_keyword)
	{
		...
		if (line_string.find("$GEOMETRY_AREA") != std::string::npos)
		{
			...
			{
				in >> geo_area_file;
				// JT, Dec. 16, 2009, added lines below to correct and globalize
				// the read of geometry area file
			...
			switch (porosity_model)
			{
				...
				case 12:
					in >> porosity_model_values[0];  // WX 03.2011, dependent on
					                                 // strain
					break;
				case 13:  // AJ 06.2014 dependant on volumetric strain and
					      // unjacketed volumetric strain (Blöcher 2013)
					in >> porosity_model_values[0];  // initial porosity
					break;
				case 14:  // AJ 06.2014 dependant on volumetric strain and
					      // unjacketed volumetric strain and temperature (case
					      // 18 + thermal dependency)
					in >> porosity_model_values[0];  // initial porosity
					in >> porosity_model_values[1];  // thermoelasticity mode
					in >> porosity_model_values[2];  // reference temperature
					in >> porosity_model_values[3];  // bulk thermal expansion
					                                 // coefficient
			...
			switch (storage_model)
			{
				...
				case 5:  // Storativity is a function of normal effective stress
					...
					in >> storage_model_values[2];  // Default storage value for
					                                // material groups > 0
					in >> storage_model_values[3];  // Angular difference
					                                // between Y direction and
					                                // Sigma 1
					pcs_name_vector.push_back("PRESSURE1");
					break;
				...
				case 7:                             // RW/WW
					...
					in >> storage_model_values[2];  // macroscopic drained bulk
					                                // modulus
					{
				...
				case 8:                             // NW: read from file
					in >> storage_model_values[0];  // some dummy default value
					                                // is read
					break;
				case 9:                             // Dynamic fluid compressibility
					in >> storage_model_values[0];  // some dummy default value
					                                // is read
					break;
				default:
			...
			switch (permeability_strain_model)
			{
				...
				case 5:  // strain volume (threshold value, can be changed with
					...
					in >> permeability_strain_model_value
					          [1];  // d_fac/d_volStrain when vol. strain <=
					                // threshold
					in >> permeability_strain_model_value
					          [2];  // d_fac/d_volStrain when vol. strain >
					                // threshold
		...
		if (line_string.find("$PERMEABILITY_SATURATION") != std::string::npos)
		{
			int num_phases = 0;
			if (H3_Process)
				num_phases = 3;  // the existence of additional fluids in the
			                     // .mfp file is not an indicator of a 4-phase
			                     // system.
			else if (H2_Process)
				num_phases = 2;  // the existence of additional fluids in the
			                     // .mfp file is not an indicator of a 3-phase
			                     // system.
			else if (H_Process)
				num_phases = 1;
			//
			for (k = 0; k < num_phases; k++)
			{
				...
				switch (permeability_saturation_model[k])
				{
					...
					case 3:  // JT: Function (power or linear)      WETTING
						...
						in >> minimum_relative_permeability;  // minimum
						                                      // relative
						                                      // permeability
						                                      // this phase
						break;
					...
					case 4:  // 2-phase Van Genuchten/Mualem Model  WETTING
						// krw = pow(se,0.5) *
						// pow(1.0-pow(1.0-pow(se,1.0/m),m),2)
						// Se  = (sl - slr) / (slm - slr)
						in >> residual_saturation[k];  // slr: residual
						                               // saturation, this phase
						in >> maximum_saturation[k];   // slm: maximum
					...
					case 44:  // 2-phase Van Genuchten/Mualem Model NON-WETTING
						// krg = pow(1.0-se,1.0/3.0) *
						// pow(1.0-pow(se,1.0/m),2.0*m)
						// Se  = (sl - slr) / (slm - slr) --> slr = 1 - sgm -->
						// slm = 1 - sgr
						in >> residual_saturation[k];  // sgr: residual
						                               // saturation, this phase
		...
		if (line_string.find("$CAPILLARY_PRESSURE") != std::string::npos)
		{
			...
			switch (capillary_pressure_model)
			{
				...
				case 1:  // const
					...
					// the old version this entry was used to represent
					// a constant value of saturation in PP models. SHOULD
					...
					{   // Then a constant saturation value has been entered.
						...
						ScreenMessage(
						    ...
						    "used for a constant saturation. THIS IS NOW MODEL "
						    "#2. PLEASE SWITCH TO MODEL #2.\n");
		...
		if (line_string.find("$POROSITY_DISTRIBUTION") != std::string::npos)
		{
			...
			// else{ //CB this is to get the correct path in case the exe is not
		...
		*mmp_file << " $PERMEABILITY_TENSOR" << endl;
		switch (permeability_tensor_type)
		{
			case 0:
				*mmp_file << "  "
				          << "ISOTROPIC"
				          << " " << permeability_tensor[0] << endl;
				break;
			case 3:  // MB for heterogeneous fields //OK
				*mmp_file << "  "
				          << "FILE"
				          << " " << permeability_file << endl;
				break;
		}
	}
	//....................................................................
		...
		switch (heat_dispersion_model)
		{
			case 1:
				...
				*mmp_file << "  " << heat_dispersion_model << " "
				          ...
				          << heat_dispersion_transverse << endl;
				break;
		}
	}
	//....................................................................
		...
		switch (mass_dispersion_model)
		{
			case 1:
				...
				*mmp_file << "  " << mass_dispersion_model << " "
				          ...
				          << mass_dispersion_transverse << endl;
				break;
		}
	}
...
CMediumProperties* MMPGet(const std::string& mat_name)
{
	...
	{
		m_mat = mmp_vector[i];
		if (mat_name.compare(m_mat->name) == 0) return m_mat;
	}
	return NULL;
}
...
CMediumProperties* CMediumProperties::GetByGroupNumber(int group_number)
{
	...
	{
		m_mat = mmp_vector[i];
		if (m_mat->number == group_number) return m_mat;
	}
	return NULL;
}
    ...
    const double wetting_saturation, int phase)
{
	...
	switch (model)
	{
		...
		case 66:  // 2-phase BROOKS/COREY --> NON-WETTING
			...
			// residual_saturation[0] - residual_saturation[1]);
			//			kr = pow(1.0 - se, 2) * (1.0 - pow(se, (2.0 +
...
double* CMediumProperties::HeatConductivityTensor(int number, double* variables)
{
	...
	//   double *tensor = NULL;
	// double a, b, Pc, T, Mw, rhow, rho_gw,rho_ga,rho_g, p_gw, mat_fac_w,
	// mat_fac_g, A, B,H_vap, dp_gw, dPc, dA, dB, dT, q,Tc=647.3,expfactor;
	double a, b, rhow, rho_gw, rho_ga, rho_g, p_gw, mat_fac_w, mat_fac_g, A, B,
	    H_vap, dp_gw, dPc, dA, dB, dT, q;
	// TF unused variable - comment fix compile warning
	//   double Tc=647.3;
	double expfactor;
	...
	{
		for (size_t ii = 0; ii < pcs_vector.size(); ii++)
			//		if (pcs_vector[ii]->pcs_type_name.find("FLOW") !=
			// string::npos) TF
			if (isFlowProcess(pcs_vector[ii]->getProcessType())) FLOW = true;
		if (FLOW)  // WW
		{
			    ...
			    Fem_Ele_Std->cpl_pcs->getProcessType() == FiniteElement::MULTI_PHASE_FLOW)
			{
				...
				heat_conductivity_fluids +=
				    (1.0 - Sw) * m_mfp->HeatConductivity();
			}
			else
			{
				if (Fem_Ele_Std->FluidProp->density_model == 14 &&
				    Fem_Ele_Std->MediaProp->heat_diffusion_model == 273 &&
				    Fem_Ele_Std->cpl_pcs)
				{
					dens_arg[0] = Fem_Ele_Std->interpolate(
					    Fem_Ele_Std->NodalValC1);  // Pressure
					dens_arg[1] =
					    Fem_Ele_Std->interpolate(Fem_Ele_Std->NodalVal1) +
					    273.15;                        // Temperature
	...
	m_msp = msp_vector[group];
	m_msp->HeatConductivityTensor(dimen, heat_conductivity_tensor, group,
	                              variables);  // MX
...
double* CMediumProperties::HeatDispersionTensorNew(int ip, double* var)
{
	...
	if (std::abs(vg) >
	        MKleinsteZahl  // For the case of diffusive transport only
	    // WW
	    &&
	    (alpha_l > MKleinsteZahl || alpha_t > MKleinsteZahl))
	{
		...
		if (Fem_Ele_Std->FluidProp->density_model ==
		    14)  // used density changing with p, T
		{
                                                   ...
                                                   int tr_phase)  // SB + BG
{
	...
	if (Fem_Ele_Std->FluidProp->density_model == 14)
	{
		...
		molecular_diffusion_value =
		    m_mfp->MaxwellStefanDiffusionCoef(index, PG, TG, component) *
		    TortuosityFunction(index, g, theta);
	}
	...
	// Global Velocity
	double velocity[3] = {0., 0., 0.};
	gp_ele->getIPvalue_vec_phase(ip, tr_phase, velocity);  // gp velocities //
	                                                       // SB
	vg = MBtrgVec(velocity, 3);
	//  if(index < 10) cout <<" Velocity in MassDispersionTensorNew():
	//  "<<velocity[0]<<", "<<velocity[1]<<", "<<velocity[2]<<", "<<vg<<endl;
...
int CMediumProperties::GetPropertyType(std::string prop_name)
{
	...
	while (p_keywords != keywd_list.end())
	{
		...
		if (prop_name.compare(keyword_name) == 0) return counter;
		++p_keywords, counter++;
	}
	return -1;
}
...
double CMediumProperties::Porosity(long number, double theta)
{
	...
	// NW commented out below to use permeability_hetero_value_id instead of
	...
	switch (porosity_model)
	{
		...
		case 11:  // n = temp const, but spatially distributed CB
			// porosity = porosity_model_values[0];
			porosity = _mesh->ele_vector[number]->mat_vector(
			    this->porosity_hetero_value_id);
			break;
		...
		case 13:
			porosity =
			    PorosityDrainedStrain(number, porosity_model_values[0], assem);
			break;
		case 14:
			porosity = PorosityDrainedStrainTemp(
			    number, porosity_model_values[0], porosity_model_values[1],
...
double CMediumProperties::Porosity(CElement* assem)
{
	...
	switch (porosity_model)
	{
		...
		case 12:  // n = n0 + vol_strain
			...
			break;
		case 13:
			porosity = PorosityDrainedStrain(number, porosity_model_values[0],
			                                 assem_tmp);
			break;
		case 14:
			porosity = PorosityDrainedStrainTemp(
    ...
    long index, double saturation, double temperature, double* porosity_sw)
{
	...
	if (porosity < porosity_min) porosity = porosity_min;
	...
	if (aktuelle_zeit >= 2 * dt) m_pcs->SetElementValue(index, indx_sw1, n_sw);
	...
	if (det_n_sw < -1.0e-8) det_n_sw = det_n_sw;
	if (aktuelle_zeit == dt)
...
double* CMediumProperties::PermeabilityTensor(long index)
{
	...
	// the values depends on the
	// value in permability_model. for 3 and 4, it gets the values from K-C
	// relationship.
	...
	{  // only when permeability is isotropic
		...
		{   // here get the initial permeability values from material perperty
			...
			//// get the
			/// index:-------------------------------------------------------------------
			// for(perm_index = 0; perm_index <
			// (int)m_pcs->m_msh->mat_names_vector.size();
			//    perm_index++)
			...
			tensor[0] = _mesh->ele_vector[index]->mat_vector(
			    this->permeability_hetero_value_id);
			// CMCD
			// 01.09.2011 WW.  int edx =
			// m_pcs->GetElementValueIndex("PERMEABILITY");
			// CMCD
		...
		else if (permeability_model ==
		         7)  // HS: 11.2008, for Clement biofilm clogging
		{
			// if first time step, do nothing. otherwise,
			if (aktueller_zeitschritt > 1)
			{
		...
		else if (permeability_model == 8)  // AJ: 05.2014
		{
			...
			else
			{
				...
				k_new = permeability_porosity_model_values[1] *
				        MathLib::fastpow(n_new, 3) /
				        MathLib::fastpow(1 - n_new, 2);
			}
			// save new permeability
			m_pcs->SetElementValue(index, idx_k + 1, k_new);
...
double CMediumProperties::PermeabilityFunctionStrain(
    long index,
    int nnodes,
    CFiniteElementStd* h_fem)  // WW:02.08.2010
{
	...
	switch (permeability_strain_model)
	{
		case 1:
		{
			fac_perm_strain = GetCurveValue(permeability_strain_model_value[0],
			                                ...
			                                vol_strain_temp,
			                                &gueltig);
			if (fac_perm_strain <= 0.) fac_perm_strain = 1.;
			break;
		}
		...
		case 3:  // if StrainP>0, factor=f(StrainP), else
			     // factor=f(strain_Volume)
			{
				...
				{
					fac_perm_strain =
					    GetCurveValue(permeability_strain_model_value[0],
					                  0,
					                  vol_strain_temp,
					                  &gueltig);
				}
...
void GetHeterogeneousFields()
{
	...
	for (int i = 0; i < (int)mmp_vector.size(); i++)
	{
		...
		if (m_mmp->permeability_file.size() > 0)
		{
			...
			// WW
			m_mmp->permeability_hetero_value_id =
			    m_mmp->SetDistributedELEProperties(m_mmp->permeability_file, i);
		...
		if (m_mmp->porosity_file.size() > 0)
		{
			...
			// Removed bugs in this function
			// CB Removed bugs in this function
			m_mmp->porosity_hetero_value_id =
			    m_mmp->SetDistributedELEProperties(m_mmp->porosity_file, i);
			// m_mmp->WriteTecplotDistributedProperties();
		}
		//....................................................................
		// For Storage
		...
		{
			m_mmp->storage_hetero_value_id =
			    m_mmp->SetDistributedELEProperties(m_mmp->storage_file, i);
			// m_mmp->WriteTecplotDistributedProperties();
		}
		//....................................................................
		// GEOMETRY_AREA
	...
	if (area != 1.0)
		for (i = 0; i < no_processes; i++)
		{
			...
			for (j = 0; j < no_ele; j++)
			{
				...
				if (ele_group == group) _mesh->ele_vector[j]->SetFluxArea(area);
			}
		}
}
...
**************************************************************************/
int CMediumProperties::SetDistributedELEProperties(const string& file_name,
                                                   size_t mmp_id)
{
	...
	while (!mmp_property_file.eof())
	{
		...
		if (line_string.find("$MMP_TYPE") != string::npos)
		{
			...
			std::vector<std::string>::iterator itr =
			    std::find(_mesh->mat_names_vector.begin(),
			              _mesh->mat_names_vector.end(), mmp_property_name);
			...
			switch (mmp_property_dis_type[0])
			{
				...
				case 'G':  // Geometric mean
					...
					for (i = 0; i < (long)_mesh->ele_vector.size(); i++)
					{
						...
						// check mat id
						if (m_ele_geo->GetPatchIndex() != mmp_id)
							continue;  // NW
						...
						if (mmp_property_dis_type[0] == 'N')
						{
							...
							// nearest given value in the input file
							// Return value ihet is the index of the het. val in
							// the mmpval-vector
	...
	if ((long)_mesh->ele_vector.size() > 0)
	{
		...
		switch (m_ele->GetElementType())
		{
			...
			case MshElemType::PRISM:
				...
				break;
			default:
				std::cerr << "CMediumProperties::"
				             "WriteTecplotDistributedProperties MshElemType "
				             "not handled"
				          << "\n";
		}
	}
                       ...
                       vector<double> mmpvals)
{
	...
	// eingelesenen Knoten und Geometrieknoten-RF;
	...
	if ((long)ele_value_dm.size() > 0)  // Deformation process coupled
	{
		...
		for (i = 0; i < 3; i++)
		{
			...
			b[i] = c_coefficient[2 + i] +
			       c_coefficient[0] *
			           exp(c_coefficient[1] * sig[i] / (T * GAS_CONSTANT));
			//
		}
	}
	else
...
bool MMPExist(ifstream* mmp_file)
{
	...
	while (!mmp_file->eof())
	{
		...
		if (line_string.find("$NAME") != string::npos)
		{
			...
			for (int i = 0; i < (int)mmp_vector.size(); i++)
				if (name.compare(mmp_vector[i]->name) == 0) return true;
    ...
    long index, double saturation, double temperature)
{
	...
	/*OK411
	 ...
	 [m^2/g]
	   fmon =     porosity_model_values[2];     // Anteil quelfaehige mineral
	 [-]
	   mat_mp_m = porosity_model_values[3]; // Schichtenanzahl eines
	 quellfähigen Partikels [-]
	   beta =     porosity_model_values[6];     // modifications coefficient
	 (z.B. free swelling Weimar beta=3.0)
	   //--------------------------------------------------------------------
	   // MSP solid properties
    ...
    long index, double saturation, double temperature, double* porosity_sw)
{
	...
	/*OK411
	 ...
	 sample is homogenous [-]
	   porosity_min = porosity_model_values[7]; //minimal porosity after
	 swelling compaction
...
double CMediumProperties::PorosityVolumetricChemicalReaction(long index)
{
	...
	/*OK411
	   ...
	   conc[component]*molar_volume[component-n-2];
                                            ...
                                            CFiniteElementStd* assem)
{
	...
	for (int j = 0; j < dim; j++)
		vol_strain_temp += strain_temp[j];
	val += vol_strain_temp;
	if (val < MKleinsteZahl) val = 1e-6;  // lower limit of porostity
	return val;
}

// AJ: 12.2014. Porosity = (n0 - vol_strain + vol_strain_us*(1 - n0)) / (1 -
// vol_strain)
double CMediumProperties::PorosityDrainedStrain(long index, double val0,
                                                CFiniteElementStd* assem)
{
	double val = val0, vol_strain_temp = 0., strain_temp[3] = {0.},
	...
	{
		for (int i = 0; i < ngp;
		     i++)  // loop on the Gauss points of the element
		{
	...
	val = (val0 + vol_strain_temp - vol_strain_us * (1 - val0)) /
	      (1 + vol_strain_temp);
...
// AJ: 12.2014. Porosity = (n0 - vol_strain + vol_strain_us*(1 - n0)) / (1 -
// vol_strain) + thermal effect
double CMediumProperties::PorosityDrainedStrainTemp(
    long index, double val0, int thermo_mode, double T0, double alpha_b,
    double alpha_s, CFiniteElementStd* assem)  // AJ: 09.2014
{
	double val = val0, vol_strain_temp = 0., strain_temp[3] = {0.},
	       mean_stress_temp = 0., stress_temp[3] = {0.}, temperature_temp = 0.,
	       temperature_nodes[20] = {0.}, vol_strain_us = 0., Ks;
	...
	if (m_pcs->getProcessType() == FiniteElement::HEAT_TRANSPORT)
	{
		...
		for (int i = 0; i < nnodes; i++)
			temperature_nodes[i] = assem->pcs->GetNodeValue(
			    assem->pcs->m_msh->ele_vector[index]->getNodeIndices()[i],
			    idx_temp[0]);
	...
	else
	{
		...
		for (int i = 0; i < nnodes; i++)
			temperature_nodes[i] = assem->cpl_pcs->GetNodeValue(
			    assem->cpl_pcs->m_msh->ele_vector[index]->getNodeIndices()[i],
			    idx_temp[0]);
		temperature_temp = assem->interpolate(temperature_nodes);
	}
	...
	switch (thermo_mode)
	{
		case 1:
			val = (val0 + vol_strain_temp - vol_strain_us * (1 - val0)) /
			          (1 + vol_strain_temp) +
			      (alpha_b - (1 - val0) * alpha_s) * (temperature_temp - T0);
			break;
		case 2:
			val = (val0 + vol_strain_temp - vol_strain_us * (1 - val0) +
			       alpha_b - (1 - val0) * alpha_s) /
			      (1 + vol_strain_temp + alpha_b);
			break;
	}
...
/**************************************************************************
   ...
   last modification:
**************************************************************************/
double CMediumProperties::TortuosityFunction(long number,
                                             double* gp,
                                             double theta,
                                             CFiniteElementStd* assem)
{
 ...
 */
/**************************************************************************/
double CMediumProperties::NonlinearFlowFunction(long index, double* gp,
                                                double theta)
{
	double k_rel = 1.0;
	// OK411
	...
	/*OK411
	   ...
	   //          if(
	   (fabs(grad_h[0])>MKleinsteZahl)||(fabs(grad_h[1])>MKleinsteZahl) ) )
	   ...
	   porosity to individual fracture porosity
	   ...
	   03/2004
	   area = ElGetElementVolume(index)/m_mmp->geo_area; //CMCD March 2004
	   removed wrong area in  code
...
Kurve
   ...
   5     Storage as normal stress in element in stress field defined by KTB
stress field.
   6		Storage as normal stress in element in stress field defined by KTB
stress
   field, function to increase storage with distance from borehole.
**************************************************************************/
double CMediumProperties::StorageFunction(long index, double* /*gp*/,
                                          double /*theta*/)
{
	// int nn, i, Type;
	// int idummy;
	...
	// WW dircosl =    dircosm =    dircosn = 0.0;             //Initialise
	// variable
	// static int nidx0,nidx1;
	// double primary_variable[10];		//OK To Do
	// int count_nodes;
	   ...
	   for(i=0;i<no_pcs_names;i++)
	    {
	     ...
	     if(mode==0) // Gauss point values
	      {
	       primary_variable[i] =
	 (1.-theta)*InterpolValue(index,nidx0,gp[0],gp[1],gp[2]) \
...
/* Funktion der effektiven Spannung und des Drucks in Elementmitte
		...
		case 7:  // poroelasticity RW
			...
			break;
		case 8:
			storage = _mesh->ele_vector[index]->mat_vector(
			    this->storage_hetero_value_id);
			break;
		default:
...
double CMediumProperties::PermeabilityPressureFunction() const
{
	...
	const double p = this->Fem_Ele_Std->interpolate(
	    this->Fem_Ele_Std->NodalVal1);  // OK4709 pressure
                                                       ...
                                                       double theta)
{
	...
	switch (permeability_pressure_model)
	{
		...
		case 5:
			...
			k_rel = GetCurveValue((int)permeability_pressure_model_values[0],
			                      0,
			                      stress_eff,
			                      &idummy);
			break;
...
system
   converts the normal stress to effective stress, reads from a curve what the
permeability
                                                              ...
                                                              double pressure)
{
	        ...
	        4) /*Function defined for square, triangular and cubic elements*/
	{
		...
		/*Take value of storage from a curve*/
		R = GetCurveValue((int)permeability_pressure_model_values[0],
		                  0,
		                  eff_norm_stress,
		                  &idummy);
...
system
   converts the normal stress to effective stress, reads from a curve what the
permeability
   of a fracture under the given effective stress is. Permeability is then
related to
...
approximate formulation
   from the relationship of stress with depth. This normal stress is then
converted into a permeabilty
...
adjusted to take account of
   thermal cooling, and the resulting effective stress across the fracture
isconverted into a permeabilty.
    ...
    long index, double pressure, double temperature)
{
	...
	if (stress_eff < 0.0) stress_eff = 0.0;
