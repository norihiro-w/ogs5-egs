...
bool CVTK::InitializePVD(const string& file_base_name,
                         const string& pcs_type_name, bool binary)
{
	// PVD
	this->vec_dataset.clear();
	this->pvd_file_name = file_base_name;
	if (pcs_type_name.size() > 0) this->pvd_file_name += "_" + pcs_type_name;
...
bool CVTK::WriteHeaderOfPVD(std::fstream& fin)
{
	fin << "<?xml version=\"1.0\"?>"
	    << "\n";
	fin << "<VTKFile type=\"Collection\" version=\"0.1\" "
...
bool CVTK::WriteDatasetOfPVD(std::fstream& fin, double timestep,
...
unsigned char CVTK::GetVTKCellType(const MshElemType::type ele_type)
{
	...
	switch (ele_type)
	{
		...
		case MshElemType::PYRAMID:  // pyramid=14
			...
			break;
		default:
			std::cerr << "***ERROR: NO CORRESPONDING VTK CELL TYPE FOUND. "
...
std::string CVTK::vtkDataType2str(VTK_XML_DATA_TYPE data_type)
{
	...
	switch (data_type)
	{
		...
		case CVTK::Int64:
			...
			break;
		case CVTK::UInt64:
			str_data_type = "UInt64";
			break;
		case CVTK::Float32:
			str_data_type = "Float32";
			break;
		case CVTK::Float64:
			str_data_type = "Float64";
			break;
	}
                                ...
                                long offset)
{
	...
	if (str_name != "") fin << " Name=\"" << str_name << "\"";
	if (nr_components > 1)
		fin << " NumberOfComponents=\"" << nr_components << "\"";
	fin << " format=\"" << str_format << "\"";
	if (useBinary) fin << " offset=\"" << offset << "\" /";
	fin << ">"
	    << "\n";
...
{
	if (!this->isInitialized) this->InitializeVTU();

	//-------------------------------------------------------------------------
	//# Setup file stream
	//-------------------------------------------------------------------------
	const std::string vtkfile =
	    vtkfile_base + "_" + number2str(time_step_number) + ".pvtu";
	std::fstream fin;
	...
	//# Header
	fin << "<?xml version=\"1.0\"?>\n";
	fin << "<!-- Time step: " << time_step_number
	    << " | Time: " << out->getTime() << " -->\n";
	...
	fin << "    <PPoints>\n";
	fin << "       <PDataArray type=\"" << vtkDataType2str(this->type_Double)
	...
	if (out->_nod_value_vector.size() > 0)
	{
		...
		for (size_t i = 0; i < out->_nod_value_vector.size(); i++)
		{
			...
			fin << "      <PDataArray type=\""
			    << vtkDataType2str(this->type_Double) << "\" Name=\""
			    << external_val_name << "\" format=\"ascii\" />\n";
		}
		if (outNodeVelocity)
			fin << "      <PDataArray type=\""
			    << vtkDataType2str(this->type_Double) << "\" Name=\""
	...
	// cell data
	fin << "    <PCellData Scalars=\""
	    << "Domain"
	    << "\" >\n";
	fin << "      <PDataArray type=\"" << vtkDataType2str(this->type_Int)
	    << "\" Name=\"Domain\" format=\"ascii\" />\n";
#if 0
	fin << "      <PDataArray type=\""<< vtkDataType2str(this->type_UChar) << "\" Name=\"vtkGhostLevels\" format=\"ascii\" />\n";
#endif
	fin << "      <PDataArray type=\"" << vtkDataType2str(this->type_Int)
	    << "\" Name=\"MatGroup\" format=\"ascii\" />\n";
	...
	for (int i = 0; i < (int)ele_value_index_vector.size(); i++)
	{
		...
		fin << "      <PDataArray type=\"" << vtkDataType2str(this->type_Double)
		    << "\" Name=\"" << out->getElementValueVector()[i]
		    << "\" format=\"ascii\" />\n";
	}
	if (outEleVelocity)
		fin << "      <PDataArray type=\"" << vtkDataType2str(this->type_Double)
	...
	for (size_t i_mmp = 0; i_mmp < out->mmp_value_vector.size(); i_mmp++)
		fin << "      <PDataArray type=\"" << vtkDataType2str(this->type_Double)
		    << "\" Name=\"" << out->mmp_value_vector[i_mmp]
		    << "\" format=\"ascii\" />\n";
	for (size_t i = 0; i < out->mfp_value_vector.size(); i++)
		fin << "      <PDataArray type=\"" << vtkDataType2str(this->type_Double)
		    << "\" Name=\"" << out->mfp_value_vector[i]
		    << "\" format=\"ascii\" />\n";
	fin << "    </PCellData>\n";
	...
	{
		std::string sub_vtu_file_name = vtkfile_base + "_part" + number2str(i) +
		                                "_" + number2str(time_step_number) +
		                                ".vtu";
                                    ...
                                    const int time_step_number)
{
	...
	WriteMeshElementConnectivity(fin, data_out, msh, offset,
	                             sum_ele_components);
	WriteDataArrayFooter(fin);
	// offset
	...
	if (useBinary)
	{
		...
		// Element
		// conncectivity
		this->WriteMeshElementConnectivity(fin, true, msh, offset,
		                                   sum_ele_components);
		// offset
...
template <typename T>
void CVTK::write_value_binary(std::fstream& fin, T val)
{
	fin.write((const char*)&val, sizeof(T));
}

bool CVTK::WriteMeshNodes(std::fstream& fin, bool output_data, CFEMesh* msh,
                          long& offset)
{
                                        ...
                                        long& sum_ele_components)
{
	if (output_data)
	{
		...
		{
			write_value_binary<unsigned int>(fin,
			                                 sizeof(long) * sum_ele_components);
			for (long i = 0; i < (long)msh->ele_vector.size(); i++)
			{
				ele = msh->ele_vector[i];
				for (size_t j = 0;
...
bool CVTK::WriteMeshElementOffset(std::fstream& fin, bool output_data,
                                  CFEMesh* msh, long& offset)
{
	if (output_data)
	{
		MeshLib::CElem* ele = NULL;
		...
		else
		{
			...
			write_value_binary<unsigned int>(
			    fin, sizeof(long) * (long)msh->ele_vector.size());
			long ele_offset = 0;
			for (long i = 0; i < (long)msh->ele_vector.size(); i++)
			{
				ele = msh->ele_vector[i];
		...
		// OK411
		offset += (long)msh->ele_vector.size() * sizeof(long) +
		          SIZE_OF_BLOCK_LENGTH_TAG;

	return true;
}

bool CVTK::WriteMeshElementType(std::fstream& fin, bool output_data,
                                CFEMesh* msh, long& offset)
{
	if (output_data)
	{
		...
		else
		{
			// OK411
			write_value_binary<unsigned int>(
			    fin, sizeof(unsigned char) * (long)msh->ele_vector.size());
			for (long i = 0; i < (long)msh->ele_vector.size(); i++)
			{
				ele = msh->ele_vector[i];
				write_value_binary(fin,
				                   this->GetVTKCellType(ele->GetElementType()));
			}
		}
	}
	else if (useBinary)
                           ...
                           long& offset)
{
	...
	if (out->m_pcs != NULL) m_pcs = out->m_pcs;
	...
	for (int i = 0; i < (int)out->_nod_value_vector.size(); i++)
	{
		...
		//    if (out->m_pcs == NULL || out->pcs_type_name.compare("NO_PCS")==0)
		if (out->m_pcs == NULL ||
		    out->getProcessType() == FiniteElement::NO_PCS)
			m_pcs = PCSGet(internal_val_name, true);
		if (!m_pcs) continue;

		NodeIndex[i] =
		    m_pcs->GetNodeValueIndex(internal_val_name, true);  // JT: Latest
		if (NodeIndex[i] < 0) continue;

		if (!useBinary || !output_data)
			WriteDataArrayHeader(fin, type_Double, external_val_name, 0,
			                     str_format, offset);

		if (output_data)
		{
			/* JT: Just get the latest index. No need for all this extra
			looping.
			for (size_t j = 0; j < m_pcs->GetPrimaryVNumber(); j++)
			    if
			(internal_val_name.compare(m_pcs->pcs_primary_function_name[j]) ==
			0) {
			...
			*/
			...
			{
				write_value_binary<unsigned int>(
				    fin, sizeof(double) * msh->GetNodesNumber(false));
			}
			for (size_t j = 0; j < msh->GetNodesNumber(false); j++)
			{
				double v = m_pcs->GetNodeValue(msh->nod_vector[j]->GetIndex(),
				                               NodeIndex[i]);
				if (!useBinary)
				{
					fin << v << " ";
	...
	if (outNodeVelocity)
	{
		...
		{
			const string& internal_val_name = out->_nod_value_vector[i];
			//            const string &external_val_name =
			//            out->_alias_nod_value_vector[i];
			if (internal_val_name.find("VELOCITY_X1") != string::npos)
			{
				if (out->m_pcs == NULL) m_pcs = PCSGet(internal_val_name, true);
				velocity_id = 0;
			}
			else if (internal_val_name.find("VELOCITY_X2") != string::npos)
			{
				if (out->m_pcs == NULL) m_pcs = PCSGet(internal_val_name, true);
				velocity_id = 1;
			}
			else if (internal_val_name.find("VELOCITY1_X") != string::npos)
			{
				if (out->m_pcs == NULL) m_pcs = PCSGet(internal_val_name, true);
				velocity_id = 2;
			}
			...
			{
				int ix, iy, iz;
				ix = m_pcs->GetNodeValueIndex(
				    velocity_name[velocity_id][0],
				    true);  // JT: Fix. Need latest value.
				...
				{
					write_value_binary<unsigned int>(
					    fin, sizeof(double) * msh->GetNodesNumber(false) * 3);
					for (size_t j = 0l; j < msh->GetNodesNumber(false); j++)
					{
						write_value_binary(
						    fin, m_pcs->GetNodeValue(
	...
	if (outNodeDisplacement)
	{
		...
		for (int i = 0; i < (int)out->_nod_value_vector.size(); i++)
		{
			...
			if (!useBinary || !output_data)
				WriteDataArrayHeader(fin, this->type_Double, "DISPLACEMENT", 3,
				                     str_format, offset);
			if (output_data)
			{
				int var_id[3] = {};
				...
				for (size_t j = 0l; j < msh->GetNodesNumber(false); j++)
				{
					for (size_t k = 0; k < 3; k++)
					{
						if (var_id[k] < 0)
							...
							u[k] = m_pcs->GetNodeValue(
							    msh->nod_vector[j]->GetIndex(), var_id[k]);
					}
                             ...
                             long& offset)
{
	...
	// Domain ID
	if (!useBinary || !output_data)
		WriteDataArrayHeader(fin, this->type_Int, "Domain", 0, str_format,
		                     offset);
	if (output_data)
	{
		if (!this->useBinary)
		...
		{
			write_value_binary<unsigned int>(
			    fin, sizeof(int) * (long)msh->ele_vector.size());
			for (long i = 0; i < (long)msh->ele_vector.size(); i++)
	...
	// Mat ID
	if (!useBinary || !output_data)
		WriteDataArrayHeader(fin, this->type_Int, "MatGroup", 0, str_format,
		                     offset);
	if (output_data)
	{
		if (!this->useBinary)
	...
	for (int i = 0; i < (int)ele_value_index_vector.size(); i++)
	{
		...
		if (!useBinary || !output_data)
			WriteDataArrayHeader(fin, this->type_Double,
			                     out->getElementValueVector()[i], 0, str_format,
			                     offset);
		if (output_data)
		{
			if (!useBinary)
			...
			else
			{
				write_value_binary<unsigned int>(
				    fin,
				    sizeof(double) * (long)msh->ele_vector.size());  // OK411
				for (long j = 0; j < (long)msh->ele_vector.size(); j++)
					write_value_binary(fin, m_pcs->GetElementValue(
					                            j, ele_value_index_vector[i]));
			}
		}
		else
		...
		if (out->getProcessType() == FiniteElement::FLUID_MOMENTUM)
		{
			...
			if (output_data)
			{
				...
				else
				{
					// OK411
					write_value_binary<unsigned int>(
					    fin, sizeof(double) * 3 * (long)msh->ele_vector.size());
					for (long i = 0; i < (long)msh->ele_vector.size(); i++)
					{
						write_value_binary(
						    fin,
						    pch_pcs->GetElementValue(
						        i,
	...
	if (out->mmp_value_vector.size() > 0)
	{
		...
		for (size_t i_mmp = 0; i_mmp < out->mmp_value_vector.size(); i_mmp++)
		{
			...
			if (!useBinary || !output_data)
				WriteDataArrayHeader(fin, this->type_Double, mmp_name, 0,
				                     str_format, offset);
			...
			if (output_data)
			{
				...
				if (!this->useBinary)
				{
					fin << "          ";
					int gp_r, gp_s, gp_t;
					for (long i_e = 0; i_e < (long)msh->ele_vector.size();
					     i_e++)
					{
	...
	if (out->mfp_value_vector.size() > 0)
	{
		...
		for (size_t i_mfp = 0; i_mfp < out->mfp_value_vector.size(); i_mfp++)
		{
			...
			if (!useBinary || !output_data)
				WriteDataArrayHeader(fin, this->type_Double, mfp_name, 0,
				                     str_format, offset);
			...
			if (output_data)
			{
				...
				if (!this->useBinary)
				{
					     ...
					     i_e++)
					{
						...
						mfp->Fem_Ele_Std = fem;
						double mat_value =
						    ELEMENT_MFP_VALUES::getValue(mfp, mfp_id);
						fin << mat_value << " ";
					}
					fin << "\n";
				}
