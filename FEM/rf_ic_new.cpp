				...
				if (geo_type_name.find("SUB") != string::npos)
				{
					*ic_file >> SubNumber;
					if (convertPrimaryVariableToString(
					        this->getProcessPrimaryVariable()).find("STRESS") !=
					        string::npos ||
...
void CInitialCondition::Write(fstream* ic_file) const
{
	...
	*ic_file << convertPrimaryVariableToString(
	                this->getProcessPrimaryVariable()) << endl;
	//--------------------------------------------------------------------
	// GEO_TYPE
...
void CInitialCondition::SetByElementValues(int nidx)
{
	...
	if (!d_file.is_open())
	{
		cout << "! Error in CInitialCondition::SetByElementValues(): Could not "
		        "find file " << fname << endl;
		abort();
	}
	...
	// convertElementDataToNodalData(*pcs, bc_ele_ids, bc_ele_values,
	// EleToNodeInterpolationMethod::VOLUME_WEIGHTED, vec_nod_values);
	...
	for (size_t i = 0; i < n_nodes; i++)
	{
		...
		double v = getNodalValueFromElementValue(*pcs, map_eleId_values,
		                                         iterpo_type, nod_id);
...
void CInitialCondition::SetPolyline(int nidx)
{
	if (this->getProcessDistributionType() == FiniteElement::CONSTANT)
	{
		...
		//				m_pcs->SetNodeValue(nodes_vector[i], nidx,
		// node_value_vector[0]->node_value);
		//			}
		//		} else {
		//			std::cout << "Error in CInitialCondition::SetPolyline -
		// polyline: "
		//					<< geo_name << " not found" << "\n";
		//		}
		if (getGeoObj())
	...
	else  // WW
	{
		...
		for (k = 0; (size_t)k < SubNumber; k++)
		{
			         ...
			         FiniteElement::FUNCTION)  // 01.07.2008 WW
			{
				ScreenMessage(
				    "subdomain %d: %s\n", subdom_index[k],
				    dis_linear_f->getExpression(subdom_index[k]).data());
				for (i = 0; i < nodes_vector.size(); i++)
				{
					...
					getProcess()->SetNodeValue(
					    nodes_vector[i],
					    nidx,
					    dis_linear_f->getValue(subdom_index[k], pnt[0], pnt[1],
					                           pnt[2]));
				}
			}
			else
