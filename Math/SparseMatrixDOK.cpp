...
void SparseMatrixDOK::Diagonize(size_t idiag, const double b_given, double* b)
{
	...
	if (!this->is_constructed)
	{
		...
		for (size_t i = 0; i < n_rows; i++)
		{
			...
			//      for(col_id_itr kk=set_col_id[cnt_rows].begin();
			//      kk!=colid_end; kk++){
			//        jj = ii->find(*kk);
			//#else
...
#ifdef USE_HASHMAP
	if (this->set_col_id.size() == 0) this->ConstructSortedColumnID();
#endif
...
#else
			for (jj = this->mat_row[i].begin(); jj != this->mat_row[i].end();
			     ++jj)
			{
#endif
				os << std::setw(10) << i + 1 << " " << std::setw(10)
				   << (*jj).first + 1 << " " << std::setw(15) << (*jj).second
				   << "\n";
			}
		}
...
//
	...
	// ptr:         an integer array with a length of n + 1, which stores the
	// starting
	//              points of the rows of the arrays value and index.
	// col_idx:     an integer array with a length of nnz, which stores the
	// column
...
#ifdef USE_HASHMAP
		const col_id_itr colid_end = set_col_id[cnt_rows].end();
		for (col_id_itr kk = set_col_id[cnt_rows].begin(); kk != colid_end;
		     ++kk)
		{
			jj = ii->find(*kk);
#else
