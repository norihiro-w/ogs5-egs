...
/**************************************************************************
 ...
 9-Knoten-Elemente)
   MGradPhi2D_9N         - Gradient der Testfunktionen (zweidim.
 9-Knoten-Elemente)
 ...
 (Lotpunkt)
   MCalcProjectionOfPointOnPlane - Projektionspunkt eines Punktes auf einer
 Flaeche (Lotpunkt)
 ...
 M2InvertiereUndTransponiere umbenannt
...
int MOmega2DTriangle(double* vf, double xx, double yy, long number)
{
	...
	/* CMCD March 2004 The area of the triangle needs to be calculated for the
	   2D projected triangle
	   not the three D as above
	    area = (x[1]*y[2]-x[2]*y[1]+x[0]*y[1]-x[1]*y[0]+x[2]*y[0]-x[0]*y[2])/2;
	   // CMCD March 2004
	   xx=0.;
	   yy=0.;
	   what is this ?
	 */
	vf[0] =
	    (x[1] * y[2] - x[2] * y[1]) + (y[1] - y[2]) * xx + (x[2] - x[1]) * yy;
	vf[1] =
	    (x[2] * y[0] - x[0] * y[2]) + (y[2] - y[0]) * xx + (x[0] - x[2]) * yy;
	vf[2] =
	    (x[0] * y[1] - x[1] * y[0]) + (y[0] - y[1]) * xx + (x[1] - x[0]) * yy;
	vf[0] /= (2. * area);
	vf[1] /= (2. * area);
	vf[2] /= (2. * area);
...
int MPhi3D_SUPG(double* vf, double r, double s, double t, double* alpha)
{
	...
	vf[0] = (1.0 + r) * (1.0 + s) * (1.0 + t) +
	        alpha[0] * (1.0 + s) * (1.0 + t) +
	        alpha[1] * (1.0 + r) * (1.0 + t) + alpha[2] * (1.0 + r) * (1.0 + s);
	vf[1] = (1.0 - r) * (1.0 + s) * (1.0 + t) -
	        alpha[0] * (1.0 + s) * (1.0 + t) +
	        alpha[1] * (1.0 - r) * (1.0 + t) + alpha[2] * (1.0 - r) * (1.0 + s);
	vf[2] = (1.0 - r) * (1.0 - s) * (1.0 + t) -
...
void M2InvertiereUndTransponiere(double* m)
{
	...
	if (fabs(eddet) > MKleinsteZahl) eddet = 1.0 / eddet;
	zecke = m[0];
	m[0] = m[3];
	m[3] = zecke;
	zecke = -m[1];
...
void M3Invertiere(double* m)
{
	...
	d = m[0] * (m[4] * m[8] - m[7] * m[5]) +
	    m[3] * (m[7] * m[2] - m[1] * m[8]) + m[6] * (m[1] * m[5] - m[4] * m[2]);
	if (fabs(d) > MKleinsteZahl) d = 1.0 / d;
	z[0] = m[4] * m[8] - m[7] * m[5];
	z[3] = m[6] * m[5] - m[3] * m[8];
	z[6] = m[3] * m[7] - m[6] * m[4];
	z[1] = m[7] * m[2] - m[1] * m[8];
...
void MInvertiere(double* mat, long m, long n)
{
	...
	{
		DisplayErrorMsg(
		    "Abbruch !!! Nicht-quadratische Matrix *** MInvertiere");
		abort();
	}
	for (k = 0; k < n; k++)
...
**************************************************************************/
#ifdef SX
int MAddVektoren(double* restrict v1, double* restrict v2,
                 double* restrict vout, long g)
#else
...
**************************************************************************/
#ifdef SX
int MAddMatrizen(double* restrict m1, double* restrict m2,
                 double* restrict mout, long m, long n)
#else
...
int MBistDuDiagMat(double* matrix, long m, long n)
{
	...
	for (i = 0; i < m * n; i += m + 1)
		for (j = i + 1; ((j < i + m) && (j < m * n)); j++)
			if (fabs(matrix[j]) > MKleinsteZahl) knall = 1;
	if (!knall) diag = 1;
	return diag;
} /* of MBistDuDiagMat */
...
{
#ifdef ERROR_CONTROL
	if (vec == NULL) DisplayErrorMsg("Fehler in MLoeschVec");
#endif
	vec = (double*)Free(vec);
} /* MLoeschVec */
...
#ifdef ERROR_CONTROL
	if (zwerg == NULL) DisplayErrorMsg("zuwenig Speicher in MMachMat");
#endif
	return zwerg;
} /* MMachMat */
...
void MNullMat(double* zwerg, long m, long n)
{
	register long i;
#ifdef ERROR_CONTROL
	if (zwerg == NULL) DisplayErrorMsg("Fehler in MNullMat");
#endif
	for (i = 0; i < m * n; i++)
		zwerg[i] = 0.0;
} /* MNullMat */
...
{
#ifdef ERROR_CONTROL
	if (mat == NULL) DisplayErrorMsg("Fehler in MLoeschMat");
#endif
	mat = (double*)Free(mat);
} /* MLoeschMat */
...
int MOmega2D_9N(double* vf, double r, double s)
{
	...
	vf[3] =
	    0.25 * (1.0 + r) * (1.0 - s) - 0.5 * vf[6] - 0.5 * vf[7] - 0.25 * vf[8];
	vf[2] =
	    0.25 * (1.0 - r) * (1.0 - s) - 0.5 * vf[5] - 0.5 * vf[6] - 0.25 * vf[8];
	vf[1] =
	    0.25 * (1.0 - r) * (1.0 + s) - 0.5 * vf[4] - 0.5 * vf[5] - 0.25 * vf[8];
	vf[0] =
	    0.25 * (1.0 + r) * (1.0 + s) - 0.5 * vf[4] - 0.5 * vf[7] - 0.25 * vf[8];
	return ok = 1;
}
 ...
 **************************************************************************/
void MQSort_LongDouble(void* DataSets, const int NumberOfDataSets,
                       const int SizeOfDataSet)
{
	qsort(DataSets, NumberOfDataSets, SizeOfDataSet,
	      MCompare_for_MQSort_LongDouble);
}
...

{
	...
	if (d[0] > MKleinsteZahl || d[4] > MKleinsteZahl || d[8] > MKleinsteZahl)
		...
		if (vg > MKleinsteZahl)
		{
	      ...
	      /* end if (d[0] > MKleinsteZahl || d[4] > MKleinsteZahl || d[8] >
	       * MKleinsteZahl) */
...
#ifdef ERROR_CONTROL
	if (zwerg == NULL) DisplayErrorMsg("zuwenig Speicher in MMachVec");
#endif
	return zwerg;
} /* MMachVec */
...
#endif
{
	...
	zwerg = (double*)Malloc(sizeof(double) * g);
#ifdef ERROR_CONTROL
	if (zwerg == NULL) DisplayErrorMsg("Fehler in MNullVec");
#endif
#ifdef SX
#pragma cdir nodep
#endif
